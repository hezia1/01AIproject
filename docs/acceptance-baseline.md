...
| 后端自动化测试 | 已验证 | 317 passed、1 skipped、0 failed | 必须使用 D 盘临时目录 |
| 前端生产构建 | 已验证 | `npm run build` 成功；仓库内 AGENT、SCA/SAST 与 SANDBOX 浏览器冒烟覆盖 1600px/390px | 主包约 606.31 kB；Vite 仍有分包体积告警 |
...
| 本地两角色登录 | 部分验证 | scrypt 密码哈希、HttpOnly 会话、首次管理员初始化、普通用户管理员接口 403、双角色浏览器冒烟 | 不等于生产 IAM；尚无外部身份源、密码恢复、项目级隔离和独立安全评估 |
