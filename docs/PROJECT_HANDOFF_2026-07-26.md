...
- 项目源码不会作为未来 AGENT 运行时的直接挂载源；预检只按名称统计敏感文件类别，要求后续在 `artifacts/agent-sandbox/staging/<project-id>` 创建排除 `.env`/凭据/私钥/VCS 元数据的过滤副本并生成摘要。当前阶段未创建任何副本。
...
- 当前是本地只读多格式解析、规则检查、离线情报和静态数据流建模，不会启动真实 Agent、连接 MCP Server、调用插件工具或验证运行时行为。
...
8. **受控沙箱预检与证据模型（已完成第一阶段）**：已完成强制不执行的计划、敏感文件名清点、隔离策略、D 盘 staging 要求、静态路径候选和证据结构；尚未创建 staging、拉取镜像或运行目标。
9. **过滤 staging 与无害夹具（推荐下一子阶段，需再次确认）**：实现可审计复制清单、排除规则、链接/越界拒绝、字节/文件上限和 staging SHA-256，仅用仓库自带无害夹具验证容器策略；不得使用真实未知 Agent。
10. **指定 Agent 运行时验证（单独批准）**：只有用户明确批准具体目标、digest 镜像、命令和资源边界，且如需下载已提前说明后，才可采集真实进程/文件/网络/工具调用证据。
11. **可选真实 AI 分析**：只有用户批准代码/配置发送边界和 API 费用后，才可设计 AGENT 专用模型复核；它不能复用文案冒充已实现能力。
