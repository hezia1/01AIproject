...
- SCA、SAST、AGENT、DAST、SANDBOX 统一为模块状态、四项核心指标、完整结果筛选、每页 10 条分页和建议动作。
- 各个可独立运行模块均提供单独执行入口；SCA、SAST、AGENT 的再次扫描会保留扫描批次，并对比“仍然存在、已经消失、新增、发生变化”。
- 复测明细默认折叠，避免页面堆砌；首次启用历史保留时会提示旧批次匹配的边界。
- 一键执行仅采用高置信度自动关联建议；治理风险清单可以从单个 Finding 展开显式 DAST / SANDBOX 证据链。
- 复测对比 API：`GET /api/findings/projects/{project_id}/retest-comparison?source=SAST|SCA|AGENT`。
- 综合总览已按 PPT 恢复为安全闭环：多源发现、等待验证、DAST 验证、SANDBOX 取证、整改/复测和已闭环。
- 综合总览优先展示三条可读风险证据链，并恢复显式关系生成的跨模块攻击链；可从风险链直接进入 DAST 或 SANDBOX。
- DAST 前端改为关联风险的动态验证中心，未关联历史记录作为 Web 基础检查展示，不计入漏洞闭环。
- SANDBOX 前端改为关联风险或 DAST 验证的运行时取证中心，并展示文件、网络、进程和工具调用账本。
- 安全知识中枢前端第一版已加入主导航，组织项目上下文、规则命中、动态验证、运行证据和治理结论，同时明确自主演进尚未实现。

### 还未完成

- 风险分规则较简单，尚未接 CVSS、EPSS、资产暴露面、业务重要性。
- 自动关联当前为规则评分 v1，不是语义模型；低置信度候选不会自动预选。
- 当前是显式关系图，不是真正的图数据库或 AI 图谱推理。
- 没有 SLA 管理。
- 没有工单系统接入。
- 已有扫描复测对比，但没有包含审批、工单和 SLA 的完整整改闭环流程。
- 没有合规报告。
- 没有管理层报表导出。
...
- ASPM 下一步建议增加项目级风险趋势和 SLA 第一版，把当前复测对比纳入趋势，再考虑图数据库或 AI 推理。
