...
- 已具备本地来源声明、版本锁定状态和文件/目录 SHA-256 证据，但哈希只用于本地批次比较，发布者仍是未验证声明；数字签名、Registry 身份、远端制品哈希、已知漏洞和恶意包情报尚未验证或关联。
...
5. **来源完整性（已完成本地基础）**：包/版本/来源/安装方式、文件及受限目录 SHA-256、来源与哈希差异和门禁已实现；签名、Registry 身份和漏洞情报仍是后续阶段。
6. **可选动态验证**：只有 PPT 和用户明确要求时，才考虑在 SANDBOX 内启动受控 MCP/Agent、采集工具调用和行为证据；不得直接在宿主机执行未知 Agent 或插件。
7. **可选真实 AI 分析**：只有用户批准代码/配置发送边界和 API 费用后，才可设计 AGENT 专用模型复核；它不能复用文案冒充已实现能力。
