...
- 本次仅通过模拟 Docker 调用测试执行器，没有真实运行任何项目 Agent，也没有下载镜像；首次真实目标验证仍必须由用户针对具体目录、命令、digest 镜像和资源边界单独批准。
...
- 静态扫描本身仍不会启动 Agent、连接 MCP Server 或调用插件工具。指定目标执行器已经实现但默认关闭且尚未对任何项目目标真实运行；只有单独开启项目策略并完成精确绑定和二次确认后才可能执行。
...
12. **指定 Agent 运行时验证（执行器已完成，首次目标仍待单独批准）**：默认关闭、绑定清单、固定 Docker 策略、有限观测证据、报告/前端/信任分接入已实现；尚未运行项目 Agent，完整子进程/文件/网络尝试/工具调用插桩仍未完成。只有用户明确批准具体目标、digest 镜像、命令和资源边界，且如需下载已提前说明后，才可首次真实执行。
