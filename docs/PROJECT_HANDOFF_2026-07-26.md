...
- 当前指定目标证据可观察主进程、容器策略和工作区前后完整性；适配平台 stdio 观察器的 MCP 目标还可观察脱敏方法调用和 MCP Server 子进程。逐文件访问、系统级子进程、网络尝试目的地以及 SSE/Streamable HTTP 等网络 MCP 传输仍未插桩，因此只给运行分 7/10、总分最高 95、置信度最高为中。
- stdio 观察器由平台目录只读挂载，不修改 staging；它代理逐行 JSON-RPC，只记录方法、脱敏目标、字节数、结果、耗时、元数据哈希及 MCP Server 启停元数据，不保存请求参数、响应结果、标准输出或标准错误内容。后端按固定 Schema、字段白名单、事件/行数上限和重算 SHA-256 校验并生成账本。
- 该日志通道对目标可见，并非密码学认证或不可抵赖通道；恶意目标可能伪造同前缀事件。平台和前端必须继续明确这一边界，不能把“格式与哈希已验证”写成“行为真实性已证明”。
- 2026-08-14 已在用户批准后，对仓库内自建代表性 MCP 集成目标完成更新后的真实目标验证；没有修改 DVNA、没有下载镜像或依赖。独立项目 ID `76292d96-1c24-4284-aded-27938798a053`，扫描批次 `cbd2a358-ee7f-4045-aa3b-5604759eb337`，8 个 staging 文件，26/26 项运行策略检查通过，退出码 0、未超时、容器已删除，信任分 82。
- 本次精确绑定：计划 SHA-256 `793e8bdae5ec14112a878b9a71e356f3e81a7dd2d312e23f2187f89389731660`，staging SHA-256 `5656c23040a2e11620474033b4fd70f945733274fe0bf8bc58d2ceab43d17ec7`，manifest SHA-256 `dab16e33e7a944f7f76cf68ef19ee2e4f30621cb28f01f6e80c0ee9a70826607`，目标证据 SHA-256 `45b9b87ff540fb97909f7816677c8aab2ec41e4442df69cfc8a176c08ff015be`。生成物位于 D 盘 Git 忽略目录，不提交仓库。
- 真实运行暴露并修复了 Docker `local` 日志驱动 `max-file=1` 与默认压缩不兼容的问题；执行器现在显式使用 `compress=false` 并在 inspect 检查中验证，仍保留 1 MiB 单文件上限。
- 代表性 MCP 夹具的真实账本记录 17 条有效事件、0 条拒绝事件：initialize、tools/list/call、resources/list/read、prompts/list/get 共 7 次请求和 7 次响应，另含 1 次无需响应的 `notifications/initialized`，并记录 1 次工具调用与 1 个 MCP Server 子进程。文件访问、系统级子进程和网络目的地仍未插桩，不得把本次结果表述为完整行为取证或生产安全认证。

- 2026-08-14 已新增 staging 绑定的 stdio MCP 自动能力探测：平台从已验证 staging 的 JSON/YAML/TOML `mcpServers` 声明中解析候选，不要求项目客户端主动接入观察器。候选命令由配置和文件 SHA-256 派生，API 请求不能替换命令。
...
- 受控运行面板已将预检、创建过滤副本、无害夹具和指定项目 Agent 真实运行分成四个入口；真实目标入口显示项目默认关闭策略、绑定副本、固定确认短语、最近策略结果、有限插桩范围和证据位置，不展示 Agent 标准输出。若证据包含 MCP 账本，页面会显示请求/成功响应/工具调用/资源读取/Prompt 获取汇总及逐条响应事件哈希，并提示日志并非密码学认证通道。
- 列表展示等级、分类、标题、文件/行号、证据、修复建议和信任影响，支持每页 10 条分页。
- 单独执行 AGENT 时只锁定 AGENT 模块，不应导致其他模块置灰或触发扫描。

- 受控运行面板新增独立的“从已验证配置自动探测 stdio MCP Server”入口：可选择 staging 和 Server，查看资格检查/阻断原因、脱敏命令、独立确认短语、Server 身份、工具/资源 scheme/Prompt 清单及证据 SHA-256；页面明确说明未调用工具或读取内容，也不把单 Server 探测写成整套 Agent 验证。

### 尚未完善或容易产生歧义的部分

- 静态扫描本身仍不会启动 Agent、连接 MCP Server 或调用插件工具。指定目标执行器默认关闭；现已对仓库内代表性 MCP 目标完成用户批准的真实运行并取得 stdio MCP 有限账本，但任何其他项目仍须单独开启策略、完成精确绑定和二次确认。
