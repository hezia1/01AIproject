...
| DAST | 已确认项目同源目标的一次无认证 GET 基础观察、验证策略、显式风险/组件关联与可解释的关联建议；自动结果只记录“需复核 / 未发现异常 / 未完成”。前端可录入、编辑和复核带证据与复现步骤的人工三态裁决；自动基础观察只读。已提供验证计划、授权范围、审批记录、仅文档运行台账、脱敏证据摘要/哈希、结论关联与专项 JSON 报告；这些本地台账操作不连接目标。 | 爬虫、登录态管理、攻击 payload、业务漏洞利用证明、OWASP ZAP/Nuclei 集成、自动复现与自动复测；截图、原始响应正文和完整执行日志归档。 |
