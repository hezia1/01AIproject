...
| DAST | 已确认项目同源目标的一次无认证 GET 基础观察、验证策略、显式风险/组件关联与可解释的关联建议；自动结果只记录“需复核 / 未发现异常 / 未完成”。前端可录入、编辑和复核带证据与复现步骤的人工三态裁决；自动基础观察只读。可导出仅汇总已保存记录的 DAST JSON 专项报告，导出不连接目标。 | 爬虫、登录态管理、攻击 payload、业务漏洞利用证明、OWASP ZAP/Nuclei 集成、自动复现与自动复测；截图、原始响应正文和完整执行日志归档。 |
...
- DAST：`POST /api/dast/probe`、`POST /api/dast/validations`、`PATCH /api/dast/validations/{validation_id}`、`GET /api/dast/projects/{project_id}/validations`、`GET /api/dast/projects/{project_id}/report`。自动基础观察要求目标与项目 `runtime_url` 或 `api_base_url` 同源，并要求输入精确确认短语；只验证 HTTP/HTTPS、状态、耗时、Server Header 和基础安全响应头，不跟随重定向、不读取正文、不发送 payload。自动结果不能证明 SQL 注入、鉴权绕过等业务漏洞可利用，也不会把未发现异常写成不可利用；自动基础观察只读。专项报告只读取已保存记录，不连接目标。
