...
管理员配置现支持本地 SCA 策略覆盖与 SAST 正则/YAML 编辑、SANDBOX 镜像下载白名单，以及 Grype、Semgrep、SCA 依赖解析和 SANDBOX 准备资源的联网权限。普通用户仍可在管理员允许时手动更新 Grype 和 Semgrep。数据记录列表统一每页 10 条，规则错误不再伪装成空列表；新增组件已独立拆分。生效范围、离线限制与剩余事项见 [管理员配置说明](docs/admin-configuration.md)。
...
`/api/health` 返回结构化的 API、PostgreSQL、Redis、Docker、固定 SCA 工具镜像和 Semgrep 就绪状态；必需依赖不可用时返回 HTTP 503，可选依赖不可用时返回 `degraded`。`?include_optional_tools=false` 可用于认证初始化时的快速依赖检查。检测页的项目级真实诊断另行展示情报时效、模型配置与最近调用证据、被测目标和各模块最近任务；工具就绪、模型已配置或任务完成都不代表无漏洞。
...
控制台固定地址：<http://127.0.0.1:5173>。开发服务器只监听本机 IPv4 回环地址并使用严格端口模式；若 5173 已被占用，启动会明确失败，不会静默切换端口。
...
4. 按项目启用需要的安全模块并执行 SCA、SAST 和/或 AGENT。SCA 按概览、风险组件、依赖影响、例外与 VEX、历史与报告组织；SAST 按概览、代码风险、AI 辅助复核、扫描策略、例外与报告组织，Semgrep 作为补充本地规则覆盖面的 SAST 增强引擎在“扫描策略”中直接可见，不属于 SCA。AGENT 治理页面按概览、风险、资产与边界、动态验证、策略与交付五个工作区复核 Finding、扫描覆盖、运行证据和降级原因。风险页只把可处置 Finding 计入“正式问题”，覆盖缺口等内容单列为“扫描提示”，动态验证按“运行条件 → 安全副本 → 验证方式 → 确认执行 → 查看证据”逐步展开。治理页面的说明、限制和证据摘要统一使用中文，业务时间统一标注为北京时间；运行探针等需要精确匹配的授权确认口令保留原始英文。
   Web 一级入口统一为“项目、检测、风险治理、安全知识中枢、报告”；管理员额外看到独立“管理中心”。普通用户仍可检测和更新 Grype 数据库，并可在确认 Semgrep Rules License 后下载或更新社区规则；规则编写、项目 Semgrep YAML 发布、漏洞情报导入和知识审核由管理员执行。
   管理中心按“用户管理 / 模块配置 / 知识审核”组织，模块配置分别展示 SCA、SAST、AGENT、DAST、SANDBOX。SAST 抑制、默认扫描政策与门禁、SCA VEX 写入仅限管理员；普通用户保留 Semgrep 引擎/社区规则开关和模块启停。通用模块接口不允许普通用户写入配置，重新接入模块也不会清除已有规则与策略。DAST 用户在既有边界内自行确认授权并执行，不需要逐次等待管理员审批；DAST/SANDBOX 入口复用现有工作台，未新增公共规则编辑或隔离策略配置系统。
...
十一组冒烟的服务、浏览器、临时账号及清理要求见 [`apps/web/README.md`](apps/web/README.md)。反馈和分页用例需要 Vite 开发服务；生产构建通过不代表这些浏览器用例已执行。
...
- 当前只有本地两角色认证，没有生产级 IAM、跨租户权限执行、签名报告、分布式任务系统或敏感证据独立加密存储。
