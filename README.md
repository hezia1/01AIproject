...
| DAST | 已确认项目同源目标的一次无认证 GET 基础观察、验证策略、显式风险/组件关联与可解释的关联建议；自动结果只记录“需复核 / 未发现异常 / 未完成”，人工验证才可记录三态裁决；动态验证中心和历史已在前端开放。 | 爬虫、登录态管理、攻击 payload、业务漏洞利用证明、OWASP ZAP/Nuclei 集成、自动复现与自动复测；截图、原始响应正文和完整执行日志归档。 |
