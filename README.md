...
- `preflight-ready` 只表示本地配置具备来源与凭据引用声明，绝不表示来源可连通、凭据有效、身份已认证、制品可信或 Agent 已安全。
- 扫描不执行 DNS、HTTP、Git、Registry、容器或 MCP 操作；私有来源实际连接仍必须单独说明具体目标、权限和风险，并取得确认。

## 2026-08-14 AGENT 配置类型识别与可选覆盖门禁

- 扫描器除固定文件名外，也会在受识别的 `plugins/` 与 `tools/` 目录内识别 `manifest.*` 和 `tool.*` 配置；该目录约定只扩大静态资产发现，不读取目录外文件、不执行配置内容。
- 项目质量门禁新增两项默认关闭的策略：阻断“仅通用结构化解析”的资产，以及阻断“声明但未校验”的 Schema 引用。启用后只反映当前静态覆盖缺口，不将其表述为漏洞、不可利用或运行时行为。
- 仍不会下载、联网获取或验证 Schema，也不会连接、执行任何 Agent、MCP、插件或工具。

## 2026-08-14 AGENT 配置适配与扫描覆盖基线

- 每个已发现的 AGENT、MCP、插件和工具配置资产都会记录实际使用的本地解析适配器、解析状态和验证层级；该记录描述扫描器已执行的解析契约，不将通用结构化解析表述为厂商 Schema 完整校验。
- 扫描 API、历史快照和本地 `scripts/agent_ci.py` 报告均统计适配器覆盖：结构化解析、通用解析、解析失败，以及声明但未校验的 `$schema` 引用。
- 前端扫描覆盖面板会显示通用解析资产和未校验 Schema 引用，并可展开查看每种适配器的覆盖缺口与限制。
- 本阶段不下载或联网获取 Schema，不连接 Agent/MCP/插件目标，不执行其代码，也不增加系统级文件、进程或网络行为观测；这些限制会保留在报告证据边界中。
...
- 新增 staging 绑定的远程 MCP 候选发现和受控探测，兼容 `2026-07-28` 无状态 Streamable HTTP、2025 会话式 Streamable HTTP、Streamable HTTP 的 SSE 响应，以及已弃用但仍常见的旧 HTTP+SSE。新协议优先使用 `server/discover`，旧协议安全回退到 `initialize`，随后只执行 `tools/list`、`resources/list` 和 `prompts/list`。
- 候选必须来自当前扫描批次的已验证 JSON/YAML/TOML staging 配置。API 请求只能提交 staging、候选和哈希绑定，不能替换 URL。只允许 HTTPS 标准端口；HTTP、URL 凭据/查询参数、自定义 Header、环境变量、认证字段、本地命令和不可识别传输均会阻断。
- 执行前重新解析 DNS；只要返回地址中包含回环、私网、链路本地、保留或元数据目标即整体拒绝。固定探测器只连接预先批准的公网 IP，使用原始主机名完成 TLS 校验，并阻断跨域重定向。旧 SSE endpoint 的会话查询参数只在内存使用，不写入证据。
- 探测在本地 digest 镜像的只读、非 root、drop-all、资源受限 Docker 容器中运行，使用 `--pull=never`。与 stdio 探测不同，该固定容器需要 bridge 网络；目的地限制由固定脚本、预解析公网 IP 和同源策略实施，目前不是独立 egress 防火墙。
- 证据只保存协议模式、Server 身份声明、工具/Prompt 名、资源 URI scheme、HTTP 状态、响应字节数、实际公网 IP、重定向摘要和哈希；不保存响应正文，不发送配置凭据，不调用工具或读取资源/Prompt 内容。私有 MCP 因无凭据可能拒绝请求或仅返回公开能力。
- 后端提供独立的 status/evidence/validation API，前端可选择精确 staging 与 endpoint、查看阻断原因、输入独立短语 `PROBE REMOTE MCP SERVER` 并查看协议与网络证据。信任算法升级为 `agent-trust-static-1.3`，远程清单进入摘要但不提高整个 Agent 的运行评分。
- 本地协议集成测试覆盖现代 JSON、现代 SSE 响应和旧 HTTP+SSE，验证不会发送 Authorization/Cookie、不会保存描述正文或 SSE 会话参数。没有连接第三方生产 MCP，也没有下载镜像、依赖或测试项目。
...
- 本次能力探测证据 SHA-256 为 `694424c6e444…`，内容动作明确为 `false`，输出内容不保存。信任算法现为 `agent-trust-static-1.3`，但 Server 能力清单不等于整个 Agent 运行证明，因此该次实测信任分仍为 82。
...
- 新增本地镜像只读发现、历史证据读取和二次确认执行接口：`GET /api/agent/projects/{project_id}/runtime-fixture-status|runtime-fixture-evidence`、`POST /api/agent/projects/{project_id}/runtime-fixture-validation`。它们只允许仓库固定无害夹具和 `python@sha256:<digest>` 本地镜像，运行命令不可由请求修改。
- 容器固定使用 `--pull=never`、禁网、只读根文件系统、只读过滤 staging、非 root 用户、`cap-drop ALL`、`no-new-privileges`、无宿主 Socket/环境注入、0.5 CPU、128 MiB 内存、32 PID、16 MiB `noexec/nosuid` 临时区和 1–15 秒超时；不使用 Shell 拼接命令。
- 证据同时核对 Docker `inspect` 配置和容器内无害探针：根目录/工作区写入被阻断、外联被禁网阻断、宿主 canary 未注入、有效 capabilities 为零、`NoNewPrivs=1`、临时区可写、cgroup CPU/内存/PID 限额生效。任一检查失败都返回 `block`。
- 2026-08-12 已使用本地 `python@sha256:423ed6ab…` 和本机 named-pipe Docker Context 完成真实无害验收，无下载；30/30 项检查通过、退出码 0、未超时，临时容器已删除。staging SHA-256 为 `c4488238e024…`，最终证据 SHA-256 为 `12b6ba8666b6…`，完整证据仅保存在 D 盘 Git 忽略目录。
- 前端已显示本地 digest 镜像、明确的真实无害夹具确认、最近一次结果、检查通过数和证据位置。该能力不接受项目源码、真实 Agent、MCP Server、插件或自定义命令，`execution_enabled_for_real_agents=false`。
...
- 仓库新增确定性离线无害夹具 `apps/api/tests/fixtures/agent_runtime_safe`：复制层使用全部夹具文件，真实验收只允许固定 `policy_probe.py`，不能替换为项目代码或自定义命令。
- 无害夹具容器策略验收已在单独确认后完成；后续指定真实 Agent 仍需再次确认具体目标、digest 镜像、固定命令和资源边界。若缺少资源，必须先说明用途、体积与 D 盘存储位置，再由用户决定是否下载。
...
- 未受控运行的目标仍只计预检计划 3/10，静态总分封顶 90；完成当前有限观测的目标运行后，运行分项为 7/10、总分最高 95，证据置信度仍最高为中。仓库无害策略夹具不增加目标信任分；代表性 MCP 目标本次实测批次的信任分为 82。
...
仍未实现、也不应被宣称为已实现的是：自动修改源码或提交补丁/PR、可执行工具的自治 Agent、跨服务或动态调度的全程序污点分析、外部漏洞知识库/RAG 自动学习，以及联网规则/镜像自动拉取。DAST/SANDBOX 只会在审批范围内执行内置固定安全探针，不支持自治生成命令、任意攻击脚本或突破目标白名单。当前七角色由平台顺序编排真实模型调用，不是可自行执行命令或操作仓库的自治进程。
...
| DAST | 当前批次的 SAST/AGENT Finding 自动归一化为运行目标、HTTP 方法、参数、前置身份和证据标准，只把无法从项目配置及 Finding 证据推断的内容列为阻塞项；SCA 不进入 DAST 队列。支持同源资产发现、多角色会话、Dry Run、审批后的受限 API 执行、固定 Playwright 浏览器步骤、截图/HAR/控制台证据、证据哈希、状态日志、三色裁决和专项报告。 | 录屏、DNS 外带、敏感证据独立加密存储、OWASP ZAP/Nuclei 适配和自动修复复测尚未完成；缺少固定运行镜像时会明确阻止，不会伪装成已执行。 |
| SANDBOX | 新工作台按能力预检、项目目标、DAST 自动队列、执行与证据四段构成闭环。支持已上线同源目标，或从单仓/多应用源码识别启动入口，在内部网络中编排非 root PostgreSQL/Redis 依赖和主应用。固定执行器支持隔离 HTTP、差分/时延/OAST 探针以及同源只读 Playwright 取证；截图、脱敏 HAR、控制台和覆盖证据以 SHA-256 索引后回传 DAST。启动失败按镜像、依赖/入口、端口、依赖可达性、只读策略、资源和健康检查分类并给出脱敏修复建议。 | Agent 运行时适配器仍待接入；DNS OAST、eBPF/Sysmon、恶意样本级强隔离、分布式 Worker 与证据对象存储/独立加密仍未实现。 |
...
- AGENT：`POST /api/agent/scan`；`GET /api/agent/projects/{project_id}/findings|scan-history|snapshot|scan-diff|gate|report|sarif|report.html|ci-config`；`POST /api/agent/projects/{project_id}/runtime-preflight|runtime-staging|runtime-fixture-validation|runtime-target-validation|runtime-mcp-probe-validation|runtime-remote-mcp-probe-validation`；`GET /api/agent/projects/{project_id}/runtime-fixture-status|runtime-fixture-evidence|runtime-target-status|runtime-target-evidence|runtime-mcp-probe-status|runtime-mcp-probe-evidence|runtime-remote-mcp-probe-status|runtime-remote-mcp-probe-evidence`；`GET/PATCH /api/agent/projects/{project_id}/profile`；`POST /api/agent/projects/{project_id}/exceptions`；`PATCH /api/agent/projects/{project_id}/exceptions/{exception_id}`。预检不复制或执行；staging 只创建过滤副本；fixture validation 只运行固定仓库夹具。指定目标执行和本地/远程 MCP 能力探测默认关闭，要求精确 staging/镜像/候选绑定、独立确认短语和本地 digest 镜像，全部使用 `--pull=never`；能力探测不调用工具或读取内容，静态规则、夹具或一次运行通过都不等于项目安全。
- AGENT 单文件上限为 512 KiB，单资产最多持久化 500 条去重权限；超过上限会在快照元数据和前端资产结果中明确显示截断数量。
- DAST：除旧有 `probe`、人工 `validations`、验证计划/台账和 JSON `report` 接口外，`POST /api/dast/projects/{project_id}/discover` 会执行确认后的同源只读资产发现，结果持久化到 `dast_asset_discoveries`，可由 `GET /api/dast/projects/{project_id}/discoveries/latest` 恢复；HTML、表单、Query、外部 JavaScript、JSON/OpenAPI 路径和参数会去重归档。`GET /api/dast/projects/{project_id}/report.html` 生成不联网的可阅读专项报告。自动基础观察仍不能代替业务漏洞裁决；旧计划运行仍是仅文档记录。
- 业务流程 DAST：`GET /api/dast/projects/{project_id}/business-candidates|business-flows`、`POST /api/dast/business-candidates/{finding_id}/materialize`、`POST /api/dast/business-flows`、`PATCH /api/dast/business-flows/{flow_id}`、`GET /api/dast/business-flows/{flow_id}/preflight`、`GET/POST /api/dast/business-flows/{flow_id}/runs`、`GET /api/dast/business-runs/{run_id}/snapshots`、`PATCH /api/dast/business-runs/{run_id}/verdict`。候选仅接收当前项目当前批次的 SAST/AGENT Finding，并从项目 URL、Finding 证据、规则、CWE 和 AI 复核中提取目标、路径、方法、参数和证据标准。SQL 注入使用重复基线与真/假条件差分，越权使用双身份响应差分；XSS、SSRF、命令注入、路径穿越、XXE、模板注入、开放重定向、CORS、文件上传和 Agent 运行验证会声明所需隔离能力。Dry Run 不联网；有界 HTTP 执行要求审批、同源、路径白名单、最多 40 个请求和限速。每个角色使用独立 Cookie Jar，Bearer/API Key/Cookie 仅允许 `env:变量名` 引用并自动注入；请求/响应脱敏后关联策略、任务、请求和日志 ID。目标不可达、执行器缺失或安全策略阻止属于未验证，不计入三色裁决。
- DAST ↔ SANDBOX：`POST /api/dast/business-flows/{flow_id}/sandbox-runs` 生成 `ai-security-platform.dast-sandbox-handoff/v1` 合同，包含任务/策略/项目/Finding ID、审批范围、路径、资源限制、网络策略、能力和证据要求；一次性回调 Token 只以哈希写入 DAST 快照。SANDBOX 通过 `POST /api/dast/business-runs/{run_id}/sandbox-result` 回传浏览器、截图/录屏、HAR、OAST、时延、环境和运行轨迹等事实。DAST 会重新校验证据强度：可利用必须有明确确认事实，不可利用必须有至少两组探针的完整覆盖；否则只能裁决不确定。阻塞/失败任务不会伪造 ANALYZING、VERDICTED、REPORTED 状态。
- DeepSeek 业务草案：在 `apps/api/.env` 填入新的 `DAST_DEEPSEEK_API_KEY`（不要提交该文件），可选配置见 `apps/api/.env.example`。接口要求精确确认短语，且只发送人工提供、已脱敏的目标域名/路径/API 说明/业务描述；不会发送 Cookie、Token、密码、原始响应、截图或个人数据。模型只生成待审批的结构化草案，不能执行请求或直接裁决漏洞。
- SANDBOX：`GET /api/sandbox/capabilities`；`GET /api/sandbox/projects/{project_id}/launch-plan|targets|tasks`；`POST /api/sandbox/projects/{project_id}/targets`；`POST /api/sandbox/targets/{target_id}/health|stop`；`GET /api/sandbox/tasks/{task_id}/events`；`POST /api/sandbox/tasks/{task_id}/execute|cancel`。多服务启动只接受从 Compose 提取且经白名单重建的 PostgreSQL/Redis 依赖，忽略 Compose 中的密钥、宿主端口、挂载和任意命令。浏览器执行镜像由 `apps/api/app/services/sandbox_browser_executor.Dockerfile` 构建，并在能力预检中真实校验 Python Playwright 包。
...
- DAST 需要已上线的同源 Web 地址，或由 SANDBOX 使用项目源码与经校验启动计划创建的临时 Web 实例。SANDBOX 需要 Docker Desktop；只会对本地确定性计划所需的固定官方运行镜像执行拉取，不会拉取项目任意镜像名。浏览器执行器使用本地锁定派生镜像。
