...
| DAST | 已确认项目同源目标的一次无认证 GET 基础观察、验证策略、显式风险/组件关联与可解释的关联建议；自动结果只记录“需复核 / 未发现异常 / 未完成”。前端可录入、编辑和复核带证据与复现步骤的人工三态裁决；自动基础观察只读。已提供验证计划、授权范围、审批记录、仅文档运行台账、脱敏证据摘要/哈希、结论关联与专项 JSON 报告；这些本地台账操作不连接目标。另有独立的业务流程验证工作台：把 SCA/SAST/AGENT Finding 适配为候选，支持多角色独立会话、API 流程、变量提取、受限重放、Dry Run、逐步快照、三色裁决和 DeepSeek 待审批草案。 | 浏览器自动化录制/执行尚待批准并安装运行时；爬虫、攻击 payload、OWASP ZAP/Nuclei 集成、原始响应正文、敏感证据加密归档、自动复现与自动复测尚未完成。页面步骤目前会明确阻止，不会被表述为已执行。 |
...
- DAST：`POST /api/dast/probe`、`POST /api/dast/validations`、`PATCH /api/dast/validations/{validation_id}`、`GET /api/dast/projects/{project_id}/plans`、`POST /api/dast/plans`、`PATCH /api/dast/plans/{plan_id}`、`GET/POST /api/dast/plans/{plan_id}/runs`、`PATCH /api/dast/runs/{run_id}`、`GET/POST /api/dast/runs/{run_id}/evidence`、`GET /api/dast/projects/{project_id}/validations`、`GET /api/dast/projects/{project_id}/report`。自动基础观察要求目标与项目 `runtime_url` 或 `api_base_url` 同源，并要求输入精确确认短语；只验证 HTTP/HTTPS、状态、耗时、Server Header 和基础安全响应头，不跟随重定向、不读取正文、不发送 payload。自动结果不能证明 SQL 注入、鉴权绕过等业务漏洞可利用，也不会把未发现异常写成不可利用；自动基础观察只读。验证计划范围变更会撤销既有审批；当前运行台账固定为仅文档记录。证据摘要会对常见凭据字段脱敏后计算哈希，专项报告只读取已保存记录，不连接目标。
- 业务流程 DAST：`GET /api/dast/projects/{project_id}/business-candidates|business-flows`、`POST /api/dast/business-flows`、`PATCH /api/dast/business-flows/{flow_id}`、`GET/POST /api/dast/business-flows/{flow_id}/runs`、`GET /api/dast/business-runs/{run_id}/snapshots`、`PATCH /api/dast/business-runs/{run_id}/verdict`。候选只适配已观察到的 SCA/SAST/AGENT Finding；目标、参数、身份等缺失信息不会被伪造。Dry Run 不连接目标；API 运行要求审批、同源项目目标和精确确认短语。会话凭据只允许 `env:变量名` 引用，逐角色隔离且不写入快照。仅允许 GET/HEAD/OPTIONS 和明确标记的登录 POST；删除、支付、发信、上传和真实业务数据修改仍被禁止。浏览器步骤尚无运行时，会产生阻止快照而非连接目标。
