...
type ScaGatePolicy = { enabled: boolean; block_severities: string[]; block_license_policies: string[]; min_risk_score: number; block_kev: boolean; max_scan_age_hours: number; require_intelligence_for_critical: boolean; source?: string };
...
function ScaIntelligencePanel() {
  ...
  useEffect(() => { void refresh(); }, []);
  return <section className="panel full"><div className="panel-header"><div><h3>CVSS / EPSS / KEV 离线情报</h3><span>风险分、在野利用和修复版本只使用实际导入的数据</span></div><button className="secondary-action" disabled={saving} onClick={() => void refresh()}>刷新状态</button></div><div className="kv-list"><div><span>状态</span><strong>{textValue(status?.status)}</strong></div><div><span>通告数</span><strong>{textValue(status?.advisory_count)}</strong></div><div><span>更新时间</span><strong>{textValue(status?.updated_at)}</strong></div></div><details><summary>导入离线情报 JSON</summary><p>每项使用 <code>id</code> 或 <code>cve</code>，可选 <code>cvss</code>（0–10）、<code>epss</code>（0–1）、<code>kev</code>、<code>known_exploited</code>、<code>fixed_version</code>。</p><div className="filter-grid"><input value={source} onChange={(event) => setSource(event.target.value)} placeholder="情报来源标识" /><textarea value={rawEntries} onChange={(event) => setRawEntries(event.target.value)} placeholder='[{"cve":"CVE-2026-0001","cvss":9.8,"epss":0.91,"kev":true,"fixed_version":"2.0.1"}]' /><button className="secondary-action" disabled={saving || !rawEntries.trim()} onClick={() => void importEntries()}>{saving ? "导入中" : "导入情报"}</button></div></details>{error ? <div className="report-error">{error}</div> : null}</section>;
}
