...
type AgentProfile = { profile_version: number; rule_version: string; disabled_rule_ids: string[]; excluded_paths: string[]; permission_allowlist: AgentAllowlistItem[]; required_approval_capabilities: string[]; exceptions: AgentException[]; audit_log: AgentAuditItem[]; quality_gate: AgentGatePolicy };
...
type AgentScanSnapshot = { project_id: string; scan_task_id: string; created_at: string; source_path: string | null; rule_version: string | null; assets: AgentAsset[]; permissions: AgentPermission[]; skipped_files: { path: string; reason: string }[]; quality_gate?: AgentQualityGate; intelligence?: AgentIntelligence };
...
function AgentGovernanceConsole({ project, snapshot }: { project: Project; snapshot: AgentScanSnapshot | null }) {
  ...
  async function saveProfile(nextProfile = profile) {
    ...
    try {
      const payload = { actor, disabled_rule_ids: nextProfile.disabled_rule_ids, excluded_paths: nextProfile.excluded_paths, permission_allowlist: nextProfile.permission_allowlist, required_approval_capabilities: nextProfile.required_approval_capabilities, quality_gate: nextProfile.quality_gate };
      setProfile(await request<AgentProfile>(`/agent/projects/${project.id}/profile`, { method: "PATCH", body: JSON.stringify(payload) }));
      setMessage("治理策略已保存；规则停用、路径排除、Allowlist 和门禁配置从下一次扫描开始生效。");
  ...
  return <details className="advanced-details governance-advanced-details" open><summary>AGENT 策略、例外审批、质量门禁与交付</summary><div className="advanced-details-body"><section className="content-grid">
    <div className="panel full"><div className="panel-header"><h2>项目扫描策略</h2><span>{message || `配置 v${profile.profile_version} · 规则 ${profile.rule_version}`}</span></div><p>保存后从下一次扫描生效；历史扫描与原始 Finding 不会被重写。操作人用于本项目策略审计，不等同于平台身份认证。</p><div className="filter-grid"><label>操作人<input value={actor} onChange={(event) => setActor(event.target.value)} /></label><label>停用规则 ID（每行一个）<textarea rows={4} value={profile.disabled_rule_ids.join("\n")} onChange={(event) => setProfile({ ...profile, disabled_rule_ids: splitLines(event.target.value) })} /></label><label>排除路径 glob（每行一个）<textarea rows={4} value={profile.excluded_paths.join("\n")} onChange={(event) => setProfile({ ...profile, excluded_paths: splitLines(event.target.value) })} placeholder="例如 fixtures/**" /></label><label>强制审批能力（每行一个）<textarea rows={4} value={profile.required_approval_capabilities.join("\n")} onChange={(event) => setProfile({ ...profile, required_approval_capabilities: splitLines(event.target.value) })} /></label><button className="primary-action" onClick={() => void saveProfile()}>保存项目策略</button></div></div>
    <div className="panel full"><div className="panel-header"><h2>质量门禁</h2><span className={`severity ${gate?.decision === "block" ? "high" : "info"}`}>{gate?.decision === "block" ? "阻断" : gate?.decision === "pass" ? "通过" : "等待扫描"}</span></div><div className="filter-grid"><label>Finding 阈值<select value={policy.threshold} onChange={(event) => setProfile({ ...profile, quality_gate: { ...policy, threshold: event.target.value as AgentGatePolicy["threshold"] } })}>{["critical", "high", "medium", "low", "info", "none"].map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>最大阻断 Finding 数<input type="number" min={0} value={policy.max_blocking_findings} onChange={(event) => setProfile({ ...profile, quality_gate: { ...policy, max_blocking_findings: Math.max(0, Number(event.target.value)) } })} /></label><label>情报最大年龄（天）<input type="number" min={1} max={3650} value={policy.max_intelligence_age_days} onChange={(event) => setProfile({ ...profile, quality_gate: { ...policy, max_intelligence_age_days: Math.max(1, Number(event.target.value)) } })} /></label>{([['enabled', '启用门禁'], ['block_new_only', '只阻断新增 Finding'], ['block_wildcard_permissions', '阻断通配权限'], ['block_parse_failures', '阻断结构化解析失败'], ['block_skipped_files', '阻断跳过文件'], ['block_permission_expansion', '阻断权限扩大'], ['require_approval_for_high_risk', '高风险权限必须声明审批'], ['block_unpinned_sources', '阻断未锁定依赖'], ['block_insecure_sources', '阻断不安全来源'], ['block_unknown_sources', '阻断来源未知'], ['block_partial_integrity', '阻断不完整哈希证据'], ['block_integrity_changes', '阻断完整性变化'], ['block_source_changes', '阻断来源变化'], ['block_known_vulnerabilities', '阻断已命中漏洞'], ['block_malicious_packages', '阻断恶意包情报命中'], ['block_package_confusion', '阻断包名混淆信号'], ['block_intelligence_gaps', '阻断情报未覆盖或版本未解析'], ['block_stale_intelligence', '阻断已配置但过期的本地情报']] as Array<[keyof AgentGatePolicy, string]>).map(([key, label]) => <label className="inline-check" key={key}><input type="checkbox" checked={Boolean(policy[key])} onChange={(event) => setProfile({ ...profile, quality_gate: { ...policy, [key]: event.target.checked } })} />{label}</label>)}<button className="primary-action" onClick={() => void saveProfile()}>保存门禁</button></div><div className="kv-list"><div><span>阻断 Finding</span><strong>{gate?.blocking_finding_count ?? 0}</strong></div><div><span>阻断权限</span><strong>{gate?.blocking_permission_count ?? 0}</strong></div><div><span>阻断来源资产</span><strong>{gate?.blocking_asset_count ?? 0}</strong></div><div><span>阻断情报命中</span><strong>{gate?.blocking_intelligence_count ?? 0}</strong></div></div>{gate?.reasons?.length ? <ul>{gate.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul> : <p>最近扫描没有门禁阻断原因；修改策略后需重新扫描才能重新裁决。</p>}</div>
...
function FindingModuleGovernance({ project, moduleKey, findings, validations, evidence, graph, comparison, scanHistory = [], agentSnapshot = null, agentScanDiff = null, loading, onRunReview, onRun, onUpdateFinding }: { project?: Project; moduleKey: "sast" | "agent"; findings: Finding[]; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; comparison: FindingRetestComparison | null; scanHistory?: AgentScanHistoryItem[]; agentSnapshot?: AgentScanSnapshot | null; agentScanDiff?: AgentScanDiff | null; loading: boolean; onRunReview?: () => Promise<void>; onRun: () => Promise<void>; onUpdateFinding: (findingId: string, patch: Partial<Pick<Finding, "status">>) => Promise<void> }) {
  ...
  return <ModuleGovernanceShell moduleKey={moduleKey} lastStatus={moduleKey === "agent" ? latestAgentScan?.status ?? null : findings.length ? "completed" : null} metrics={moduleKey === "agent" ? [["已识别资产", latestAgentScan?.coverage.discovered_asset_count ?? 0], ["问题总数", findings.length], ["严重 / 高危", high], ["待人工复核", pendingReview]] : [["问题总数", findings.length], ["严重 / 高危", high], ["待处理", open], ["已复核", reviewed]]} action={high ? `优先处理 ${high} 个严重或高危问题，确认影响后分配整改负责人。` : findings.length ? "逐项确认中低风险问题，记录误报或修复结论。" : latestAgentScan?.status === "completed" ? "本批次已完成，已识别资产中未命中当前规则；可查看覆盖情况确认扫描边界。" : "当前没有检测结果，请先在安全检测中执行该模块。"} loading={loading} onRun={onRun}>
    ...
    <Pagination page={pagination.page} pageCount={pagination.pageCount} total={filtered.length} onPageChange={setPage} />
    {moduleKey === "agent" ? <>{project ? <AgentGovernanceConsole project={project} snapshot={agentSnapshot} /> : null}<AgentScanCoveragePanel history={scanHistory} /><AgentAssetInventoryPanel snapshot={agentSnapshot} /><AgentProvenancePanel snapshot={agentSnapshot} /><AgentIntelligencePanel snapshot={agentSnapshot} /><AgentPermissionMatrixPanel snapshot={agentSnapshot} /><AgentSemanticDiffPanel diff={agentScanDiff} /></> : null}
    <RetestComparisonPanel comparison={comparison} />
