  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace"><header className="topbar"><div><p className="eyebrow">{viewEyebrow(activeView)}</p><h1>{viewTitle(activeView)}</h1></div><div className="topbar-actions"><div className="current-project-pill"><span>当前项目</span><strong>{project?.name ?? "未选择"}</strong></div><button className="primary-action" onClick={() => void bootstrap()} disabled={loading}>刷新数据</button></div></header>
        ...
        {activeView === "sca" && <ScaView project={project} components={components} scanHistory={scaScanHistory} selectedScanId={selectedScaScanId} scanDiff={scaScanDiff} dependencyGraph={dependencyGraph} sourcePath={sourcePath} ecosystemSummary={ecosystemSummary} riskSummary={scaRiskSummary} loading={loading} onSourcePathChange={setSourcePath} onRunScan={() => runScan("sca")} onExportSbom={exportScaSbom} onExportReport={exportScaReport} onSelectScan={selectScaScanSnapshot} />}
...
function ScaView({ project, components, scanHistory, selectedScanId, scanDiff, dependencyGraph, sourcePath, ecosystemSummary, riskSummary, loading, onSourcePathChange, onRunScan, onExportSbom, onExportReport, onSelectScan }: { project: Project | null; components: Component[]; scanHistory: ScaScanHistoryItem[]; selectedScanId: string | null; scanDiff: ScaScanDiff | null; dependencyGraph: DependencyGraph | null; sourcePath: string; ecosystemSummary: Record<string, number>; riskSummary: Record<string, number>; loading: boolean; onSourcePathChange: (value: string) => void; onRunScan: () => Promise<void>; onExportSbom: (format: "cyclonedx" | "spdx") => Promise<void>; onExportReport: () => Promise<void>; onSelectScan: (scanTaskId: string) => Promise<void> }) {
  const [page, setPage] = useState(1);
