...
function AdminCenter({ project, components, agentSnapshot, onOpenModule, onOpenKnowledge }: { project: Project | null; components: Component[]; agentSnapshot: AgentScanSnapshot | null; onOpenModule: (module: "dast" | "sandbox") => void; onOpenKnowledge: () => void }) {
  ...
  const groups: AdminModuleGroup[] = [
    { id: "sca", label: "SCA", items: [
      { id: "rules", label: "本地规则与门禁", scope: "平台默认 / 当前项目覆盖（保存时选择）", content: <><ScaPolicyPanel /><ScaPolicyOverridePanel project={project} /></> },
      ...
      { id: "data", label: "本地漏洞资源", scope: "平台公共资源", content: <><GrypeDatabasePanel /><OsvMirrorPanel /><ScaIntelligencePanel /></> },
    ] },
    ...
    { id: "sandbox", label: "SANDBOX", items: [
      { id: "runtime", label: "目标与运行配置", scope: "当前项目目标 / 单次运行", content: <section className="panel"><h2>受控目标与运行环境</h2><p>已有镜像、启动命令、容器端口和健康检查配置保留在 SANDBOX 工作台，普通用户可在既有安全限制内使用。</p><p>镜像允许范围、隔离规则与资源上限仍由现有执行代码约束，本次没有将其改为可编辑策略。</p><button className="primary-action" disabled={!project} onClick={() => onOpenModule("sandbox")}>打开 SANDBOX 工作台</button></section> },
    ] },
  ];
...
function AgentScanCoveragePanel({ history }: { history: AgentScanHistoryItem[] }) {
  ...
  return <section className="retest-panel">
    ...
    {history.length > 1 ? <details className="advanced-details"><summary>查看最近 {Math.min(history.length, 10)} 个扫描批次</summary><p className="retest-note">“问题”是正式 Finding；“原始审计记录”是扫描时保存的历史口径，旧批次可能包含后来已合并的数据流路径，当前风险页会按新口径展示。</p><table className="compact-table"><thead><tr><th>时间</th><th>状态</th><th>资产</th><th>问题</th><th>原始审计记录</th><th>规则版本</th></tr></thead><tbody>{history.slice(0, 10).map((item) => <tr key={item.scan_task_id}><td>{formatDateTime(item.finished_at ?? item.created_at)}</td><td>{scanStatusLabel(item.status)}</td><td>{item.coverage.discovered_asset_count}</td><td>{item.finding_count}</td><td>{item.audit_summary?.available ? <><strong>{item.audit_summary.review_item_count} 条原始记录</strong><span className="cell-subtext">模型：{item.audit_summary.external_model_invoked ? "已调用" : "未调用"} · {item.audit_summary.model_status ?? "未知"}</span></> : <span className="cell-subtext">旧批次无兼容草案</span>}</td><td>{item.rule_version ?? "旧批次未记录"}</td></tr>)}</tbody></table></details> : null}
  </section>;
}
...
function AgentOfflineAuditPanel({ snapshot, findings }: { snapshot: AgentScanSnapshot | null; findings: Finding[] }) {
  ...
  return <section className="retest-panel">
    ...
    {advisories.length ? <div className="agent-advisory-list">{advisories.map((item) => <article key={item.id}><div><span className={`severity ${item.priority === "critical" || item.priority === "high" ? "high" : item.priority === "medium" ? "medium" : "info"}`}>{priorityLabel[item.priority] ?? item.priority}</span><strong>{kindLabel[item.kind] ?? item.kind}</strong></div><h4>{agentUiText(item.title)}</h4><p>{agentUiText(item.rationale)}</p><details><summary>查看证据引用</summary>{item.evidence_refs.map((value) => <code key={value}>{value}</code>)}</details></article>)}</div> : <div className="agent-empty-state">当前没有独立于正式问题之外的扫描提示。</div>}
...
function FindingEvidenceDetail({ finding, validations, evidence, graph, onClose }: { finding: Finding; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; onClose: () => void }) {
  ...
  return <section className="finding-evidence-detail">
    ...
    <div className="evidence-conclusion"><strong>{conclusion}</strong><span>{nodes.length ? "以下内容来自已保存的显式关联，不代表自动确认漏洞成立。" : "当前问题还没有关联 DAST 或 SANDBOX 证据。"}</span></div>
      {finding.ai_review?.occurrences?.length ? <details className="record-evidence" open><summary>代码证据位置（{finding.ai_review.occurrences.length}）</summary><dl>{finding.ai_review.occurrences.map((item, index) => <div key={`${item.file_path}-${item.line_start}-${index}`}><dt>{item.file_path}:{item.line_start}</dt><dd>{item.evidence}</dd></div>)}</dl></details> : null}
