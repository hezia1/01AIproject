...
function SastCommunityRuleManager({ project }: { project: Project }) {
  ...
  return <div className="panel full"><div className="panel-header"><h2>Semgrep 社区安全规则</h2><span>{status?.installed ? `本地固定版 ${status.revision?.slice(0, 12)}` : "尚未下载"}</span></div><p>更新只在点击按钮时联网；扫描只读取本地固定版本，并按源码语言与配置类型选择相关规则目录，不会在扫描过程中下载。</p><div className="filter-grid"><label className="inline-check"><input type="checkbox" checked={profile?.community_rules_enabled ?? true} disabled={!profile} onChange={(event) => void toggleEnabled(event.target.checked)} />项目启用已安装的社区规则</label><label className="inline-check"><input type="checkbox" checked={licenseAccepted} onChange={(event) => setLicenseAccepted(event.target.checked)} />我已阅读并接受 <a href={status?.license_url ?? "https://semgrep.dev/legal/rules-license/"} target="_blank" rel="noreferrer">Semgrep Rules License</a></label><button className="primary-action" disabled={updating || !licenseAccepted} onClick={() => void updateRules()}>{updating ? "更新中" : status?.installed ? "检查并更新规则" : "下载社区规则"}</button><button className="secondary-action" disabled={updating} onClick={() => void load()}>刷新状态</button></div><div className="kv-list"><div><span>规则 / 文件</span><strong>{status?.rule_count ?? 0} / {status?.rule_file_count ?? 0}</strong></div><div><span>更新时间</span><strong>{status?.updated_at ? formatDateTime(status.updated_at) : "尚未更新"}</strong></div><div><span>本地目录</span><strong>{status?.root ?? "-"}</strong></div><div><span>更新方式</span><strong>仅人工触发</strong></div></div>{channels.length ? <p>已安装分类：{channels.map(([name, value]) => `${name} ${value.rules}`).join("、")}。实际扫描只加载识别到的技术栈。</p> : null}{message ? <div className="empty-project">{message}</div> : null}</div>;
}
