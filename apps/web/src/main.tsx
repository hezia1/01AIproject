...
function AgentOfflineAuditPanel({ snapshot }: { snapshot: AgentScanSnapshot | null }) {
  ...
  return <section className="retest-panel">
    ...
    <table className="compact-table"><thead><tr><th>优先级</th><th>类型</th><th>复核候选</th><th>证据引用</th><th>状态</th></tr></thead><tbody>{audit.items.length ? audit.items.map((item) => <tr key={item.id}><td><span className={`severity ${item.priority === "critical" || item.priority === "high" ? "high" : item.priority === "medium" ? "medium" : "info"}`}>{priorityLabel[item.priority] ?? item.priority}</span></td><td>{kindLabel[item.kind] ?? item.kind}</td><td><strong>{item.title}</strong><span className="cell-subtext">{item.rationale}</span></td><td>{item.evidence_refs.map((value) => <span className="cell-subtext" key={value}>{value}</span>)}</td><td>{item.review_status === "pending-human-review" ? "待人工复核" : item.review_status}</td></tr>) : <tr><td colSpan={5}>当前静态证据未生成复核候选。</td></tr>}</tbody></table>
    <details className="advanced-details"><summary>查看审计边界与哈希</summary><div className="advanced-details-body"><p>模式：{audit.mode}；模型状态：{audit.model_status}；审计 SHA-256：<code>{audit.audit_sha256}</code></p><ul>{audit.limitations.map((item) => <li key={item}>{item}</li>)}</ul></div></details>
  </section>;
}
...
function AgentOfflineAuditHistoryPanel({ diff }: { diff: AgentOfflineAuditDiff | null }) {
  ...
  return <section className="retest-panel">
    ...
    <details className="advanced-details"><summary>查看能力边界</summary><ul>{diff.limitations.map((item) => <li key={item}>{item}</li>)}</ul></details>
  </section>;
}
...
function DastBusinessFlowWorkspace({ project }: { project: Project }) {
  ...
  return <details className="advanced-details governance-advanced-details" open><summary>业务流程验证（SCA / SAST / AGENT 候选 → 策略 → 快照 → 三色裁决）</summary><div className="advanced-details-body">
    <p>此工作台独立于旧 DAST 基础观察。它支持 API、页面或混合流程定义；目前 API 执行器可在审批与精确确认后运行，页面自动化步骤会安全阻止，直到浏览器运行时获得批准并安装。</p>
    <section className="verification-strategy-card"><div><span>① 统一验证候选</span><strong>{candidates.length} 条来自 SCA / SAST / AGENT 的风险线索</strong><p>候选不会伪造接口或身份信息。缺失的目标、方法、参数和测试身份必须由人工或审批后的 AI 草案补全。</p></div><div className="filter-grid"><label>选择静态风险<select value={candidateId} onChange={(event) => setCandidateId(event.target.value)}><option value="">不关联静态风险</option>{candidates.map((item) => <option key={item.id} value={item.id}>{item.source} · {severityLabel(item.severity)} · {item.title}</option>)}</select></label>{selectedCandidate ? <div className="empty-project">类型：{selectedCandidate.vulnerability_type}；缺失：{selectedCandidate.missing.join("、")}</div> : null}</div></section>
    <section className="verification-strategy-card"><div><span>② 人工 / 模板流程草案</span><strong>角色、步骤、断言与授权范围</strong><p>角色中的 <code>credential_ref</code> 只能写 <code>env:变量名</code>，不得填写密码或 Token。页面录制产生的步骤也会在此编辑。</p></div><div className="filter-grid"><label>流程名称<input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><label>目标 URL<input value={draft.target_url} onChange={(event) => setDraft({ ...draft, target_url: event.target.value })} placeholder="https://已授权测试环境" /></label><label>流程类型<select value={draft.flow_mode} onChange={(event) => setDraft({ ...draft, flow_mode: event.target.value })}><option value="api">API</option><option value="browser">页面</option><option value="hybrid">页面 + API</option></select></label><label>授权范围<textarea value={draft.authorized_scope} onChange={(event) => setDraft({ ...draft, authorized_scope: event.target.value })} placeholder="环境、时间窗口、身份、禁止项" /></label><label>允许路径（每行一条）<textarea value={draft.allowed_paths} onChange={(event) => setDraft({ ...draft, allowed_paths: event.target.value })} /></label><label>申请人<input value={draft.requester} onChange={(event) => setDraft({ ...draft, requester: event.target.value })} /></label><label>角色 JSON<textarea rows={8} value={draft.roles} onChange={(event) => setDraft({ ...draft, roles: event.target.value })} /></label><label>步骤 JSON<textarea rows={12} value={draft.steps} onChange={(event) => setDraft({ ...draft, steps: event.target.value })} /></label><label>充分验证标准 JSON<textarea rows={5} value={draft.criteria} onChange={(event) => setDraft({ ...draft, criteria: event.target.value })} /></label><button className="primary-action" disabled={loading} onClick={() => void createFlow()}>保存流程草案</button></div></section>
    <section className="verification-strategy-card"><div><span>③ DeepSeek 辅助草案</span><strong>{aiHealth?.configured ? `已配置 · ${aiHealth.model ?? "DeepSeek"}` : "未配置新 DAST DeepSeek Key"}</strong><p>{aiHealth?.data_boundary ?? "仅发送已批准的脱敏描述。"}</p></div><div className="filter-grid"><label>业务说明<textarea value={aiDraft.business} onChange={(event) => setAiDraft({ ...aiDraft, business: event.target.value })} /></label><label>目标/API 说明<textarea value={aiDraft.target} onChange={(event) => setAiDraft({ ...aiDraft, target: event.target.value })} /></label><label>精确确认<input value={aiDraft.confirmation} onChange={(event) => setAiDraft({ ...aiDraft, confirmation: event.target.value })} placeholder={selectedCandidate ? `DAST_DEEPSEEK_DRAFT:${selectedCandidate.id}` : "先选择候选"} /></label><button className="secondary-action" disabled={loading || !aiHealth?.configured || !selectedCandidate} onClick={() => void generateAiDraft()}>生成待审批草案</button></div></section>
    <section className="verification-strategy-card"><div><span>④ 审批、运行与快照</span><strong>{flows.length} 个流程</strong><p>Dry Run 永不连接目标。API 执行需要已审批流程与精确确认；浏览器流程当前只会生成阻止快照。</p></div><div className="filter-grid"><label>选择流程<select value={flowId} onChange={(event) => { setFlowId(event.target.value); setRunId(""); void loadRuns(event.target.value); }}><option value="">请选择流程</option>{flows.map((item) => <option key={item.id} value={item.id}>{item.status} · {item.name}</option>)}</select></label>{selectedFlow ? <><label>审批依据<input value={approval.reference} onChange={(event) => setApproval({ ...approval, reference: event.target.value })} /></label><label>审批人<input value={approval.approved_by} onChange={(event) => setApproval({ ...approval, approved_by: event.target.value })} /></label><button className="secondary-action" disabled={loading || selectedFlow.status === "approved"} onClick={() => void approveFlow()}>记录审批</button><label>操作人<input value={runDraft.operator} onChange={(event) => setRunDraft({ ...runDraft, operator: event.target.value })} /></label><label>运行模式<select value={runDraft.execution_mode} onChange={(event) => setRunDraft({ ...runDraft, execution_mode: event.target.value })}><option value="dry_run">Dry Run（不连接）</option><option value="api_execution">API 业务验证（连接目标）</option></select></label><label>连接确认<input value={runDraft.confirmation} onChange={(event) => setRunDraft({ ...runDraft, confirmation: event.target.value })} placeholder={`DAST_BUSINESS_FLOW:${selectedFlow.id}:${selectedFlow.target_url}`} /></label><button className="primary-action" disabled={loading || (runDraft.execution_mode === "api_execution" && selectedFlow.status !== "approved")} onClick={() => void execute()}>{runDraft.execution_mode === "dry_run" ? "执行 Dry Run" : "执行已审批 API 流程"}</button></> : null}</div>{runs.length ? <div className="filter-grid"><label>查看运行记录<select value={runId} onChange={(event) => { setRunId(event.target.value); void loadSnapshots(event.target.value); }}><option value="">请选择运行记录</option>{runs.map((item) => <option key={item.id} value={item.id}>{item.execution_mode} · {item.status} · {item.verdict ? dastVerdictLabel(item.verdict) : "无裁决"}</option>)}</select></label>{selectedRun ? <><label>人工裁决<select value={verdict.value} onChange={(event) => setVerdict({ ...verdict, value: event.target.value })}><option value="exploitable">可利用</option><option value="not_exploitable">不可利用</option><option value="uncertain">不确定</option></select></label><label>裁决理由<textarea value={verdict.reason} onChange={(event) => setVerdict({ ...verdict, reason: event.target.value })} placeholder="必须引用实际完成步骤与证据；不可利用不等于一次请求失败。" /></label><button className="secondary-action" disabled={loading} onClick={() => void saveVerdict()}>保存三色裁决</button></> : null}</div> : null}{snapshots.length ? <div className="evidence-record-stack">{snapshots.map((item) => <details key={item.id} className="record-evidence"><summary>{item.step_id} · {item.step_kind} · {item.status} · {item.evidence_hash.slice(0, 12)}…</summary><dl><div><dt>身份</dt><dd>{item.role_alias ?? "无"}</dd></div><div><dt>请求摘要</dt><dd>{item.request_summary ?? "未发起请求"}</dd></div><div><dt>响应 / 结果</dt><dd>{item.response_summary ?? "未记录"}</dd></div><div><dt>步骤详情</dt><dd><pre className="code-preview">{JSON.stringify(item.detail, null, 2)}</pre></dd></div></dl></details>)}</div> : null}</section>
    {message ? <div className="empty-project">{message}</div> : null}
  </div></details>;
}
