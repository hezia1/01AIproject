...
function SastRuleManagement({ project, view = "engine" }: { project: Project; view?: "engine" | "rules" }) {
  const [profile, setProfile] = useState<SastProfile | null>(null);
  const [health, setHealth] = useState<SastToolHealth | null>(null);
  const [history, setHistory] = useState<SastScanHistoryItem[]>([]);
  ...
  async function saveProfile(nextProfile: SastProfile | null = profile) {
    ...
    try {
      setProfile(await request<SastProfile>(`/sast/projects/${project.id}/profile`, { method: "PATCH", body: JSON.stringify(nextProfile) }));
