...
function ScaView({ project, components, sourcePath, ecosystemSummary, riskSummary, loading, onSourcePathChange, onRunScan, onExportSbom }: { project: Project | null; components: Component[]; sourcePath: string; ecosystemSummary: Record<string, number>; riskSummary: Record<string, number>; loading: boolean; onSourcePathChange: (value: string) => void; onRunScan: () => Promise<void>; onExportSbom: () => Promise<void> }) {
  ...
  const impactChainCount = directCount > 0 ? riskyTransitiveCount : 0;
...
function FilterSelect({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return <label className="filter-control"><span>{label}</span><select value={value} onChange={(event) => onChange(event.target.value)}><option value="all">全部</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}
