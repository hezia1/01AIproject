...
function AdminCenter({ project, components, agentSnapshot, onOpenModule, onOpenKnowledge }: { project: Project | null; components: Component[]; agentSnapshot: AgentScanSnapshot | null; onOpenModule: (module: "dast" | "sandbox") => void; onOpenKnowledge: () => void }) {
  ...
  const groups: AdminModuleGroup[] = [
    ...
    { id: "dast", label: "DAST", items: [
      { id: "scope", label: "验证范围与流程", scope: "当前项目的具体策略 / 单次验证", content: <section className="panel"><h2>验证范围与执行确认</h2><p>已有策略中的目标、路径、方法、身份、流程步骤及证据标准在 DAST 工作台维护。普通用户可在现有执行器限制内确认授权并执行，不需要逐次等待管理员审批。</p><p>平台公共模板、映射规则及预算上限的可视化编辑尚未实现，本次不新增或放宽这些限制。</p><button className="primary-action" disabled={!project} onClick={() => onOpenModule("dast")}>打开 DAST 策略工作台</button></section> },
    ] },
    { id: "sandbox", label: "SANDBOX", items: [
      { id: "allowlist", label: "镜像白名单与联网", scope: "平台级；不放宽运行隔离", content: <MaintenancePolicy sandbox /> },
      { id: "runtime", label: "目标与运行配置", scope: "当前项目目标 / 单次运行", content: <section className="panel"><h2>受控目标与运行环境</h2><p>已有镜像、启动命令、容器端口和健康检查配置保留在 SANDBOX 工作台，普通用户可在既有安全限制内使用。</p><p>自动下载镜像仓库与准备阶段联网由管理员配置；运行隔离规则、命令校验与资源上限仍由执行代码约束。</p><button className="primary-action" disabled={!project} onClick={() => onOpenModule("sandbox")}>打开 SANDBOX 工作台</button></section> },
    ] },
  ];
...
function SecurityDetectionCenter({
  ...
  return <section className="security-center">
    ...
    {relationNotices.length > 0 ? <section className="relation-notices">
      <h3>模块关系提示</h3>
      {<PagedItems>{relationNotices.map((notice, index) => <div className="relation-notice" key={`${index}-${notice.text}`}><span>{notice.text}</span>{notice.modules ? <button className="secondary-action" disabled={loading} onClick={() => void onEnableRelated(notice.modules!)}>{notice.action}</button> : null}</div>)}</PagedItems>}
    </section> : null}
...
function ScaWorkspaceNavigation({ active, onChange }: { active: ScaWorkspaceTab; onChange: (value: ScaWorkspaceTab) => void }) {
  ...
  return <nav className="module-workspace-tabs" aria-label="SCA 工作区">{items.map(([key, label, description]) => <button type="button" className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined} onClick={() => onChange(key)} key={key}><strong>{label}</strong><span>{description}</span></button>)}</nav>;
}
...
function ScaOverviewCapabilities({ project, components, toolScanEnabled, loading, onToolScanChange, onExportSbom, onExportReport, onOpenTab }: { project: Project | null; components: Component[]; toolScanEnabled: boolean; loading: boolean; onToolScanChange: (enabled: boolean) => void; onExportSbom: (format: "cyclonedx" | "spdx") => Promise<void>; onExportReport: () => Promise<void>; onOpenTab: (value: ScaWorkspaceTab) => void }) {
  ...
  return <section className="module-overview-workspace">
    <div className="module-overview-cards">
      <button type="button" onClick={() => onOpenTab("risks")}><span>风险组件</span><strong>{components.filter(isRiskyScaComponent).length}</strong><small>进入漏洞与修复清单</small><ArrowRight size={17} /></button>
      <button type="button" onClick={() => onOpenTab("dependencies")}><span>依赖影响</span><strong>查看图谱</strong><small>定位直接依赖升级杠杆</small><ArrowRight size={17} /></button>
      <button type="button" onClick={() => onOpenTab("exceptions")}><span>适用性治理</span><strong>例外与 VEX</strong><small>记录本地治理结论</small><ArrowRight size={17} /></button>
  ...
  return <section className="agent-governance-console"><div className="agent-governance-summary"><div><span>当前治理配置</span><strong>v{profile.profile_version}</strong><small>规则 {profile.rule_version}</small></div><div><span>质量门禁</span><strong>{gate?.decision === "block" ? "阻断" : gate?.decision === "pass" ? "通过" : "等待扫描"}</strong><small>{agentUiText(gate?.reasons?.[0] ?? "修改后需重新扫描裁决")}</small></div><div><span>项目例外</span><strong>{profile.exceptions.length}</strong><small>{profile.exceptions.filter((item) => item.status === "pending").length} 条待审批</small></div><div><span>权限 Allowlist</span><strong>{profile.permission_allowlist.length}</strong><small>仅影响治理裁决</small></div></div>{profileDirty ? <div className="agent-unsaved-banner" role="status"><div><strong>有 {profileChanges.length} 类策略修改尚未保存</strong><span>{profileChanges.join("、")}</span></div><span>扫描规则保存后从下一次扫描生效；真实执行入口开关保存后立即生效。</span></div> : <div className="agent-saved-state"><span>当前没有未保存修改</span><small>{lastSavedAt ? `最近保存：${formatDateTime(lastSavedAt)}` : "页面中的配置与服务器已保存版本一致"}</small></div>}<nav className="agent-governance-nav" aria-label="策略与交付工作区"><button type="button" className={workspace === "policy" ? "active" : ""} onClick={() => setWorkspace("policy")}>扫描与门禁</button><button type="button" className={workspace === "exceptions" ? "active" : ""} onClick={() => setWorkspace("exceptions")}>例外与边界</button><button type="button" className={workspace === "delivery" ? "active" : ""} onClick={() => setWorkspace("delivery")}>报告与审计</button></nav><section className="content-grid agent-governance-panels">
    ...
    <div className="panel full">
      ...
      <div className="filter-grid agent-form-grid agent-gate-form">
        ...
        <div className="agent-form-actions"><button className="primary-action" disabled={!profileDirty} onClick={() => void saveProfile()}>{profileDirty ? "保存门禁与策略" : "已保存"}</button></div>
      </div>
...
function AgentRuntimePreflightPanel({ project, snapshot }: { project: Project; snapshot: AgentScanSnapshot | null }) {
  ...
  return <section className="retest-panel">
    ...
    <nav className="agent-runtime-steps" aria-label="动态验证步骤">{runtimeSteps.map(([step, label, description]) => { const disabled = step === 2 ? !exactPlanConfirmed : step === 3 ? verifiedBuildCount === 0 && !stagingResult : step === 4 ? !runtimeMethod : step === 5 ? !hasRuntimeEvidence : false; return <button type="button" key={step} className={runtimeStep === step ? "active" : runtimeStep > step ? "completed" : ""} disabled={disabled} onClick={() => setRuntimeStep(step)}><i>{runtimeStep > step ? "✓" : step}</i><span><strong>{label}</strong><small>{description}</small></span></button>; })}</nav>
    {runtimeStep === 1 ? <>
    ...
    <div className="agent-step-actions"><button type="button" className="primary-action" disabled={!exactPlanConfirmed} onClick={() => setRuntimeStep(2)}>下一步：创建安全副本</button>{!exactPlanConfirmed ? <span>完成全部阻断项并重新预检后才能继续。</span> : null}</div>
    </> : null}
    {runtimeStep === 2 ? <>
    {message ? <p className="agent-runtime-message">{message}</p> : null}
    ...
    <div className="agent-step-actions"><button type="button" className="secondary-action" onClick={() => setRuntimeStep(1)}>返回运行条件</button><button type="button" className="primary-action" disabled={verifiedBuildCount === 0 && !stagingResult} onClick={() => setRuntimeStep(3)}>下一步：选择验证方式</button></div>
    </> : null}
    {runtimeStep === 3 ? <section className="agent-runtime-methods"><div className="agent-step-intro"><strong>选择本次验证范围</strong><span>三种方式互不自动升级；能力探测不会调用工具，真实 Agent 运行仍默认关闭。</span></div><div className="agent-method-grid"><button type="button" onClick={() => { setRuntimeMethod("stdio"); setRuntimeStep(4); }}><span>低风险</span><strong>stdio MCP 能力探测</strong><small>只盘点工具、资源和 Prompt 名称</small></button><button type="button" onClick={() => { setRuntimeMethod("remote"); setRuntimeStep(4); }}><span>受限联网</span><strong>远程 MCP 能力探测</strong><small>仅连接通过公网边界检查的 HTTPS 端点</small></button><button type="button" className="danger" onClick={() => { setRuntimeMethod("target"); setRuntimeStep(4); }}><span>高风险</span><strong>真实 Agent 隔离运行</strong><small>需要项目开关、精确绑定和再次确认</small></button></div><div className="agent-step-actions"><button type="button" className="secondary-action" onClick={() => setRuntimeStep(2)}>返回安全副本</button></div></section> : null}
    ...
    <div className="agent-step-actions"><button type="button" className="secondary-action" onClick={() => setRuntimeStep(3)}>更换验证方式</button>{mcpProbeEvidence ? <button type="button" className="primary-action" onClick={() => setRuntimeStep(5)}>查看证据</button> : null}</div>
    </> : null}
    ...
    <div className="agent-step-actions"><button type="button" className="secondary-action" onClick={() => setRuntimeStep(3)}>更换验证方式</button>{remoteMcpProbeEvidence ? <button type="button" className="primary-action" onClick={() => setRuntimeStep(5)}>查看证据</button> : null}</div>
    </> : null}
...
function AgentOverviewWorkspace({ findings, history, snapshot, onOpenTab }: { findings: Finding[]; history: AgentScanHistoryItem[]; snapshot: AgentScanSnapshot | null; onOpenTab: (value: AgentWorkspaceTab) => void }) {
  ...
  return <section className="agent-overview-workspace">
    ...
    <div className="agent-overview-columns">
      <section className="agent-section-card"><div className="agent-section-heading"><div><span>优先风险</span><h3>先处理最影响当前项目的事项</h3></div><button type="button" className="text-action" onClick={() => onOpenTab("risks")}>全部 {findings.length} 条</button></div>{priorityFindings.length ? <div className="agent-priority-list">{<PagedItems>{priorityFindings.map((finding) => <button type="button" onClick={() => onOpenTab("risks")} key={finding.id}><span className={`severity ${finding.severity}`}>{severityLabel(finding.severity)}</span><div><strong>{agentFindingDisplayTitle(finding)}</strong><small>{finding.file_path ?? "项目级问题"} · {agentCategoryLabel(finding.ai_review?.category ?? "unknown")}</small></div><ArrowRight size={16} /></button>)}</PagedItems>}</div> : <div className="agent-empty-state">当前批次没有风险 Finding。请继续确认资产覆盖和未验证边界。</div>}</section>
...
function SastWorkspaceNavigation({ active, onChange }: { active: SastWorkspaceTab; onChange: (value: SastWorkspaceTab) => void }) {
  ...
  return <nav className="module-workspace-tabs" aria-label="SAST 工作区">{items.map(([key, label, description]) => <button type="button" className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined} onClick={() => onChange(key)} key={key}><strong>{label}</strong><span>{description}</span></button>)}</nav>;
}
...
function SastOverviewWorkspace({ project, findings, onOpenTab }: { project: Project; findings: Finding[]; onOpenTab: (value: SastWorkspaceTab) => void }) {
  ...
  return <section className="module-overview-workspace">
    <div className="module-overview-cards">
      <button type="button" onClick={() => onOpenTab("risks")}><span>待处理代码风险</span><strong>{open}</strong><small>{high} 条严重或高危</small><ArrowRight size={17} /></button>
      <button type="button" onClick={() => onOpenTab("ai")}><span>AI 辅助复核</span><strong>{reviewed}</strong><small>可选，不替代人工结论</small><ArrowRight size={17} /></button>
      <button type="button" onClick={() => onOpenTab("strategy")}><span>Semgrep 增强</span><strong>前端可配置</strong><small>补充本地规则覆盖</small><ArrowRight size={17} /></button>
      <button type="button" onClick={() => onOpenTab("delivery")}><span>例外与报告</span><strong>集中管理</strong><small>保留历史原始证据</small><ArrowRight size={17} /></button>
    </div>
...
function SastStrategyWorkspace({ project }: { project: Project }) {
  ...
  return <section className="module-tab-panel">
    ...
    <div className="module-segmented-tabs" role="tablist" aria-label="SAST 扫描策略分类">
      <button type="button" role="tab" aria-selected={tab === "semgrep"} className={tab === "semgrep" ? "active" : ""} onClick={() => setTab("semgrep")}>Semgrep 增强</button>
      <button type="button" role="tab" aria-selected={tab === "git"} className={tab === "git" ? "active" : ""} onClick={() => setTab("git")}>Git 增量</button>
...
function DastGovernanceView({ project, loading: parentLoading, onExportReport }: DastGovernanceProps) {
  ...
  return <ModuleGovernanceShell moduleKey="dast" lastStatus={runs.length ? "completed" : candidates.length ? "waiting" : null} metrics={[["待验证场景", candidates.length], ["可直接生成", readyCount], ["待运行上下文", blockedCount], ["已执行任务", report?.summary.business_run_count ?? 0]]} action={blockedCount ? `${blockedCount} 个场景尚待运行上下文，其中 ${identityContextCount} 个需要统一测试身份配置；这不是要求逐条补录漏洞信息。` : candidates.length ? "上游风险已自动归一化，可直接生成策略并预执行。" : "请先运行 SAST 或 AGENT，新的 Finding 会自动进入此工作台。"} loading={parentLoading || busy} hideRunButton onRun={() => execute("dry_run")}>
    <section className="dast-verdict-panel dast-verdict-overview">
      ...
      <div className="dast-verdict-results">{visibleVerdictResults.length === 0 ? <div className="workbench-empty">当前筛选下暂无已形成三色裁决的漏洞。</div> : <PagedItems>{visibleVerdictResults.map((item) => <button type="button" key={item.id} className={selectedCandidate?.id === item.id ? "active" : ""} onClick={() => void openVerdictResult(item)}><span className={`dast-result-verdict ${item.latest_verdict}`}>{dastVerdictLabel(item.latest_verdict ?? "")}</span><strong>{item.title}</strong><small>{item.recommended_strategy_name} · {formatDateTime(item.latest_verified_at)}</small><ArrowRight size={16} /></button>)}</PagedItems>}</div>
...
function FindingEvidenceDetail({ finding, validations, evidence, graph, onClose }: { finding: Finding; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; onClose: () => void }) {
  ...
  return <section className="finding-evidence-detail">
    <div className="panel-header"><div><h3>风险证据链</h3><span>{finding.source === "AGENT" ? agentFindingDisplayTitle(finding) : finding.title}</span></div><button className="secondary-action" onClick={onClose}>关闭</button></div>
