...
import { FeedbackButton, FeedbackForm, ActionFeedbackViewport } from "./action-feedback";
...
import { API_BASE, request, requestWithTimeout, feedbackFetch } from "./api";
import { AuthGate, useAuth } from "./auth";
import { UserAdministration } from "./admin";
import { AdminWorkspace, type AdminModuleGroup } from "./admin-workspace";
...
type ScaToolSecurityFinding = { kind: "misconfiguration" | "secret"; rule_id: string; severity: Severity; title: string; target: string; line?: number | null; description?: string | null; remediation?: string | null };
...
type SastCommunityRulesStatus = { installed: boolean; source: string; source_ref: string; license_url: string; update_mode: "manual_only"; root: string; revision: string | null; updated_at: string | null; rule_count: number; rule_file_count: number; inventory: Record<string, { rule_files: number; rules: number }>; archive_sha256?: string | null };
type SastToolHealth = { semgrep_cli: { available: boolean; version: string | null; path: string | null }; docker: { available: boolean; version: string | null; path: string | null }; docker_image: { available: boolean; image: string }; community_rules?: SastCommunityRulesStatus; can_run_semgrep: boolean };
...
type AiReview = { summary: string; false_positive_likelihood: string; remediation: string; category?: string | null; cwe?: string | null; owasp?: string | null; language?: string | null; description?: string | null; trust_impact?: string | null; review_status?: string | null; analysis_source?: string | null; agent_pipeline?: string[]; review_verdict?: string | null; evidence_summary?: string | null; fix_strategy?: string | null; priority?: string | null; occurrence_count?: number; occurrences?: { file_path: string; line_start: number; line_end: number; evidence: string }[]; ai_provider?: string | null; ai_confidence?: number | null; ai_review_source?: string | null; fix_draft?: { recommended_change?: string; patch?: string; tests?: string[]; limitations?: string[] } };
...
type AgentOfflineAudit = { schema: string; mode: string; generated_at: string; model_status: string; external_model_invoked: boolean; summary: { active_finding_count: number; review_item_count: number; finding_review_count?: number; advisory_review_count?: number; critical_or_high_finding_count: number; coverage_gap_count: number; private_source_preflight_gap_count: number; high_risk_static_path_count: number; local_intelligence_gap_count: number; trust_score: number; trust_grade: string }; items: AgentOfflineAuditItem[]; limitations: string[]; audit_sha256: string };
...
function Root() {
  ...
  return <><ActionFeedbackViewport /><AuthGate><App /></AuthGate></>;
}

function App() {
  const { isAdmin } = useAuth();
  const [activeView, setActiveView] = useState<ViewKey>("detection");
  const [governanceInitialScope, setGovernanceInitialScope] = useState<GovernanceScope>("overview");
    ...
    if (moduleKey === "sca" || moduleKey === "sast" || moduleKey === "agent") {
      ...
      const result = await requestWithTimeout<ScaScanResult | unknown>(`/${moduleKey}/scan`, {
        ...
        body: JSON.stringify({
          ...
          ...(moduleKey === "sast" ? { quick_mode: scanMode === "quick" } : moduleKey === "sca" ? { clear_previous: false, quick_mode: scanMode === "quick", enable_tool_scan: scaToolScanEnabled } : { clear_previous: true }),
        }),
      }, 360_000);
      return {
        status: "completed",
  ...
  async function runScan(kind: "sca" | "sast" | "agent") {
    ...
    try {
      const result = await requestWithTimeout<ScaScanResult | unknown>(`/${kind}/scan`, { method: "POST", body: JSON.stringify({ project_id: project.id, source_path: source, ...(kind === "sast" ? { quick_mode: scanMode === "quick" } : kind === "sca" ? { clear_previous: false, quick_mode: scanMode === "quick", enable_tool_scan: scaToolScanEnabled } : { clear_previous: true }) }) }, 360_000);
  ...
  async function runRecommendedScans() {
    ...
    try {
      ...
      for (const kind of runnable) {
        const result = await requestWithTimeout<ScaScanResult | unknown>(`/${kind}/scan`, { method: "POST", body: JSON.stringify({ project_id: project.id, source_path: project.source_path ?? sourcePath, ...(kind === "sast" ? { quick_mode: scanMode === "quick" } : kind === "sca" ? { clear_previous: false, quick_mode: scanMode === "quick", enable_tool_scan: scaToolScanEnabled } : { clear_previous: true }) }) }, 360_000);
  ...
  async function exportDastReport() {
    ...
    setModuleBusy("dast", true);
    try {
      const response = await feedbackFetch(`${API_BASE}/dast/projects/${project.id}/report.html`, { credentials: "include" });
  ...
  async function exportScaSbom(format: "cyclonedx" | "spdx") {
    ...
    try {
      ...
      const response = await feedbackFetch(`${API_BASE}/sca/projects/${project.id}/sbom?format=${format}${scanQuery}`, { credentials: "include" });
      if (!response.ok) {
  ...
  async function exportScaReport() {
    ...
    try {
      const scanQuery = selectedScaScanId ? `?scan_task_id=${selectedScaScanId}` : "";
      const response = await feedbackFetch(`${API_BASE}/sca/projects/${project.id}/report.html${scanQuery}`, { credentials: "include" });
      if (!response.ok) {
        let detail = `${response.status} ${response.statusText}`;
        try {
          const payload = await response.json();
  ...
  return (
    <AppShell activeView={activeView as PrimaryView} projects={projects} project={project} busy={projectControlsLoading} onNavigate={(view) => setActiveView(view)} onSelectProject={(projectId) => { const next = projects.find((item) => item.id === projectId); if (next) void selectProject(next); }} onRefresh={() => void bootstrap()}>
        ...
        {(activeView === "projects" || activeView === "assets") ? <nav className="project-section-tabs" aria-label="项目工作区"><FeedbackButton className={activeView === "projects" ? "active" : ""} onClick={() => setActiveView("projects")}>项目列表与新增</FeedbackButton><FeedbackButton className={activeView === "assets" ? "active" : ""} disabled={!project} onClick={() => setActiveView("assets")}>资产配置与准备度</FeedbackButton></nav> : null}
    ...
    {activeView === "governance" && <GovernanceCenter initialScope={governanceInitialScope} project={project} enabledModules={enabledModules} summary={summary} components={components} findings={findings} validations={validations} evidence={evidence} graph={evidenceGraph} retestComparisons={retestComparisons} scaScanHistory={scaScanHistory} agentScanHistory={agentScanHistory} agentSnapshot={agentSnapshot} agentScanDiff={agentScanDiff} agentAuditDiff={agentAuditDiff} selectedScaScanId={selectedScaScanId} scaScanDiff={scaScanDiff} dependencyGraph={dependencyGraph} scaToolScanEnabled={scaToolScanEnabled} sandboxTemplates={sandboxTemplates} dastStrategies={dastStrategies} dastStrategyId={dastStrategyId} dastTargetConfirmation={dastTargetConfirmation} loading={loading} unifiedLoading={unifiedLoading} moduleLoading={moduleLoading} targetUrl={targetUrl} runCommand={runCommand} sandboxImage={sandboxImage} selectedFindingId={correlationFindingId} selectedValidationId={correlationValidationId} onTargetUrlChange={setTargetUrl} onRunCommandChange={setRunCommand} onSandboxImageChange={setSandboxImage} onDastStrategyChange={setDastStrategyId} onDastTargetConfirmationChange={setDastTargetConfirmation} onScaToolScanChange={setScaToolScanEnabled} onSelectScaScan={selectScaScanSnapshot} onExportScaSbom={exportScaSbom} onExportScaReport={exportScaReport} onRunSastAgentReview={runSastAgentReview} onRunAgentAiReview={runAgentDeepseekReview} onSelectDastRisk={selectDastRisk} onSelectSandboxRisk={selectSandboxRisk} onSelectSandboxValidation={selectSandboxValidation} onRunDast={createDastValidation} onCreateManualDast={createManualDastValidation} onUpdateManualDast={updateManualDastValidation} onExportDastReport={exportDastReport} onRunSandbox={createSandboxEvidence} onRunModule={runSingleModuleCheck} onUpdateFinding={updateFindingGovernance} />}
        ...
        {activeView === "admin" && isAdmin ? <AdminCenter project={project} components={components} agentSnapshot={agentSnapshot} onOpenModule={(module) => { setGovernanceInitialScope(module); setActiveView("governance"); }} onOpenKnowledge={() => setActiveView("knowledge")} /> : null}
    </AppShell>
  );
}
...
function ReportCenter({ project, onExportSca, onExportSbom, onExportDast }: { project: Project | null; onExportSca: () => Promise<void>; onExportSbom: (format: "cyclonedx" | "spdx") => Promise<void>; onExportDast: () => Promise<void> }) {
  ...
  return <section className="content-grid report-center">
    ...
    <article className="panel"><div className="panel-header"><div><h2>SCA 供应链报告</h2><span>组件、漏洞、许可证与依赖证据</span></div></div><div className="report-actions"><FeedbackButton className="primary-action" onClick={() => onExportSca()}>导出 HTML 报告</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => onExportSbom("cyclonedx")}>CycloneDX SBOM</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => onExportSbom("spdx")}>SPDX SBOM</FeedbackButton></div></article>
    <article className="panel"><div className="panel-header"><div><h2>DAST 动态验证报告</h2><span>三色裁决、未验证状态与运行证据</span></div></div><div className="report-actions"><FeedbackButton className="primary-action" onClick={() => onExportDast()}>导出 DAST 报告</FeedbackButton></div></article>
...
function AdminCenter({ project, components, agentSnapshot, onOpenModule, onOpenKnowledge }: { project: Project | null; components: Component[]; agentSnapshot: AgentScanSnapshot | null; onOpenModule: (module: "dast" | "sandbox") => void; onOpenKnowledge: () => void }) {
  const noProject = <div className="panel empty-project">请先选择项目。</div>;
  const groups: AdminModuleGroup[] = [
    { id: "sca", label: "SCA", items: [
      { id: "rules", label: "本地规则与门禁", scope: "平台默认 / 当前项目覆盖（保存时选择）", content: <ScaPolicyOverridePanel project={project} /> },
      { id: "exceptions", label: "风险例外", scope: "当前项目", content: <ScaExceptionPanel project={project} components={components} /> },
      { id: "vex", label: "VEX 适用性", scope: "当前项目", content: <ScaVexPanel project={project} components={components} /> },
      { id: "data", label: "本地漏洞资源", scope: "平台公共资源", content: <><MaintenancePolicy /><GrypeDatabasePanel /><OsvMirrorPanel /><ScaIntelligencePanel /></> },
    ] },
    { id: "sast", label: "SAST", items: [
      { id: "rules", label: "本地规则", scope: "当前项目", content: project ? <SastRuleManagement project={project} view="rules" /> : noProject },
      { id: "downloads", label: "联网下载策略", scope: "平台公共资源", content: <MaintenancePolicy /> },
      { id: "semgrep", label: "Semgrep 规则", scope: "项目 YAML / 平台公共社区规则", content: project ? <SastExpertDelivery project={project} view="semgrep" /> : noProject },
      { id: "profile", label: "扫描默认配置", scope: "当前项目", content: project ? <><SastRuleManagement project={project} view="config" /><SastExpertDelivery project={project} view="git" /></> : noProject },
      { id: "ai", label: "AI 复核配置", scope: "当前项目；API Key 仍在后端环境配置中", content: project ? <SastAiAgentsPanel project={project} onRunReview={async () => { await request(`/sast/projects/${project.id}/agent-review`, { method: "POST" }); }} /> : noProject },
      { id: "suppressions", label: "规则与路径抑制", scope: "当前项目；影响后续扫描，不删除历史证据", content: project ? <SastEvidenceGovernance project={project} /> : noProject },
      { id: "gate", label: "质量门禁", scope: "当前项目", content: project ? <SastOperationsConsole project={project} view="gate" /> : noProject },
    ] },
    { id: "agent", label: "AGENT", items: [
      { id: "policy", label: "规则、白名单与门禁", scope: "当前项目", content: project ? <AgentGovernanceConsole project={project} snapshot={agentSnapshot} /> : noProject },
    ] },
    { id: "dast", label: "DAST", items: [
      { id: "scope", label: "验证范围与流程", scope: "当前项目的具体策略 / 单次验证", content: <section className="panel"><h2>验证范围与执行确认</h2><p>已有策略中的目标、路径、方法、身份、流程步骤及证据标准在 DAST 工作台维护。普通用户可在现有执行器限制内确认授权并执行，不需要逐次等待管理员审批。</p><p>平台公共模板、映射规则及预算上限的可视化编辑尚未实现，本次不新增或放宽这些限制。</p><FeedbackButton className="primary-action" disabled={!project} onClick={() => onOpenModule("dast")}>打开 DAST 策略工作台</FeedbackButton></section> },
    ] },
    { id: "sandbox", label: "SANDBOX", items: [
      { id: "allowlist", label: "镜像白名单与联网", scope: "平台级；不放宽运行隔离", content: <MaintenancePolicy sandbox /> },
      { id: "runtime", label: "目标与运行配置", scope: "当前项目目标 / 单次运行", content: <section className="panel"><h2>受控目标与运行环境</h2><p>已有镜像、启动命令、容器端口和健康检查配置保留在 SANDBOX 工作台，普通用户可在既有安全限制内使用。</p><p>自动下载镜像仓库与准备阶段联网由管理员配置；运行隔离规则、命令校验与资源上限仍由执行代码约束。</p><FeedbackButton className="primary-action" disabled={!project} onClick={() => onOpenModule("sandbox")}>打开 SANDBOX 工作台</FeedbackButton></section> },
    ] },
  ];
  return <AdminWorkspace key={project?.id ?? "no-project"} projectName={project?.name} users={<UserAdministration />} groups={groups} onOpenKnowledge={onOpenKnowledge} />;
}
  ...
  return <section className={`project-workspace onboarding-workspace ${isAdmin ? "" : "user-project-workspace"}`}>
    <div className="panel project-create onboarding-create">
      ...
      <div className="import-mode-switch" role="tablist" aria-label="项目接入方式">
        {(["local", "git", "zip"] as ProjectImportMode[]).map((mode) => <FeedbackButton type="button" role="tab" aria-selected={importMode === mode} className={importMode === mode ? "active" : ""} onClick={() => onImportModeChange(mode)} key={mode}>{mode === "local" ? "本地文件夹" : mode === "git" ? "Git 仓库" : "ZIP 上传"}</FeedbackButton>)}
      </div>
      <FeedbackForm className="project-form" onSubmit={(event) => onCreate(event)}>
        ...
        <FeedbackButton className="primary-action wide-field" disabled={loading || !draft.name.trim()}><Plus size={16} />{loading ? "正在安全接入" : "接入并检查准备度"}</FeedbackButton>
      </FeedbackForm>
    </div>
    <div className="panel project-directory"><div className="panel-header"><h2>项目列表</h2><span>{projects.length} 个项目</span></div><div className="project-list">{projects.length === 0 ? <div className="empty-project">暂无项目。接入后，检测与治理数据会按项目隔离。</div> : <PagedItems>{projects.map((item) => <div className={`project-row ${project?.id === item.id ? "active" : ""}`} key={item.id}><FeedbackButton className="project-main" onClick={() => onSelect(item)} disabled={loading}><div><strong>{item.name}</strong><span>{item.repository_url ?? "本地 / ZIP 来源"} · {item.default_branch}</span><span>{item.source_path ?? "未配置本地源码路径"}</span></div><span>{item.business_owner ?? "未配置业务负责人"}</span><span>{item.security_owner ?? "未配置安全负责人"}</span></FeedbackButton><FeedbackButton className="danger-action" disabled={loading} onClick={() => onDelete(item.id)}>删除</FeedbackButton></div>)}</PagedItems>}</div></div>
...
function ProjectReadinessPanel({ project, readiness, onOpenDetection }: { project: Project | null; readiness: ProjectReadiness | null; onOpenDetection: () => void }) {
  ...
  return <section className={`panel readiness-panel ${readiness.overall_status}`}>
    <div className="readiness-heading"><div><span className={`readiness-badge ${readiness.overall_status}`}>{readinessStatusLabel(readiness.overall_status)}</span><h2>{project.name} 接入准备度</h2><p>{readiness.overall_status === "blocked" ? "当前存在静态扫描阻塞项，请先按建议补齐源码输入。" : `可执行推荐：${readiness.recommended_tasks.map((item) => item.toUpperCase()).join(" + ") || "暂无"}`}</p></div><FeedbackButton className="primary-action" disabled={!readiness.quick_scan.available} onClick={onOpenDetection}>进入快速检测</FeedbackButton></div>
    <div className="readiness-check-grid">{<PagedItems>{readiness.checks.map((check) => <article className={`readiness-check ${check.status}`} key={check.key}><div><i>{check.status === "ready" ? <Check size={15} /> : check.status === "blocked" ? "!" : "·"}</i><strong>{check.title}</strong><span>{readinessCheckLabel(check.status)}</span></div><p>{check.detail}</p>{check.remediation ? <small>{check.remediation}</small> : null}</article>)}</PagedItems>}</div>
...
function ProjectAssets({ project, assetProbe, enabledModules, components, findings, validations, evidence, summary, onOpenTasks, onOpenModules }: { project: Project | null; assetProbe: ProjectAssetProbe | null; enabledModules: Set<ModuleKey>; components: Component[]; findings: Finding[]; validations: DastValidation[]; evidence: SandboxEvidence[]; summary: AspmSummary | null; onOpenTasks: () => void; onOpenModules: () => void }) {
  ...
  return <section className="asset-workspace"><section className="module-summary"><Metric label="资产路径" value={pathStatus} /><Metric label="依赖清单" value={assetProbe?.sca_files.length ?? 0} /><Metric label="源码文件" value={assetProbe?.source_files.length ?? 0} /><Metric label="推荐任务" value={recommended.length} /></section><div className="asset-grid"><div className="panel asset-hero full"><div><div className="panel-header"><h2>{project?.name ?? "未选择项目"}</h2><span>{project?.default_branch ?? "main"}</span></div><div className="asset-path">{sourcePath}</div><div className="asset-tags"><span>{project?.repository_url ?? "未配置仓库"}</span><span>业务：{project?.business_owner ?? "未配置"}</span><span>安全：{project?.security_owner ?? "未配置"}</span></div></div><div className="asset-actions"><FeedbackButton className="primary-action" onClick={onOpenTasks} disabled={!project || runnable.length === 0}>执行推荐任务</FeedbackButton><FeedbackButton className="secondary-action" onClick={onOpenModules}>配置模块</FeedbackButton></div></div><div className="panel"><div className="panel-header"><h2>识别结果</h2><span>{assetProbe?.message ?? "暂无探测结果"}</span></div><div className="output-strip"><div><span>SCA</span><strong>{assetProbe?.sca_files.length ?? 0}</strong></div><div><span>SAST</span><strong>{assetProbe?.source_files.length ?? 0}</strong></div><div><span>AGENT</span><strong>{assetProbe?.agent_files.length ?? 0}</strong></div></div><div className="asset-note">{runnable.length ? `当前可直接执行：${runnable.map((item) => item.toUpperCase()).join(" + ")}` : "暂无可直接执行的推荐任务"}</div>{blocked.length ? <div className="asset-warning">需先启用模块：{blocked.map((item) => item.toUpperCase()).join(" + ")}</div> : null}</div><div className="panel"><div className="panel-header"><h2>模块准备度</h2><span>{enabledNames.length} 个已启用</span></div><div className="readiness-list">{<PagedItems>{OPTIONAL_MODULES.map((moduleKey) => <div className="readiness-row" key={moduleKey}><span>{moduleKey.toUpperCase()}</span><strong className={enabledModules.has(moduleKey) ? "ready" : "muted"}>{enabledModules.has(moduleKey) ? "已接入" : "未接入"}</strong></div>)}</PagedItems>}</div></div><div className="panel full"><div className="panel-header"><h2>资产文件</h2><span>来自本地源码路径自动识别</span></div><div className="asset-file-grid"><AssetFileList title="依赖清单" files={assetProbe?.sca_files ?? []} /><AssetFileList title="源码文件" files={assetProbe?.source_files ?? []} /><AssetFileList title="Agent 配置" files={assetProbe?.agent_files ?? []} /></div></div><div className="panel full"><div className="panel-header"><h2>当前项目结果</h2><span>进入 ASPM 治理总览前的资产侧摘要</span></div><div className="output-strip wide"><div><span>组件</span><strong>{summary?.component_count ?? components.length}</strong></div><div><span>Findings</span><strong>{summary?.finding_count ?? findings.length}</strong></div><div><span>DAST 验证</span><strong>{summary?.dast_validation_count ?? validations.length}</strong></div><div><span>Sandbox 证据</span><strong>{summary?.sandbox_evidence_count ?? evidence.length}</strong></div></div></div></div></section>;
}

function AssetFileList({ title, files }: { title: string; files: string[] }) {
  return <div className="asset-file-list"><h3>{title}</h3>{files.length === 0 ? <span className="empty-inline">暂无文件</span> : <ul>{<PagedItems>{files.map((file) => <li key={file}>{file}</li>)}</PagedItems>}</ul>}</div>;
}
...
function SecurityDetectionCenter({
  ...
  return <section className="security-center">
    ...
    <div className="detection-module-grid">
      {modules.map((module) => {
        ...
        return <article className={`detection-module-card ${enabled ? "enabled" : ""}`} key={module.key}>
          ...
          <FeedbackButton className={`module-select-button ${enabled ? "selected" : ""}`} disabled={!project || savingKey === module.key || loading || moduleLoading[module.key as ExecutableModuleKey]} onClick={() => onToggle(module)}>{enabled ? "已接入" : "接入模块"}</FeedbackButton>
        </article>;
      })}
    </div>
    ...
    {relationNotices.length > 0 ? <section className="relation-notices">
      <h3>模块关系提示</h3>
      {<PagedItems>{relationNotices.map((notice, index) => <div className="relation-notice" key={`${index}-${notice.text}`}><span>{notice.text}</span>{notice.modules ? <FeedbackButton className="secondary-action" disabled={loading} onClick={() => onEnableRelated(notice.modules!)}>{notice.action}</FeedbackButton> : null}</div>)}</PagedItems>}
    </section> : null}
    ...
    <section className="panel detection-run-panel">
      ...
      <div className="scan-mode-switch" role="radiogroup" aria-label="扫描模式"><FeedbackButton type="button" className={scanMode === "quick" ? "active" : ""} onClick={() => onScanModeChange("quick")}><strong>快速演示</strong><span>默认 · 基础范围有界</span></FeedbackButton><FeedbackButton type="button" className={scanMode === "deep" ? "active" : ""} onClick={() => onScanModeChange("deep")}><strong>深度扫描</strong><span>完整范围 · 耗时不定</span></FeedbackButton></div>
      <FeedbackButton className="primary-action run-all-button" disabled={!project || loading || runBlocked || selected.length === 0} onClick={() => onRun()}>{loading ? "检测执行中" : runBlocked ? "单模块执行中" : "一键执行"}</FeedbackButton>
      {executionSteps.length > 0 ? <div className="execution-progress">{<PagedItems>{executionSteps.map((step) => <div className={`execution-step ${step.status}`} key={step.module}><span>{MODULE_DISPLAY[step.module].name}</span><strong>{executionStatusLabel(step.status)}</strong><small>{step.detail}</small></div>)}</PagedItems>}</div> : null}
    </section>
  </section>;
}
...
function GovernanceCenter({
  ...
  const [scope, setScope] = useState<GovernanceScope>(initialScope);
  ...
  return <section className="governance-center">
    <nav className="governance-scope" aria-label="治理查看范围">
      {scopes.map((item) => <FeedbackButton className={scope === item ? "active" : ""} key={item} onClick={() => setScope(item)}>{item === "overview" ? "综合总览" : MODULE_DISPLAY[item].name}</FeedbackButton>)}
    </nav>
...
function SastGovernanceWorkspace({ project, findings, validations, evidence, graph, comparison, loading, onRunReview, onRun, onUpdateFinding }: { project: Project; findings: Finding[]; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; comparison: FindingRetestComparison | null; loading: boolean; onRunReview: () => Promise<void>; onRun: () => Promise<void>; onUpdateFinding: (findingId: string, patch: Partial<Pick<Finding, "status">>) => Promise<void> }) {
  return <section className="sast-governance-workspace">
    <FindingModuleGovernance project={project} moduleKey="sast" findings={findings} validations={validations} evidence={evidence} graph={graph} comparison={comparison} loading={loading} onRunReview={onRunReview} onRun={onRun} onUpdateFinding={onUpdateFinding} />
  </section>;
}
...
function SastExpertDelivery({ project, view = "git" }: { project: Project; view?: "git" | "semgrep" }) {
  ...
  async function load() {
    ...
    const [nextProfile, packResult, nextReport] = await Promise.all([
      ...
      request<{ semgrep_rules: SastSemgrepRule[] }>(`/sast/projects/${project.id}/semgrep-rules`).catch(error => { setLoadError(`项目 YAML 规则加载失败：${errorMessage(error)}`); return { semgrep_rules: [] }; }),
  ...
  async function savePack() {
    try {
      const saved = await request<SastProfile>(`/sast/projects/${project.id}/semgrep-rules${editingPack ? `/${editingPack}` : ""}`, { method: editingPack ? "PATCH" : "POST", body: JSON.stringify(draft) });
      setProfile(saved); setPacks(saved.semgrep_rules ?? []); setEditingPack(null); setMessage(draft.status === "draft" ? "规则包已保存为草稿；发布后才会参与扫描。" : "规则包已发布，将从下一次 Semgrep 扫描开始生效。");
    } catch (error) { setMessage(`保存失败：${errorMessage(error)}`); }
  }
  ...
  return <section className={`content-grid sast-expert-delivery ${view}`}>
    {loadError && <div role="alert" className="report-error">{loadError}</div>}
    {view === "git" ? <><div className="panel full"><div className="panel-header"><h2>Git 对比与历史密钥检查</h2><span>留空基线时仍会执行完整扫描</span></div><fieldset disabled={!isAdmin} className="filter-grid policy-readonly"><label>对比的 Git 版本<input value={profile?.git_baseline_ref ?? ""} placeholder="例如 origin/main 或 HEAD~1" onChange={(event) => setProfile((value) => value ? { ...value, git_baseline_ref: event.target.value } : value)} /></label><label className="inline-check"><input type="checkbox" checked={profile?.scan_git_history_secrets ?? true} onChange={(event) => setProfile((value) => value ? { ...value, scan_git_history_secrets: event.target.checked } : value)} />检查历史提交中的疑似密钥路径</label><label className="inline-check"><input type="checkbox" checked={profile?.changed_files_only ?? false} disabled={!profile?.git_baseline_ref} onChange={(event) => setProfile((value) => value ? { ...value, changed_files_only: event.target.checked } : value)} />只扫描相对基线发生变化的文件</label><FeedbackButton className="secondary-action" disabled={!profile} onClick={() => saveGitProfile()}>保存 Git 配置</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => load()}>刷新统计</FeedbackButton></fieldset>{message ? <div className="empty-project">{message}</div> : null}</div>
    ...
    <div className="panel full"><div className="panel-header"><h2>后续验证建议</h2><span>不会自动发送请求或执行沙箱命令</span></div>{report?.validation_suggestions.length ? <ul>{<PagedItems>{report.validation_suggestions.map((item) => <li key={item.finding_id}><strong>{item.recommended_module}</strong> · {item.next_step}</li>)}</PagedItems>}</ul> : <div className="empty-project">当前没有需要跨模块验证的高风险线索。</div>}</div></> : null}
    {view === "semgrep" ? <><SastCommunityRuleManager project={project} /><div className="panel full"><div className="panel-header"><h2>项目 Semgrep YAML 规则包</h2><span>只有“已发布 + 已启用”的规则包才参与扫描</span></div><details className="advanced-details" open={editingPack !== null || undefined}><summary>{editingPack ? "编辑项目规则包" : "新建专家规则包"}</summary><p>普通项目无需创建。只有熟悉 Semgrep YAML 时才使用这里。</p><div className="filter-grid"><label>规则包名称<input value={draft.name} onChange={(event) => setDraft((value) => ({ ...value, name: event.target.value }))} /></label><label>保存状态<select value={draft.status} onChange={(event) => setDraft((value) => ({ ...value, status: event.target.value as "draft" | "published" }))}><option value="draft">仅保存草稿</option><option value="published">保存并发布</option></select></label><label className="wide-field">规则 YAML<textarea value={draft.content} onChange={(event) => setDraft((value) => ({ ...value, content: event.target.value }))} rows={9} /></label><FeedbackButton className="secondary-action" onClick={() => validatePack()}>校验规则</FeedbackButton><FeedbackButton className="primary-action" onClick={() => savePack()}>{draft.status === "draft" ? "保存草稿" : "保存并发布"}</FeedbackButton></div></details>{packs.length ? <PagedTable><thead><tr><th>规则包</th><th>规则 ID</th><th>生命周期</th><th>操作</th></tr></thead><tbody>{packs.map((pack) => <tr key={pack.id}><td>{pack.name}<span className="cell-subtext">v{pack.version} · {pack.approved_by ? `审批：${pack.approved_by}` : "未记录审批人"}</span></td><td>{pack.rule_ids.join(", ")}</td><td>{semgrepRuleStatusLabel(pack.status)}<span className="cell-subtext">{pack.enabled ? "已启用" : "已停用"}</span></td><td><FeedbackButton className="secondary-action" onClick={() => { setEditingPack(pack.id); setDraft({ name: pack.name, content: pack.content, status: "draft" }); setMessage(`正在编辑 ${pack.name}；修改保存为草稿后需重新发布。`); }}>编辑</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => preflight(pack)}>真实引擎预检</FeedbackButton>{pack.status !== "published" ? <FeedbackButton className="secondary-action" onClick={() => publish(pack)}>发布</FeedbackButton> : null}<FeedbackButton className="secondary-action" onClick={() => toggle(pack)}>{pack.enabled ? "停用" : "启用"}</FeedbackButton></td></tr>)}</tbody></PagedTable> : <div className="empty-project">没有项目自定义 YAML 规则包；默认离线规则和已启用的社区规则仍会参与扫描。</div>}</div></> : null}
  </section>;
}

function SastCommunityRuleManager({ project }: { project: Project }) {
  const download = useDownloadPermission("semgrep_download_allowed");
  const [status, setStatus] = useState<SastCommunityRulesStatus | null>(null);
  const [profile, setProfile] = useState<SastProfile | null>(null);
  ...
  async function load() {
    const [nextStatus, nextProfile] = await Promise.all([
      request<SastCommunityRulesStatus>("/sast/community-rules").catch(() => null),
      request<SastProfile>(`/sast/projects/${project.id}/profile`).catch(() => null),
    ]);
  ...
  async function updateRules() {
    if (!licenseAccepted) { setMessage("请先确认 Semgrep Rules License 的使用限制。"); return; }
    setUpdating(true); setMessage("正在从官方仓库解析版本并下载安全规则……");
    try {
      const next = await request<SastCommunityRulesStatus>("/sast/community-rules/update", { method: "POST", body: JSON.stringify({ source_ref: status?.source_ref ?? "develop", license_accepted: true }) });
      setStatus(next); setMessage(`更新完成：固定版本 ${next.revision?.slice(0, 12)}，共 ${next.rule_count} 条安全规则。`);
    } catch (error) { setMessage(`更新失败：${errorMessage(error)}`); }
  ...
  async function toggleEnabled(enabled: boolean) {
    if (!profile) return;
    try {
      const next = await request<SastProfile>(`/sast/projects/${project.id}/profile`, { method: "PATCH", body: JSON.stringify({ community_rules_enabled: enabled }) });
      setProfile(next); setMessage(enabled ? "已启用本地社区规则；下次扫描将按项目技术栈选择。" : "已停用社区规则；内置和项目规则不受影响。");
    } catch (error) { setMessage(`保存失败：${errorMessage(error)}`); }
  }

  const channels = Object.entries(status?.inventory ?? {}).sort(([left], [right]) => left.localeCompare(right));
  return <div className="panel full sast-community-rules-panel"><div className="panel-header"><h2>Semgrep 社区安全规则</h2><p>{download.detail}</p><span>{status?.installed ? `本地固定版 ${status.revision?.slice(0, 12)}` : "尚未下载"}</span></div><p>更新只在点击按钮时联网；扫描只读取本地固定版本，并按源码语言与配置类型选择相关规则目录，不会在扫描过程中下载。</p><div className="filter-grid sast-community-actions"><label className="inline-check"><input type="checkbox" checked={profile?.community_rules_enabled ?? true} disabled={!profile} onChange={(event) => void toggleEnabled(event.target.checked)} /><span><strong>启用社区规则</strong><small>按当前项目技术栈自动选择</small></span></label><label className="inline-check community-license-consent"><input type="checkbox" checked={licenseAccepted} onChange={(event) => setLicenseAccepted(event.target.checked)} /><span>我已阅读并接受 <a href={status?.license_url ?? "https://semgrep.dev/legal/rules-license/"} target="_blank" rel="noreferrer">Semgrep Rules License</a></span></label><FeedbackButton className="primary-action" disabled={updating || !licenseAccepted || !download.allowed} onClick={() => updateRules()}>{updating ? "更新中" : status?.installed ? "检查并更新规则" : "下载社区规则"}</FeedbackButton><FeedbackButton className="secondary-action" disabled={updating} onClick={() => load()}>刷新状态</FeedbackButton></div><div className="kv-list"><div><span>规则 / 文件</span><strong>{status?.rule_count ?? 0} / {status?.rule_file_count ?? 0}</strong></div><div><span>更新时间</span><strong>{status?.updated_at ? formatDateTime(status.updated_at) : "尚未更新"}</strong></div><div><span>本地目录</span><strong>{status?.root ?? "-"}</strong></div><div><span>更新方式</span><strong>仅人工触发</strong></div></div>{channels.length ? <p>已安装分类：{channels.map(([name, value]) => `${name} ${value.rules}`).join("、")}。实际扫描只加载识别到的技术栈。</p> : null}{message ? <div className="empty-project">{message}</div> : null}</div>;
}
...
function SastAiAgentsPanel({ project, onRunReview }: { project: Project; onRunReview: () => Promise<void> }) {
  ...
  async function save() {
    ...
    try {
      ...
      setProfile(saved); setMessage("AI 辅助复核配置已保存。下一次 SAST 扫描将按该配置执行。");
    } catch (error) { setMessage(`保存失败：${errorMessage(error)}`); }
  }
  ...
  return <section className="content-grid sast-ai-review-workspace">
    <div className="panel full"><div className="panel-header"><h2>连接与安全边界</h2><span>{health?.configured ? "API Key 已在后端安全加载" : "尚未配置 API Key"}</span></div><div className="kv-list"><div><span>服务</span><strong>{health?.base_url ?? "https://api.deepseek.com"}</strong></div><div><span>发现 / 分析模型</span><strong>{health?.model ?? "-"}</strong></div><div><span>独立复核模型</span><strong>{health?.review_model ?? "-"}</strong></div><div><span>结构化思考模式</span><strong>{health?.thinking_enabled ? "已开启（更慢、费用更高）" : "已关闭（默认，更稳定）"}</strong></div><div><span>Key 文件</span><strong>{health?.api_key_location ?? "apps/api/.env"}</strong></div></div><p>代码上传前会脱敏密钥并移除疑似提示注入；Agent 不能执行代码、调用工具、访问扫描目标或直接修改源码。连接测试会产生一次极小的真实 API 调用。</p><div className="filter-grid"><FeedbackButton className="secondary-action" disabled={!health?.configured || testing} onClick={() => testConnection()}>{testing ? "测试中" : "测试 DeepSeek 连接"}</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => load()}>刷新状态</FeedbackButton></div>{message ? <div className="empty-project">{message}</div> : null}</div>
    <div className="panel full"><div className="panel-header"><h2>项目级 Agent 策略</h2><span>默认关闭，防止无意消耗额度</span></div><div className="filter-grid"><label className="inline-check"><input type="checkbox" checked={profile?.ai_enabled ?? false} disabled={!profile || !isAdmin || !health?.configured} onChange={(event) => setProfile((value) => value ? { ...value, ai_enabled: event.target.checked } : value)} />启用 DeepSeek 多 Agent</label><label className="inline-check"><input type="checkbox" checked={profile?.ai_auto_scan ?? true} disabled={!profile || !isAdmin} onChange={(event) => setProfile((value) => value ? { ...value, ai_auto_scan: event.target.checked } : value)} />随 SAST 自动执行</label><label>最大上传字符数<input type="number" min={10000} max={200000} step={10000} value={profile?.ai_max_input_chars ?? 60000} disabled={!profile || !isAdmin} onChange={(event) => setProfile((value) => value ? { ...value, ai_max_input_chars: Number(event.target.value) } : value)} /></label><label>正式 Finding 置信度<input type="number" min={50} max={100} value={profile?.ai_confidence_threshold ?? 80} disabled={!profile || !isAdmin} onChange={(event) => setProfile((value) => value ? { ...value, ai_confidence_threshold: Number(event.target.value) } : value)} /></label><label className="inline-check"><input type="checkbox" checked={profile?.ai_include_fix_drafts ?? true} disabled={!profile || !isAdmin} onChange={(event) => setProfile((value) => value ? { ...value, ai_include_fix_drafts: event.target.checked } : value)} />保存人工评审用补丁草案</label><FeedbackButton className="secondary-action" disabled={!profile || !isAdmin} onClick={() => save()}>保存 Agent 配置</FeedbackButton><FeedbackButton className="primary-action" disabled={!profile?.ai_enabled || !health?.configured || running} onClick={() => runAgents()}>{running ? "七角色审计中" : "立即执行 AI 深度审计"}</FeedbackButton></div></div>
    <div className="panel full"><div className="panel-header"><h2>最近一次多 Agent 审计</h2><span>{latest ? `${formatDateTime(latest.finished_at ?? latest.started_at)} · ${latest.status}` : "尚未执行"}</span></div>{latest ? <><section className="module-summary"><Metric label="角色完成" value={`${latest.result_summary.completed_agent_count ?? latest.agent_steps.filter((step) => step.status === "completed").length} / ${latest.result_summary.expected_agent_count ?? 7}`} /><Metric label="候选漏洞" value={latest.result_summary.candidate_count ?? 0} /><Metric label="确认 Finding" value={latest.result_summary.confirmed_count ?? 0} /><Metric label="Token" value={(latest.token_usage.prompt_tokens ?? 0) + (latest.token_usage.completion_tokens ?? 0)} /></section><PagedTable><thead><tr><th>Agent</th><th>状态</th><th>模型</th><th>耗时</th><th>Token / 估算费用</th></tr></thead><tbody>{latest.agent_steps.map((step) => <tr key={`${latest.id}-${step.role}`}><td>{sastAgentRoleLabel(step.role)}</td><td>{step.status === "completed" ? "完成" : `失败：${step.error ?? "未知原因"}`}</td><td>{step.model ?? "-"}</td><td>{step.latency_ms != null ? `${step.latency_ms} ms` : "-"}</td><td>{(step.prompt_tokens ?? 0) + (step.completion_tokens ?? 0)}<span className="cell-subtext">{step.estimated_cost_usd == null ? "费用未计算" : `$${step.estimated_cost_usd.toFixed(6)}`}</span></td></tr>)}</tbody></PagedTable><div className="kv-list"><div><span>上传范围</span><strong>{latest.result_summary.context_summary?.uploaded_file_count ?? 0} 文件 / {latest.result_summary.context_summary?.uploaded_char_count ?? 0} 字符</strong></div><div><span>脱敏项</span><strong>{latest.result_summary.context_summary?.redaction_count ?? 0}</strong></div><div><span>未完成角色</span><strong>{latest.result_summary.incomplete_roles?.map(sastAgentRoleLabel).join("、") || "无"}</strong></div><div><span>Agent 分歧</span><strong>{latest.result_summary.disagreement_count ?? 0}</strong></div><div><span>执行错误</span><strong>{latest.error ?? "无"}</strong></div></div></> : <div className="empty-project">启用后执行 SAST 或点击“立即执行 AI 深度审计”，这里会显示七个 Agent 的真实调用状态、Token 和裁决统计。</div>}</div>
  </section>;
}

function SastOperationsConsole({ project, view = "all" }: { project: Project; view?: "all" | "gate" }) {
  const { isAdmin } = useAuth();
  ...
  return <section className={`content-grid sast-ci-worker-workspace ${view}`}><section className="module-context-note full"><strong>部署选项</strong><span>后台任务只在服务器部署 Worker 时使用；CI 门禁用于流水线发布判断。普通本地扫描无需创建后台任务。</span></section><div className="panel full sast-jobs-panel"><div className="panel-header"><h2>后台扫描任务</h2><span>仅在需要排队、Worker 或失败重试时使用</span></div><div className="filter-grid"><label className="wide-field">源码路径<input value={sourcePath} onChange={(event) => setSourcePath(event.target.value)} /></label><FeedbackButton className="primary-action" onClick={() => queue()} disabled={!sourcePath.trim()}>创建后台任务</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => load()}>刷新状态</FeedbackButton></div><p>服务器常驻 Worker：<code>python scripts\sast_worker.py --max-jobs 0 --concurrency 1</code>。本地日常扫描不需要使用这里。</p>{jobs.length ? <PagedTable><thead><tr><th>状态</th><th>阶段 / 进度</th><th>尝试次数</th><th>操作</th></tr></thead><tbody>{jobs.map((job) => <tr key={job.id}><td>{scanStatusLabel(job.status)}<span className="cell-subtext">{formatDateTime(job.created_at)}</span></td><td>{scanStageLabel(job.stage)}<span className="cell-subtext">{job.progress}% {job.error ? `· ${job.error}` : ""}</span></td><td>{job.attempt}</td><td>{job.status === "queued" ? <><FeedbackButton className="secondary-action" onClick={() => run(job)}>立即执行此任务</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => changeJob(job, "cancel")}>取消</FeedbackButton></> : job.status === "failed" || job.status === "cancelled" ? <FeedbackButton className="secondary-action" onClick={() => changeJob(job, "retry")}>重新排队</FeedbackButton> : "-"}</td></tr>)}</tbody></PagedTable> : <div className="empty-project">没有后台 SAST 任务；普通扫描无需创建任务。</div>}</div><div className="panel full"><div className="panel-header"><h2>CI 发布门禁</h2><span>用于决定代码流水线是否允许继续发布，不影响本地查看扫描结果</span></div><fieldset disabled={!isAdmin} className="filter-grid policy-readonly"><label>阻断等级<select value={profile?.quality_gate.threshold ?? "high"} disabled={!profile} onChange={(event) => setProfile((value) => value ? { ...value, quality_gate: { ...value.quality_gate, threshold: event.target.value as SastQualityGate["threshold"] } } : value)}>{(["critical", "high", "medium", "low", "info", "none"] as const).map((item) => <option value={item} key={item}>{severityLabel(item)}</option>)}</select></label><label>适用分支（glob）<input value={profile?.quality_gate.branch_patterns.join(",") ?? "*"} disabled={!profile} onChange={(event) => setProfile((value) => value ? { ...value, quality_gate: { ...value.quality_gate, branch_patterns: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) } } : value)} /></label><label>不参与阻断的规则 ID<input value={profile?.quality_gate.excluded_rule_ids.join(",") ?? ""} disabled={!profile} onChange={(event) => setProfile((value) => value ? { ...value, quality_gate: { ...value.quality_gate, excluded_rule_ids: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) } } : value)} /></label><label className="inline-check"><input type="checkbox" checked={profile?.quality_gate.enabled ?? true} onChange={(event) => setProfile((value) => value ? { ...value, quality_gate: { ...value.quality_gate, enabled: event.target.checked } } : value)} />启用 CI 门禁</label><label className="inline-check"><input type="checkbox" checked={profile?.quality_gate.block_new_only ?? false} onChange={(event) => setProfile((value) => value ? { ...value, quality_gate: { ...value.quality_gate, block_new_only: event.target.checked } } : value)} />只阻断本次新增问题</label><FeedbackButton className="secondary-action" disabled={!profile} onClick={() => saveGate()}>保存门禁配置</FeedbackButton></fieldset>{message ? <div className="empty-project">{message}</div> : null}</div></section>;
}
...
function SastSemanticDelivery({ project }: { project: Project }) {
  ...
  return <details className="advanced-details governance-advanced-details" open><summary>SAST 语义分析、Git 基线与交付门禁</summary><div className="advanced-details-body"><section className="content-grid"><div className="panel full"><div className="panel-header"><h2>受限语义分析</h2><span>Python AST + 直接本地跨函数数据流；JS/TS 为保守本地数据流</span></div><p>覆盖 SQL、命令执行、SSRF、路径穿越和不安全反序列化的 Source → Sink → Sanitizer 检查。结果是可复核的静态线索，不等同于全程序可达性或可利用性证明。</p><div className="filter-grid"><label>Git 基线 revision<input value={profile?.git_baseline_ref ?? ""} placeholder="例如 origin/main 或 HEAD~1" onChange={(event) => setProfile((value) => value ? { ...value, git_baseline_ref: event.target.value } : value)} /></label><label className="inline-check"><input type="checkbox" checked={profile?.scan_git_history_secrets ?? true} onChange={(event) => setProfile((value) => value ? { ...value, scan_git_history_secrets: event.target.checked } : value)} />扫描 Git 历史中的密钥标识（仅保存路径）</label><label className="inline-check"><input type="checkbox" checked={profile?.changed_files_only ?? false} disabled={!profile?.git_baseline_ref} onChange={(event) => setProfile((value) => value ? { ...value, changed_files_only: event.target.checked } : value)} />仅扫描基线差异文件</label><FeedbackButton className="secondary-action" disabled={!profile} onClick={() => saveGitProfile()}>保存基线配置</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => refreshReport()}>刷新报告</FeedbackButton></div>{message ? <div className="empty-project">{message}</div> : null}</div><div className="panel"><div className="panel-header"><h2>质量门禁</h2><span>{report?.quality_gate.status ?? "等待扫描"}</span></div><div className="kv-list"><div><span>阈值</span><strong>{report?.quality_gate.threshold ?? "high"}</strong></div><div><span>阻断 Finding</span><strong>{report?.quality_gate.blocking_finding_count ?? 0}</strong></div><div><span>本次扫描</span><strong>{report?.summary.finding_count ?? 0}</strong></div></div></div><div className="panel"><div className="panel-header"><h2>Git 证据</h2><span>{report?.git.available ? "已关联" : "等待 Git 项目扫描"}</span></div><div className="kv-list"><div><span>基线</span><strong>{report?.git.baseline_ref ?? "未设置"}</strong></div><div><span>差异文件</span><strong>{report?.git.changed_files?.length ?? 0}</strong></div><div><span>历史密钥路径</span><strong>{report?.git.history_secret_count ?? report?.git.history_secret_files?.length ?? 0}</strong></div></div></div><div className="panel full"><div className="panel-header"><h2>项目 Semgrep YAML 规则包</h2><span>草稿、真实引擎预检、发布、启停和离线 materialization 均已接入</span></div><div className="filter-grid"><label>规则包名称<input value={draft.name} onChange={(event) => setDraft((value) => ({ ...value, name: event.target.value }))} /></label><label>保存状态<select value={draft.status} onChange={(event) => setDraft((value) => ({ ...value, status: event.target.value as "draft" | "published" }))}><option value="draft">草稿（不扫描）</option><option value="published">直接发布</option></select></label><label className="wide-field">YAML<textarea value={draft.content} onChange={(event) => setDraft((value) => ({ ...value, content: event.target.value }))} rows={9} /></label><FeedbackButton className="secondary-action" onClick={() => validatePack()}>校验 YAML</FeedbackButton><FeedbackButton className="primary-action" onClick={() => savePack()}>{draft.status === "draft" ? "保存草稿" : "发布规则包"}</FeedbackButton></div>{packs.length ? <PagedTable><thead><tr><th>规则包</th><th>规则 ID</th><th>校验值</th><th>状态</th></tr></thead><tbody>{packs.map((pack) => <tr key={pack.id}><td>{pack.name}<span className="cell-subtext">v{pack.version}</span></td><td>{pack.rule_ids.join(", ")}</td><td>{pack.sha256}</td><td><FeedbackButton className="secondary-action" onClick={() => togglePack(pack)}>{pack.enabled ? "停用" : "启用"}</FeedbackButton></td></tr>)}</tbody></PagedTable> : <div className="empty-project">尚未添加项目 YAML 规则包；默认离线规则包仍会参与 Semgrep 扫描。</div>}</div><div className="panel full"><div className="panel-header"><h2>跨模块验证建议</h2><span>只生成建议，不会自动发送请求或执行沙箱命令</span></div>{report?.validation_suggestions.length ? <ul>{<PagedItems>{report.validation_suggestions.map((item) => <li key={item.finding_id}><strong>{item.recommended_module}</strong> · {item.next_step}</li>)}</PagedItems>}</ul> : <div className="empty-project">执行一次 SAST 后，高风险 Source/Sink 发现会在这里给出人工确认后的 DAST 或 SANDBOX 下一步。</div>}</div></section></div></details>;
}

function SastRuleManagement({ project, view = "engine" }: { project: Project; view?: "engine" | "rules" | "config" }) {
  const { isAdmin } = useAuth();
  ...
  async function load() {
    setLoadError(""); setEditingRule(null);
    const [nextProfile, nextHealth, nextHistory, nextDiff] = await Promise.all([
      request<SastProfile>(`/sast/projects/${project.id}/profile`).catch(error => { setLoadError(`规则配置加载失败：${errorMessage(error)}`); return null; }),
      request<SastToolHealth>("/sast/tool-health").catch(() => null),
      request<SastScanHistoryItem[]>(`/sast/projects/${project.id}/scan-history`).catch(() => []),
  ...
  async function saveProfile(nextProfile: SastProfile | null = profile) {
    if (!nextProfile) return;
    try {
      setProfile(await request<SastProfile>(`/sast/projects/${project.id}/profile`, { method: "PATCH", body: JSON.stringify(isAdmin ? nextProfile : { semgrep_enabled: nextProfile.semgrep_enabled }) }));
  ...
  async function createRule() {
    try {
      const saved = await request<SastProfile>(`/sast/projects/${project.id}/rules${editingRule ? `/${editingRule}` : ""}`, { method: editingRule ? "PATCH" : "POST", body: JSON.stringify(rulePayload()) });
      setProfile(saved); setEditingRule(null); setValidation(null); setDraft({ rule_id: "CUSTOM.PROJECT.PATTERN", title: "", severity: "medium", category: "custom", pattern: "", file_extensions: ".py,.ts", description: "", remediation: "", test_sample: "" });
  ...
  return <section className={`content-grid sast-rule-workspace ${view}`}><div className="panel full sast-rule-config-panel"><div className="panel-header"><h2>扫描配置与规则版本</h2><span>{message || `配置 v${profile?.profile_version ?? "-"} · 本地规则包 ${profile?.rule_pack_version ?? "-"}`}</span></div><div className="filter-grid sast-scan-config-grid"><div className="sast-base-rule-summary"><span>基础 Semgrep 规则</span><strong>平台内置离线安全规则</strong><small>随平台发布 · 配置标识 {profile?.semgrep_config ?? "builtin/offline-default.yml"}</small></div><label className="inline-check"><input type="checkbox" checked={profile?.semgrep_enabled ?? false} disabled={!profile} onChange={(event) => { if (!profile) return; const next = { ...profile, semgrep_enabled: event.target.checked }; setProfile(next); void saveProfile(next); }} /><span><strong>启用 Semgrep</strong><small>切换后立即保存</small></span></label><label className="inline-check"><input type="checkbox" checked={profile?.include_local_rules ?? false} disabled={!profile || !isAdmin} onChange={(event) => { if (!profile) return; const next = { ...profile, include_local_rules: event.target.checked }; setProfile(next); void saveProfile(next); }} /><span><strong>启用本地规则</strong><small>切换后立即保存</small></span></label><label className="inline-check"><input type="checkbox" checked={profile?.clear_previous ?? true} disabled={!profile || !isAdmin} onChange={(event) => setProfile((value) => value ? { ...value, clear_previous: event.target.checked } : value)} /><span><strong>关闭旧 Finding</strong><small>新扫描保存时执行</small></span></label><FeedbackButton className="secondary-action" disabled={!profile || !isAdmin} onClick={() => saveProfile()}>保存其他配置</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => downloadCiConfig()}>导出 CI 配置</FeedbackButton></div></div><div className="panel sast-semgrep-engine-panel"><div className="panel-header"><h2>固定版 Semgrep</h2><span>{health?.can_run_semgrep ? "可用" : "离线降级"}</span></div><div className="kv-list"><div><span>CLI</span><strong>{health?.semgrep_cli.version ?? "未检测到"}</strong></div><div><span>Docker 镜像</span><strong>{health?.docker_image.image ?? "-"}</strong></div><div><span>镜像状态</span><strong>{health?.docker_image.available ? "本地已准备" : "未找到（不会自动下载）"}</strong></div></div><p>勾选后快速扫描也会执行 Semgrep；若工具不可用，扫描历史会显示“降级”和具体原因。</p></div><div className="panel sast-scan-diff-panel"><div className="panel-header"><h2>扫描差异</h2><span>{diff?.base_scan_id ? "与上一批次比较" : "等待第二次扫描"}</span></div><div className="kv-list"><div><span>新增</span><strong>{diff?.summary.added ?? 0}</strong></div><div><span>消失</span><strong>{diff?.summary.removed ?? 0}</strong></div><div><span>等级变化</span><strong>{diff?.summary.severity_changed ?? 0}</strong></div><div><span>历史批次</span><strong>{history.length}</strong></div></div></div><div className="panel full sast-local-rules-panel">{loadError && <div role="alert" className="report-error">{loadError}</div>}{message && <p role="status">{message}</p>}<p>当前项目：{project.name}。以下仅显示该项目人工创建的正则规则；平台内置检测逻辑和 Semgrep YAML / 社区规则不属于此列表。</p><FeedbackButton className="secondary-action" onClick={() => load()}>重新加载规则</FeedbackButton><div className="panel-header"><h2>项目自定义规则</h2><span>正则规则会在本地 SAST 引擎中实际运行；样例仅做命中预览</span></div><div className="filter-grid"><label>规则 ID<input value={draft.rule_id} onChange={(event) => setDraft((value) => ({ ...value, rule_id: event.target.value }))} /></label><label>标题<input value={draft.title} onChange={(event) => setDraft((value) => ({ ...value, title: event.target.value }))} /></label><label>等级<select value={draft.severity} onChange={(event) => setDraft((value) => ({ ...value, severity: event.target.value as Severity }))}>{(["critical", "high", "medium", "low", "info"] as Severity[]).map((item) => <option key={item}>{item}</option>)}</select></label><label>分类<input value={draft.category} onChange={(event) => setDraft((value) => ({ ...value, category: event.target.value }))} /></label><label>文件后缀（逗号分隔）<input value={draft.file_extensions} onChange={(event) => setDraft((value) => ({ ...value, file_extensions: event.target.value }))} /></label><label>正则模式<input value={draft.pattern} onChange={(event) => setDraft((value) => ({ ...value, pattern: event.target.value }))} /></label><label>命中样例<textarea value={draft.test_sample} onChange={(event) => setDraft((value) => ({ ...value, test_sample: event.target.value }))} /></label><label>说明<input value={draft.description} onChange={(event) => setDraft((value) => ({ ...value, description: event.target.value }))} /></label><label>修复建议<input value={draft.remediation} onChange={(event) => setDraft((value) => ({ ...value, remediation: event.target.value }))} /></label><FeedbackButton className="secondary-action" onClick={() => validateRule()}>校验 / 预览</FeedbackButton><FeedbackButton className="primary-action" disabled={!profile || !isAdmin} onClick={() => createRule()}>{editingRule ? "保存修改" : "保存并启用"}</FeedbackButton></div>{validation ? <div className="empty-project">规则有效；样例：{validation.test_sample_matched === null ? "未提供" : validation.test_sample_matched ? "命中" : "未命中"}。</div> : null}{profile?.custom_rules.length ? <PagedTable><thead><tr><th>ID / 版本</th><th>规则</th><th>范围</th><th>状态</th></tr></thead><tbody>{profile.custom_rules.map((rule) => <tr key={rule.id}><td>{rule.rule_id}<span className="cell-subtext">v{rule.version}</span></td><td><strong>{rule.title}</strong><span className="cell-subtext">{rule.pattern}</span></td><td>{rule.file_extensions.join(", ") || "所有已扫描文件"}<span className="cell-subtext">{rule.severity} · {rule.category}</span></td><td><FeedbackButton className="secondary-action" onClick={() => { setEditingRule(rule.id); setValidation(null); setDraft({ rule_id: rule.rule_id, title: rule.title, severity: rule.severity, category: rule.category, pattern: rule.pattern, file_extensions: rule.file_extensions.join(","), description: rule.description ?? "", remediation: rule.remediation ?? "", test_sample: "" }); setMessage(`正在编辑 ${rule.rule_id}`); }}>编辑</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => toggleRule(rule)}>{rule.enabled ? "停用" : "启用"}</FeedbackButton></td></tr>)}</tbody></PagedTable> : <div className="empty-project">{loadError ? "加载失败，无法判断是否存在自定义规则。" : "当前项目尚未创建自定义正则规则；Semgrep YAML 请到「Semgrep 规则」查看。"}</div>}</div></section>;
}
...
function SastEvidenceGovernance({ project }: { project: Project }) {
  ...
  async function downloadReport(format: "json" | "html" | "sarif") {
    ...
    try {
      const response = await feedbackFetch(`${API_BASE}/sast/projects/${project.id}/${endpoint}?scan_task_id=${selectedScanId}`, { credentials: "include" });
      if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
  ...
  return <section className="content-grid sast-evidence-workspace">
    <div className="panel full"><div className="panel-header"><h2>规则 / 路径豁免</h2><span>{message || "豁免仅影响后续扫描，历史原始批次保持不变"}</span></div><fieldset disabled={!isAdmin} className="filter-grid policy-readonly"><label>规则 ID 或 *<input value={suppression.rule_id} onChange={(event) => setSuppression((value) => ({ ...value, rule_id: event.target.value }))} /></label><label>相对路径 glob<input value={suppression.path_pattern} onChange={(event) => setSuppression((value) => ({ ...value, path_pattern: event.target.value }))} /></label><label>失效日期（可选）<input type="date" value={suppression.expires_at} onChange={(event) => setSuppression((value) => ({ ...value, expires_at: event.target.value }))} /></label><label>豁免理由<input value={suppression.reason} onChange={(event) => setSuppression((value) => ({ ...value, reason: event.target.value }))} placeholder="例如：测试夹具中的固定假数据" /></label><FeedbackButton className="primary-action" onClick={() => addSuppression()}>新增豁免</FeedbackButton></fieldset>{profile?.suppressions.length ? <PagedTable><thead><tr><th>规则</th><th>路径</th><th>理由</th><th>有效期</th><th>操作</th></tr></thead><tbody>{profile.suppressions.map((item) => <tr key={item.id}><td>{item.rule_id}</td><td>{item.path_pattern}</td><td>{item.reason}</td><td>{item.expires_at ? formatDateTime(item.expires_at) : "永久"}</td><td><FeedbackButton className="secondary-action" disabled={!isAdmin} onClick={() => toggleSuppression(item)}>{item.enabled ? "停用" : "启用"}</FeedbackButton></td></tr>)}</tbody></PagedTable> : <div className="empty-project">暂无项目级豁免。</div>}</div>
    <div className="panel full"><div className="panel-header"><h2>扫描历史与报告</h2><span>{history.length} 个已完成批次</span></div><div className="filter-grid"><label>报告批次<select value={selectedScanId} onChange={(event) => setSelectedScanId(event.target.value)}><option value="">请选择批次</option>{history.map((item) => <option key={item.scan_task_id} value={item.scan_task_id}>{formatDateTime(item.finished_at ?? item.created_at)} · {item.scan_task_id.slice(0, 8)}</option>)}</select></label><FeedbackButton className="secondary-action" disabled={!selectedScanId} onClick={() => downloadReport("json")}>导出 JSON</FeedbackButton><FeedbackButton className="secondary-action" disabled={!selectedScanId} onClick={() => downloadReport("html")}>导出 HTML</FeedbackButton><FeedbackButton className="secondary-action" disabled={!selectedScanId} onClick={() => downloadReport("sarif")}>导出 SARIF</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => load()}>刷新历史</FeedbackButton></div>{selected ? <div className="kv-list"><div><span>完成时间</span><strong>{formatDateTime(selected.finished_at ?? selected.created_at)}</strong></div><div><span>Finding / 已抑制</span><strong>{selected.finding_count} / {selected.suppressed_count}</strong></div><div><span>Semgrep</span><strong>{selected.engine_status.semgrep?.status ?? "旧批次未记录"}</strong></div><div><span>Semgrep 说明</span><strong>{selected.engine_status.semgrep?.detail ?? selected.engine_status.semgrep?.config ?? "-"}</strong></div><div><span>本地规则</span><strong>{selected.engine_status.local_rules?.status ?? "旧批次未记录"}</strong></div><div><span>配置版本</span><strong>v{selected.profile.profile_version ?? "-"}</strong></div></div> : <div className="empty-project">执行一次 SAST 后，可按批次查看引擎状态并下载三种报告。</div>}</div>
  </section>;
}
  ...
  return <div className="governance-view governance-closed-loop">
    ...
    <section className="governance-two-column">
      ...
      <div className="panel">
        ...
        <div className="module-result-overview">{<PagedItems>{moduleCards.map((moduleKey) => <div key={moduleKey}><strong>{MODULE_DISPLAY[moduleKey].name}</strong><span>{moduleOverviewText(moduleKey, components, findings, validations, evidence)}</span></div>)}</PagedItems>}</div>
      </div>
    </section>
...
function RiskEvidenceChainCard({ finding, validations, evidence, graph, canRunDast, canRunSandbox, onOpenDast, onOpenSandbox, onUpdateFinding }: { finding: Finding; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; canRunDast: boolean; canRunSandbox: boolean; onOpenDast: (findingId: string) => void; onOpenSandbox: (findingId: string) => void; onUpdateFinding: (findingId: string, patch: Partial<Pick<Finding, "status">>) => Promise<void> }) {
  ...
  return <article className="risk-chain-card">
    ...
    <div className="risk-chain-footer"><div className="risk-chain-meta"><span>显式关系节点：{graphNodes.length}</span><span>DAST 记录：{relatedValidations.length}</span><span>SANDBOX 证据：{relatedEvidence.length}</span></div><div className="risk-chain-actions">{relatedValidations.length === 0 ? <FeedbackButton className="secondary-action" disabled={!canRunDast} onClick={() => onOpenDast(finding.id)}>{canRunDast ? "发起 DAST 验证" : "DAST 未接入"}</FeedbackButton> : relatedEvidence.length === 0 ? <FeedbackButton className="secondary-action" disabled={!canRunSandbox} onClick={() => onOpenSandbox(finding.id)}>{canRunSandbox ? "进入 SANDBOX 取证" : "SANDBOX 未接入"}</FeedbackButton> : <FeedbackButton className="secondary-action" onClick={() => onOpenSandbox(finding.id)}>查看关联证据</FeedbackButton>}</div></div>
  </article>;
}
...
function AttackChainSummary({ chains }: { chains: AttackChain[] }) {
  ...
  return <div className="attack-chain-list">{pagination.items.map((chain) => <details key={chain.id}><summary><span className={`severity ${chain.severity}`}>{severityLabel(chain.severity)}</span><strong>{chain.name}</strong><small>{chain.modules.join(" → ")} · 可信度 {chain.confidence}%</small></summary><ol>{<PagedItems>{chain.steps.map((step) => <li key={`${chain.id}-${step.node_id ?? step.title}`}><b>{step.module}</b><span>{step.title}</span><small>{step.evidence ?? "无证据摘要"}</small></li>)}</PagedItems>}</ol><p>{chain.recommended_action}</p></details>)}<Pagination page={pagination.page} pageCount={pagination.pageCount} total={chains.length} onPageChange={setPage} /></div>;
}
...
function KnowledgeHubView({ project, findings, validations, evidence, summary }: { project: Project | null; findings: Finding[]; validations: DastValidation[]; evidence: SandboxEvidence[]; summary: AspmSummary | null }) {
  ...
  const candidatePagination = paginate(knowledgeCandidates, candidatePage);
  ...
  return <section className="knowledge-command-center">
    ...
    <nav className="knowledge-tabs" aria-label="安全知识中枢工作区">
      {tabs.map(([key, label, detail]) => <FeedbackButton className={activeTab === key ? "active" : ""} key={key} onClick={() => setActiveTab(key)}><span>{label}</span><small>{detail}</small></FeedbackButton>)}
    </nav>
    ...
    {activeTab === "overview" ? <>
      ...
      <section className="knowledge-overview-grid">
        ...
        <article className="knowledge-ready-card">
          ...
          <FeedbackButton className="secondary-action" onClick={() => setActiveTab("candidates")}>查看知识候选 <ArrowRight size={15} /></FeedbackButton>
        </article>
      </section>
    </> : null}
      ...
      <div className="knowledge-candidate-grid">{candidatePagination.items.length ? candidatePagination.items.map(({ finding, type, state, tone, validationCount, evidenceCount }) => {
        ...
        return <article className="knowledge-candidate-card" key={finding.id}>
          ...
          <footer><span className={"knowledge-state " + persistedTone}>{persistedState}</span><div className="knowledge-candidate-actions">
            {!entry || entry.status === "rejected" ? <FeedbackButton className="secondary-action" disabled={knowledgeBusyId === finding.id} onClick={() => submitKnowledgeCandidate(finding)}>{knowledgeBusyId === finding.id ? "提交中…" : entry ? "补充后重提" : "提交审核"}</FeedbackButton> : null}
            {isAdmin && entry?.status === "pending_review" ? <><FeedbackButton className="primary-action" title={entry.publish_ready ? "证据门槛已满足" : "需先补充 DAST、SANDBOX 或治理结论"} disabled={!entry.publish_ready || knowledgeBusyId === entry.id} onClick={() => reviewKnowledgeEntry(entry, "publish")}>审核发布</FeedbackButton><FeedbackButton className="secondary-action" disabled={knowledgeBusyId === entry.id} onClick={() => reviewKnowledgeEntry(entry, "reject")}>退回</FeedbackButton></> : null}
            {isAdmin && entry?.status === "published" ? <FeedbackButton className="secondary-action" disabled={knowledgeBusyId === entry.id || entry.version <= 1} onClick={() => rollbackKnowledgeEntry(entry)}>回滚上一版</FeedbackButton> : null}
          </div></footer>
        </article>;
    ...
    {activeTab === "enterprise" ? <section className="knowledge-enterprise-workspace">
      ...
      <section className="knowledge-enterprise-panel">
        ...
        <div className="knowledge-enterprise-grid">{publishedProjectEntries.length ? <PagedItems>{publishedProjectEntries.map((entry) => <KnowledgeEntryCard key={entry.id} entry={entry} />)}</PagedItems> : <div className="knowledge-empty">当前项目尚未发布企业知识。先在“知识候选”中提交并完成审核。</div>}</div>
      </section>
      <section className="knowledge-enterprise-panel">
        ...
        <div className="knowledge-enterprise-grid">{knowledgeWorkspace?.recommendations.length ? <PagedItems>{knowledgeWorkspace.recommendations.map((item) => <article className="knowledge-recommendation-card" key={item.entry.id}><div><span>{item.entry.source_project_name}</span><strong>{item.score}% 匹配</strong></div><h4>{item.entry.title}</h4><p>{truncateText(item.entry.summary, 180)}</p><small>{item.reasons.join(" · ")} · 命中 {item.matched_finding_ids.length} 条当前 Finding</small><footer><span>{item.entry.source_module} · {item.entry.rule_id}</span><b>v{item.entry.version}</b></footer></article>)}</PagedItems> : <div className="knowledge-empty">暂时没有与当前项目规则或风险分类匹配的已发布知识。</div>}</div>
      </section>
    </section> : null}
    ...
    {activeTab === "library" ? <section className="knowledge-library-workspace">
      ...
      <section className="knowledge-library-panel">
        ...
        <div className="knowledge-strategy-list">{dastLibrary?.total ? <PagedItems>{[...dastLibrary.builtin, ...dastLibrary.learned].map((item) => <article key={String(item.id)}><div><Activity size={16} /><span><strong>{String(item.name)}</strong><small>{truncateText(String(item.description ?? item.scope_summary ?? "未记录策略范围"), 90)}</small></span></div><b>{item.source === "deepseek_local" ? "项目经验" : "内置模板"}</b></article>)}</PagedItems> : <div className="knowledge-empty">当前没有可用的 DAST 策略。</div>}</div>
      </section>
    ...
    <section className="knowledge-report-bar">
      ...
      <div><FeedbackButton className="primary-action" disabled={reportLoading} onClick={() => generateReportPreview()}>{reportLoading ? "正在生成…" : report ? "刷新报告" : "生成报告预览"}</FeedbackButton><FeedbackButton className="secondary-action" disabled={!report} onClick={() => report && downloadSecurityReport(report, "json")}>导出 JSON</FeedbackButton><FeedbackButton className="secondary-action" disabled={!report} onClick={() => report && downloadSecurityReport(report, "html")}>导出 HTML</FeedbackButton></div>
...
function KnowledgeEntryCard({ entry }: { entry: KnowledgeEntry }) {
  return <article className="knowledge-enterprise-card"><div><span>{knowledgeTypeLabel(entry.knowledge_type)}</span><b>v{entry.version}</b></div><h4>{entry.title}</h4><p>{truncateText(entry.summary, 180)}</p><dl><div><dt>来源</dt><dd>{entry.source_project_name} · {entry.source_module}</dd></div><div><dt>规则</dt><dd>{entry.rule_id}</dd></div><div><dt>审核</dt><dd>{entry.reviewer ?? "未记录"} · {formatDateTime(entry.published_at)}</dd></div></dl><footer>{<PagedItems>{entry.tags.map((tag) => <span key={tag}>{tag}</span>)}</PagedItems>}</footer></article>;
}
...
function SecurityReportPreview({ report }: { report: SecurityReport }) {
  ...
  return <section className="report-preview">
    ...
    <section className="panel report-boundaries"><div className="panel-header"><h2>报告能力边界</h2><span>避免将演示能力误读为生产结论</span></div>{<PagedItems>{Object.entries(report.capability_boundaries).map(([module, items]) => <details key={module} open><summary>{module}</summary><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul></details>)}</PagedItems>}</section>
  </section>;
}
...
function ReportDataSection({ title, emptyText, rows }: { title: string; emptyText: string; rows: ReportRow[] }) {
  ...
  return <section className="panel report-data-section"><div className="panel-header"><h2>{title}</h2><span>完整结果 · 每页 10 条</span></div>{rows.length === 0 ? <div className="empty-project">{emptyText}</div> : <><table className="concise-table"><thead><tr><th>条目</th><th>结论 / 摘要</th><th>完整字段</th></tr></thead><tbody>{pagination.items.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><span className="cell-subtext">{item.subtitle}</span></td><td>{item.summary}</td><td><details className="report-row-details"><summary>查看完整记录</summary><dl>{<PagedItems>{item.details.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</PagedItems>}</dl></details></td></tr>)}</tbody></table><Pagination page={pagination.page} pageCount={pagination.pageCount} total={rows.length} onPageChange={setPage} /></>}</section>;
}

type ScaWorkspaceTab = "overview" | "risks" | "dependencies" | "exceptions" | "history";

function ScaWorkspaceNavigation({ active, onChange }: { active: ScaWorkspaceTab; onChange: (value: ScaWorkspaceTab) => void }) {
  const items: Array<[ScaWorkspaceTab, string, string]> = [
    ["overview", "概览", "态势与扫描能力"],
    ["risks", "风险组件", "漏洞与修复"],
    ["dependencies", "依赖影响", "图谱与升级路径"],
    ["exceptions", "例外与 VEX", "适用性和策略"],
    ["history", "历史与报告", "批次、SBOM 与证据"],
  ];
  return <nav className="module-workspace-tabs" aria-label="SCA 工作区">{items.map(([key, label, description]) => <FeedbackButton type="button" className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined} onClick={() => onChange(key)} key={key}><strong>{label}</strong><span>{description}</span></FeedbackButton>)}</nav>;
}
...
function ScaOverviewCapabilities({ project, components, toolScanEnabled, loading, onToolScanChange, onExportSbom, onExportReport, onOpenTab }: { project: Project | null; components: Component[]; toolScanEnabled: boolean; loading: boolean; onToolScanChange: (enabled: boolean) => void; onExportSbom: (format: "cyclonedx" | "spdx") => Promise<void>; onExportReport: () => Promise<void>; onOpenTab: (value: ScaWorkspaceTab) => void }) {
  ...
  return <section className="module-overview-workspace">
    <div className="module-overview-cards">
      <FeedbackButton type="button" onClick={() => onOpenTab("risks")}><span>风险组件</span><strong>{components.filter(isRiskyScaComponent).length}</strong><small>进入漏洞与修复清单</small><ArrowRight size={17} /></FeedbackButton>
      <FeedbackButton type="button" onClick={() => onOpenTab("dependencies")}><span>依赖影响</span><strong>查看图谱</strong><small>定位直接依赖升级杠杆</small><ArrowRight size={17} /></FeedbackButton>
      <FeedbackButton type="button" onClick={() => onOpenTab("exceptions")}><span>适用性治理</span><strong>例外与 VEX</strong><small>记录本地治理结论</small><ArrowRight size={17} /></FeedbackButton>
      <FeedbackButton type="button" onClick={() => onOpenTab("history")}><span>交付与追溯</span><strong>历史和报告</strong><small>SBOM、批次与扫描证据</small><ArrowRight size={17} /></FeedbackButton>
    </div>
    <section className="panel full module-delivery-actions"><div><span>当前扫描能力</span><h3>基础解析始终可用，Docker 增强按需启用</h3><p>SCA 在网络可用时优先查询在线 OSV，失败时回退人工准备的本地镜像；勾选 Docker 增强后，无论快速或深度范围都会实际执行 Syft/Grype/Trivy。只有 package.json 而没有锁文件时，会在临时隔离容器中生成依赖快照，且不会修改项目源码。</p></div><div className="advanced-actions"><label className="inline-check"><input type="checkbox" checked={toolScanEnabled} disabled={loading} onChange={(event) => onToolScanChange(event.target.checked)} />使用 Docker 增强扫描</label><FeedbackButton className="secondary-action" disabled={loading || !project || components.length === 0} onClick={() => onExportSbom("cyclonedx")}>导出 CycloneDX</FeedbackButton><FeedbackButton className="secondary-action" disabled={loading || !project || components.length === 0} onClick={() => onExportReport()}>导出 SCA 报告</FeedbackButton></div></section>
...
function GrypeDatabasePanel() {
  const download = useDownloadPermission("grype_download_allowed");
  ...
  return <section className="panel full grype-database-panel">
    <div className="panel-header"><div><h2>Grype 漏洞数据库</h2><p>{download.detail}</p><span>扫描保持离线；只在点击更新时联网下载</span></div><span>{grypeDatabaseStatusLabel(database?.status)}</span></div>
    <div className="kv-list">
      ...
      <div className="grype-database-actions"><span>操作</span><strong><FeedbackButton className="secondary-action" disabled={checking || updating} onClick={() => checkDatabase()}>{checking ? "检测中" : "检测 Grype 数据库"}</FeedbackButton>{needsUpdate ? <FeedbackButton className="primary-action" disabled={updating || !download.allowed} onClick={() => updateDatabase()}>{updating ? "更新中" : "更新到最新"}</FeedbackButton> : null}</strong></div>
    </div>
...
function ScaGovernanceView({ project, components, summary, comparison, scanHistory, selectedScanId, scanDiff, dependencyGraph, toolScanEnabled, loading, onToolScanChange, onSelectScan, onExportSbom, onExportReport, onRun }: { project: Project | null; components: Component[]; summary: AspmSummary | null; comparison: FindingRetestComparison | null; scanHistory: ScaScanHistoryItem[]; selectedScanId: string | null; scanDiff: ScaScanDiff | null; dependencyGraph: DependencyGraph | null; toolScanEnabled: boolean; loading: boolean; onToolScanChange: (enabled: boolean) => void; onSelectScan: (scanTaskId: string) => Promise<void>; onExportSbom: (format: "cyclonedx" | "spdx") => Promise<void>; onExportReport: () => Promise<void>; onRun: () => Promise<void> }) {
  ...
  useEffect(() => { setWorkspace("overview"); }, [project?.id]);
  return <ModuleGovernanceShell moduleKey="sca" lastStatus={summary?.sca_governance.latest_scan_status ?? null} metrics={[["当前组件数", components.length], ["风险组件数", risky.length], ["严重 / 高危组件", high.length], ["建议升级组件", risky.filter((item) => item.remediation).length]]} action={risky.length ? "优先升级高危组件，并确认升级是否影响直接和传递依赖。" : "当前未发现高风险组件，建议定期重新扫描依赖。"} loading={loading} onRun={onRun}>
    <div className="module-workspace sca-workspace"><ScaWorkspaceNavigation active={workspace} onChange={setWorkspace} />
...
function ScaExceptionPanel({ project, components }: { project: Project | null; components: Component[] }) {
  ...
  return <section className="panel full"><div className="panel-header"><div><h3>风险例外审批</h3><span>仅用于有明确业务理由且限定范围的临时接受风险</span></div><FeedbackButton className="secondary-action" disabled={saving} onClick={() => refresh()}>刷新</FeedbackButton></div><p>批准且未过期的例外会在下一次 SCA 扫描中标记为“已接受风险”，但原始漏洞证据、扫描历史和门禁审计仍会保留。申请与审批记录使用当前登录身份；普通用户只能申请，管理员负责批准或撤销。</p><div className="filter-grid"><select value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)}><option value="">选择一个当前风险组件</option>{riskyComponents.map((item) => <option key={item.id} value={componentKey(item)}>{item.name} {item.version ?? "版本未知"} · {item.ecosystem} · {severityLabel(item.severity)}</option>)}</select><input placeholder="接受风险的业务原因（必填）" value={reason} onChange={(event) => setReason(event.target.value)} /><span>申请人：{user.username}</span><label><span>失效日期（可选）</span><input type="date" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} /></label><FeedbackButton className="secondary-action" disabled={saving || !selected || !reason.trim()} onClick={() => submit()}>{saving ? "处理中" : "提交例外申请"}</FeedbackButton></div>{isAdmin ? <div className="filter-grid"><span>审批身份：{user.username}（管理员）</span></div> : null}{riskyComponents.length === 0 ? <div className="empty-project">当前扫描没有可申请例外的风险组件。</div> : null}{error ? <div className="report-error">{error}</div> : null}<table className="compact-table"><thead><tr><th>组件与范围</th><th>业务理由</th><th>状态 / 到期</th><th>审批操作</th></tr></thead><tbody>{items.length ? pagination.items.map((item) => <tr key={item.id}><td><strong>{item.package_name}</strong><span className="cell-subtext">{item.ecosystem} · {item.package_version ?? "全部版本"}</span></td><td>{item.reason}<span className="cell-subtext">申请人：{item.requester ?? "未填写"} · {item.requester_role ?? "未声明角色"}</span></td><td>{scaExceptionStatusLabel(item.status)}<span className="cell-subtext">{item.expires_at ? `到期：${formatDateTime(item.expires_at)}` : "未设置到期日"}</span></td><td>{!isAdmin ? <span>由管理员处理</span> : item.status === "pending" ? <><FeedbackButton className="secondary-action" disabled={saving} onClick={() => update(item, "approved")}>批准</FeedbackButton><FeedbackButton className="secondary-action" disabled={saving} onClick={() => update(item, "rejected")}>拒绝</FeedbackButton></> : item.status === "approved" ? <><span className="cell-subtext">{item.approver ?? "已批准"} · {item.approver_role ?? "-"}</span><FeedbackButton className="secondary-action" disabled={saving} onClick={() => update(item, "revoked")}>撤销</FeedbackButton></> : item.approver ?? "-"}<details><summary>审计记录</summary>{<PagedItems>{(item.approval_history ?? []).map((event, index) => <span className="cell-subtext" key={index}>{textValue(event.status)} · {textValue(event.role)} · {formatDateTime(typeof event.at === "string" ? event.at : null)}</span>)}</PagedItems>}</details></td></tr>) : <tr><td colSpan={4} className="empty-cell">暂无例外申请。</td></tr>}</tbody></table><Pagination page={pagination.page} pageCount={pagination.pageCount} total={items.length} onPageChange={setPage} /></section>;
}
...
function OsvMirrorPanel() {
  ...
  useEffect(() => { void refresh(); }, []);
  return <section className="panel full"><div className="panel-header"><div><h3>OSV 离线镜像</h3><span>在线查询失败时自动回退；请人工导入你维护的本地漏洞库。</span></div><FeedbackButton className="secondary-action" disabled={saving} onClick={() => refresh()}>刷新状态</FeedbackButton></div><div className="kv-list"><div><span>状态</span><strong>{scaDataStatusLabel(status?.status)}</strong></div><div><span>记录数</span><strong>{textValue(status?.entry_count)}</strong></div><div><span>保存位置</span><strong>{textValue(status?.path)}</strong></div></div><p>{textValue(status?.detail)}</p><details><summary>导入本地漏洞库 JSON</summary><p>输入 JSON 数组；每条记录需包含 ecosystem、package、version 或 affected，以及 vulnerabilities。导入文件仅写入被 Git 忽略的离线资源目录。</p><div className="filter-grid"><input value={source} onChange={(event) => setSource(event.target.value)} placeholder="情报来源标识" /><textarea value={rawEntries} onChange={(event) => setRawEntries(event.target.value)} placeholder='[{"ecosystem":"pypi","package":"example","version":"1.0.0","vulnerabilities":[{"id":"CVE-...","severity":"high","summary":"..."}]}]' /><FeedbackButton className="secondary-action" disabled={saving || !rawEntries.trim()} onClick={() => importMirror()}>{saving ? "导入中" : "导入漏洞库"}</FeedbackButton></div></details>{error ? <div className="report-error">{error}</div> : null}</section>;
}
...
function ScaIntelligencePanel() {
  ...
  useEffect(() => { void refresh(); }, []);
  return <section className="panel full"><div className="panel-header"><div><h3>补充风险数据（可选）</h3><span>用于补充风险分、已被利用标记和修复版本；未导入时不会伪造这些信息。</span></div><FeedbackButton className="secondary-action" disabled={saving} onClick={() => refresh()}>刷新状态</FeedbackButton></div><div className="kv-list"><div><span>状态</span><strong>{scaDataStatusLabel(status?.status)}</strong></div><div><span>通告数</span><strong>{textValue(status?.advisory_count)}</strong></div><div><span>更新时间</span><strong>{formatDateTime(typeof status?.updated_at === "string" ? status.updated_at : null)}</strong></div></div><details><summary>导入补充风险数据 JSON</summary><p>每项使用 <code>id</code> 或 <code>cve</code>，可选 <code>cvss</code>（0–10）、<code>epss</code>（0–1）、<code>kev</code>、<code>known_exploited</code>、<code>fixed_version</code>。</p><div className="filter-grid"><input value={source} onChange={(event) => setSource(event.target.value)} placeholder="情报来源标识" /><textarea value={rawEntries} onChange={(event) => setRawEntries(event.target.value)} placeholder='[{"cve":"CVE-2026-0001","cvss":9.8,"epss":0.91,"kev":true,"fixed_version":"2.0.1"}]' /><FeedbackButton className="secondary-action" disabled={saving || !rawEntries.trim()} onClick={() => importEntries()}>{saving ? "导入中" : "导入数据"}</FeedbackButton></div></details>{error ? <div className="report-error">{error}</div> : null}</section>;
}
...
function ScaVexPanel({ project, components }: { project: Project | null; components: Component[] }) {
  ...
  useEffect(() => { void refresh(); }, [project?.id]);
  if (!project) return null;
  return <section className="panel full"><div className="panel-header"><div><h3>VEX 适用性结论</h3><span>对特定组件漏洞记录“未受影响、已修复、受影响或调查中”，不删除原始漏洞证据</span></div><FeedbackButton className="secondary-action" disabled={saving} onClick={() => refresh()}>刷新</FeedbackButton></div>{isAdmin ? <div className="filter-grid"><select value={selectedKey} onChange={(event) => { setSelectedKey(event.target.value); const component = components.find((item) => componentKey(item) === event.target.value); setVulnerabilityId(component?.vulnerability_ids?.[0] ?? ""); }}><option value="">选择组件</option>{components.filter((item) => item.vulnerability_ids?.length).map((item) => <option key={item.id} value={componentKey(item)}>{item.name} · {item.version ?? "-"} · {item.vulnerability_ids?.join(", ")}</option>)}</select><input value={vulnerabilityId} onChange={(event) => setVulnerabilityId(event.target.value)} placeholder="漏洞 ID，例如 CVE-2026-0001" /><select value={status} onChange={(event) => setStatus(event.target.value as ScaVex["status"])}><option value="not_affected">未受影响</option><option value="fixed">已修复</option><option value="affected">受影响</option><option value="under_investigation">调查中</option></select><span>结论提交人：{user.username}</span><textarea value={justification} onChange={(event) => setJustification(event.target.value)} placeholder="适用性依据、缓解措施或修复说明" /><FeedbackButton className="secondary-action" disabled={saving || !selected || !vulnerabilityId.trim()} onClick={() => submit()}>{saving ? "保存中" : "保存 VEX 结论"}</FeedbackButton></div> : <p>VEX 结论由管理员维护；普通用户可以查看结论与依据。</p>}{error ? <div className="report-error">{error}</div> : null}<PagedTable className="compact-table"><thead><tr><th>组件 / 漏洞</th><th>结论</th><th>依据</th><th>有效期</th></tr></thead><tbody>{items.length ? items.map((item) => <tr key={item.id}><td><strong>{item.package_name}</strong><span className="cell-subtext">{item.ecosystem} · {item.package_version ?? "全部版本"} · {item.vulnerability_id}</span></td><td>{vexStatusLabel(item.status)}<span className="cell-subtext">{item.actor ?? "未填写提交人"}</span></td><td>{item.justification ?? item.action_statement ?? item.evidence ?? "-"}</td><td>{item.expires_at ? formatDateTime(item.expires_at) : "未设置"}</td></tr>) : <tr><td colSpan={4} className="empty-cell">暂无 VEX 结论。</td></tr>}</tbody></PagedTable></section>;
}
...
function ScaPolicyOverridePanel({ project }: { project: Project | null }) {
  ...
  const saveCustomPolicy = async () => {
    ...
    try {
      ...
      setError("");
      await request("/sca/policies/overrides", { method: "POST", body: JSON.stringify({ project_id: scope === "project" ? project?.id : null, policy_kind: policyKind, policy_id: policyId.trim(), enabled: policyEnabled, config, actor: "platform-admin", change_note: "通过治理页新增或覆盖策略" }) });
      setMessage("策略已保存，后续扫描将使用该覆盖配置；已有扫描结果不重写。"); await refresh();
    } catch (requestError) { setError(`策略保存失败：${errorMessage(requestError)}`); }
    finally { setSaving(false); }
  };
  useEffect(() => { void refresh(); }, [project?.id]);
  if (!project) return null;
  const edit = (kind: "vulnerability" | "license" | "gate", id: string) => {
    const value = kind === "vulnerability" ? policies?.vulnerability_rules.find(item => item.id === id) : kind === "license" ? policies?.license_policies.find(item => item.id === id) : policies?.gate_policy;
    const config = { ...value } as Record<string, unknown>;
    setPolicyEnabled(config.enabled !== false);
    delete config.id; delete config.source; delete config.enabled;
    setPolicyKind(kind); setPolicyId(id); setPolicyConfig(JSON.stringify(config, null, 2)); setEditorOpen(true); setMessage(`编辑 ${id}，请确认保存范围。`);
  };
  const rows = [
  ...
  return <section className="panel full"><div className="panel-header"><div><h3>项目级策略覆盖与审计</h3><span>覆盖不改写随代码发布的基线规则，并在后续扫描快照中留痕</span></div><FeedbackButton className="secondary-action" disabled={saving} onClick={() => refresh()}>刷新</FeedbackButton></div>{message && <p role="status">{message}</p>}{error ? <div className="report-error">{error}</div> : null}<PagedTable className="compact-table"><thead><tr><th>策略</th><th>范围</th><th>来源</th><th>状态</th><th>操作</th></tr></thead><tbody>{rows.length ? rows.map((item) => <tr key={`${item.kind}-${item.id}`}><td><strong>{item.title}</strong><span className="cell-subtext">{item.detail}</span></td><td>当前项目</td><td>{item.source}</td><td>{item.enabled ? "已启用" : "已停用"}</td><td><FeedbackButton className="secondary-action" disabled={saving} onClick={() => edit(item.kind, item.id)}>编辑</FeedbackButton><FeedbackButton className="secondary-action" disabled={saving} onClick={() => toggle(item.kind, item.id, !item.enabled)}>{item.enabled ? "停用" : "启用"}</FeedbackButton></td></tr>) : <tr><td colSpan={5} className="empty-cell">暂无可管理策略。</td></tr>}</tbody></PagedTable><details className="advanced-details" open={editorOpen} onToggle={event => setEditorOpen(event.currentTarget.open)}><summary>新增或编辑策略</summary><FeedbackButton className="secondary-action" onClick={() => { setPolicyId(""); setPolicyConfig(""); setPolicyEnabled(true); setMessage("正在新增策略"); }}>新建策略</FeedbackButton><p>漏洞策略 JSON 需包含 ecosystem、package、affected、severity、summary、fixed_version；许可证策略需包含 keywords、policy、summary、remediation；门禁策略的 ID 固定为 default，可配置 block_severities、block_license_policies、min_risk_score、block_kev、max_scan_age_hours。编辑以覆盖方式保存，原始基线规则保留。选择“平台级”会应用于所有本地项目（项目覆盖仍优先）。</p><div className="filter-grid"><select value={scope} onChange={(event) => setScope(event.target.value as "project" | "platform")}><option value="project">当前项目</option><option value="platform">平台级</option></select><select value={policyKind} onChange={(event) => setPolicyKind(event.target.value as "vulnerability" | "license" | "gate")}><option value="vulnerability">漏洞规则</option><option value="license">许可证策略</option><option value="gate">CI 门禁策略</option></select><input value={policyId} onChange={(event) => setPolicyId(event.target.value)} placeholder={policyKind === "gate" ? "固定为 default" : "策略 ID，例如 CUSTOM-CVE-2026-001"} /><label className="inline-check"><input type="checkbox" checked={policyEnabled} onChange={event => setPolicyEnabled(event.target.checked)} />启用此策略</label><ScaPolicyFields kind={policyKind} value={policyConfig} onChange={setPolicyConfig} /><textarea aria-label="完整策略 JSON" value={policyConfig} onChange={(event) => setPolicyConfig(event.target.value)} placeholder={policyKind === "gate" ? '{"block_severities":["critical","high"],"min_risk_score":80,"block_kev":true,"max_scan_age_hours":168}' : '{"ecosystem":"npm","package":"example","affected":"<2.0.0","severity":"high","summary":"...","fixed_version":"2.0.0"}'} /><FeedbackButton className="secondary-action" disabled={saving || !policyId.trim() || !policyConfig.trim()} onClick={() => saveCustomPolicy()}>保存策略</FeedbackButton></div></details><details className="advanced-details"><summary>策略审计记录（最近 {audit.length} 条）</summary>{audit.length ? <PagedTable className="compact-table"><thead><tr><th>时间</th><th>事件</th><th>操作人</th><th>详情</th></tr></thead><tbody>{audit.map((item) => <tr key={item.id}><td>{formatDateTime(item.created_at)}</td><td>{item.event_type}</td><td>{item.actor ?? "-"}</td><td>{<PagedItems>{Object.entries(item.details).map(([key, value]) => <span className="cell-subtext" key={key}>{key}: {textValue(value)}</span>)}</PagedItems>}</td></tr>)}</tbody></PagedTable> : <div className="empty-project">暂无项目级策略变更记录。</div>}</details></section>;
}
...
function AgentGovernanceConsole({ project, snapshot }: { project: Project; snapshot: AgentScanSnapshot | null }) {
  ...
  const [savedProfile, setSavedProfile] = useState<AgentProfile | null>(null);
  ...
  const profileChanges = agentProfileChangeSummary(savedProfile, profile);
  ...
  async function load() {
    ...
    setProfile(nextProfile); setSavedProfile(nextProfile); setGate(nextGate); setLastSavedAt(null);
  }
  ...
  async function saveProfile(nextProfile = profile) {
    ...
    try {
      ...
      const saved = await request<AgentProfile>(`/agent/projects/${project.id}/profile`, { method: "PATCH", body: JSON.stringify(payload) });
      const savedAt = new Date().toISOString();
      setProfile(saved); setSavedProfile(saved); setLastSavedAt(savedAt);
      setMessage(`治理策略已于 ${formatDateTime(savedAt)} 保存；扫描与门禁规则从下一次 AGENT 扫描生效，真实目标执行入口开关立即生效但仍需独立确认。`);
  ...
  async function downloadAgentArtifact(kind: "json" | "html" | "sarif" | "ci") {
    ...
    try {
      const response = await feedbackFetch(`${API_BASE}/agent/projects/${project.id}/${endpoint}`, { credentials: "include" });
      if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
  ...
  return <section className="agent-governance-console"><div className="agent-governance-summary"><div><span>当前治理配置</span><strong>v{profile.profile_version}</strong><small>规则 {profile.rule_version}</small></div><div><span>质量门禁</span><strong>{gate?.decision === "block" ? "阻断" : gate?.decision === "pass" ? "通过" : "等待扫描"}</strong><small>{agentUiText(gate?.reasons?.[0] ?? "修改后需重新扫描裁决")}</small></div><div><span>项目例外</span><strong>{profile.exceptions.length}</strong><small>{profile.exceptions.filter((item) => item.status === "pending").length} 条待审批</small></div><div><span>权限 Allowlist</span><strong>{profile.permission_allowlist.length}</strong><small>仅影响治理裁决</small></div></div>{profileDirty ? <div className="agent-unsaved-banner" role="status"><div><strong>有 {profileChanges.length} 类策略修改尚未保存</strong><span>{profileChanges.join("、")}</span></div><span>扫描规则保存后从下一次扫描生效；真实执行入口开关保存后立即生效。</span></div> : <div className="agent-saved-state"><span>当前没有未保存修改</span><small>{lastSavedAt ? `最近保存：${formatDateTime(lastSavedAt)}` : "页面中的配置与服务器已保存版本一致"}</small></div>}<nav className="agent-governance-nav" aria-label="策略与交付工作区"><FeedbackButton type="button" className={workspace === "policy" ? "active" : ""} onClick={() => setWorkspace("policy")}>扫描与门禁</FeedbackButton><FeedbackButton type="button" className={workspace === "exceptions" ? "active" : ""} onClick={() => setWorkspace("exceptions")}>例外与边界</FeedbackButton><FeedbackButton type="button" className={workspace === "delivery" ? "active" : ""} onClick={() => setWorkspace("delivery")}>报告与审计</FeedbackButton></nav><section className="content-grid agent-governance-panels">
    {workspace === "policy" ? <>
    <div className="panel full"><div className="panel-header"><h2>项目扫描策略</h2><span>{message || "修改后需显式保存"}</span></div><p>历史扫描与原始 Finding 不会被重写。操作人用于本项目策略审计，不等同于平台身份认证。</p><div className="filter-grid agent-form-grid"><label>操作人<input value={actor} onChange={(event) => setActor(event.target.value)} /></label><label>停用规则 ID（每行一个）<textarea rows={4} value={profile.disabled_rule_ids.join("\n")} onChange={(event) => setProfile({ ...profile, disabled_rule_ids: splitLines(event.target.value) })} /></label><label>排除路径 glob（每行一个）<textarea rows={4} value={profile.excluded_paths.join("\n")} onChange={(event) => setProfile({ ...profile, excluded_paths: splitLines(event.target.value) })} placeholder="例如 fixtures/**" /></label><label>强制审批能力（每行一个）<textarea rows={4} value={profile.required_approval_capabilities.join("\n")} onChange={(event) => setProfile({ ...profile, required_approval_capabilities: splitLines(event.target.value) })} /></label><label className="inline-check agent-full-row"><input type="checkbox" checked={profile.target_runtime_execution_enabled} onChange={(event) => setProfile({ ...profile, target_runtime_execution_enabled: event.target.checked })} />允许本项目显示真实目标执行入口；仍需精确 staging、固定镜像和二次确认</label><div className="agent-form-actions"><FeedbackButton className="primary-action" disabled={!profileDirty} onClick={() => saveProfile()}>{profileDirty ? "保存项目策略" : "已保存"}</FeedbackButton></div></div></div>
    <div className="panel full">
      ...
      <div className="filter-grid agent-form-grid agent-gate-form">
        ...
        <div className="agent-form-actions"><FeedbackButton className="primary-action" disabled={!profileDirty} onClick={() => saveProfile()}>{profileDirty ? "保存门禁与策略" : "已保存"}</FeedbackButton></div>
      </div>
      ...
      {gate?.reasons?.length ? <ul>{<PagedItems>{gate.reasons.map((reason) => <li key={reason}>{agentUiText(reason)}</li>)}</PagedItems>}</ul> : <p>最近扫描没有门禁阻断原因；修改策略后需重新扫描才能重新裁决。</p>}
    </div>
    </> : null}
    {workspace === "exceptions" ? <>
    <div className="panel full"><div className="panel-header"><h2>权限 Allowlist</h2><span>{profile.permission_allowlist.length} 条项目级边界</span></div><div className="filter-grid agent-form-grid"><label>资产路径 glob<input value={allowlist.path_pattern} onChange={(event) => setAllowlist({ ...allowlist, path_pattern: event.target.value })} /></label><label>主体 glob<input value={allowlist.subject_pattern} onChange={(event) => setAllowlist({ ...allowlist, subject_pattern: event.target.value })} /></label><label>能力<input value={allowlist.capability} onChange={(event) => setAllowlist({ ...allowlist, capability: event.target.value })} /></label><label>范围 glob<input value={allowlist.scope_pattern} onChange={(event) => setAllowlist({ ...allowlist, scope_pattern: event.target.value })} /></label><label className="agent-full-row">治理理由<input value={allowlist.reason} onChange={(event) => setAllowlist({ ...allowlist, reason: event.target.value })} /></label><div className="agent-form-actions"><FeedbackButton className="primary-action" onClick={addAllowlist}>新增并保存</FeedbackButton></div></div>{profile.permission_allowlist.length ? <PagedTable className="compact-table"><thead><tr><th>资产 / 主体</th><th>能力 / 范围</th><th>理由</th><th>操作</th></tr></thead><tbody>{profile.permission_allowlist.map((item, index) => <tr key={item.id ?? index}><td>{item.path_pattern}<span className="cell-subtext">{item.subject_pattern}</span></td><td>{item.capability}<span className="cell-subtext">{item.scope_pattern}</span></td><td>{item.reason}</td><td><FeedbackButton className="secondary-action" onClick={() => { const next = { ...profile, permission_allowlist: profile.permission_allowlist.filter((_, itemIndex) => itemIndex !== index) }; setProfile(next); void saveProfile(next); }}>移除</FeedbackButton></td></tr>)}</tbody></PagedTable> : <div className="empty-project">暂无 Allowlist；新权限与高风险权限按门禁策略裁决。</div>}</div>
    <div className="panel full"><div className="panel-header"><h2>Finding / 权限例外审批</h2><span>申请与批准分离记录；批准后下一次扫描生效</span></div><div className="filter-grid agent-form-grid"><label>对象<select value={exception.kind} onChange={(event) => setException({ ...exception, kind: event.target.value as "finding" | "permission" })}><option value="finding">Finding</option><option value="permission">权限</option></select></label><label>处置<select value={exception.disposition} onChange={(event) => setException({ ...exception, disposition: event.target.value as "suppress" | "accept_risk" })}><option value="accept_risk">接受风险</option><option value="suppress">抑制 / 误报</option></select></label><label>规则 ID<input value={exception.rule_id} disabled={exception.kind !== "finding"} onChange={(event) => setException({ ...exception, rule_id: event.target.value })} /></label><label>资产路径 glob<input value={exception.path_pattern} onChange={(event) => setException({ ...exception, path_pattern: event.target.value })} /></label><label>主体 glob<input value={exception.subject_pattern} disabled={exception.kind !== "permission"} onChange={(event) => setException({ ...exception, subject_pattern: event.target.value })} /></label><label>能力<input value={exception.capability} disabled={exception.kind !== "permission"} onChange={(event) => setException({ ...exception, capability: event.target.value })} /></label><label>范围 glob<input value={exception.scope_pattern} disabled={exception.kind !== "permission"} onChange={(event) => setException({ ...exception, scope_pattern: event.target.value })} /></label><label>失效日期<input type="date" value={exception.expires_at} onChange={(event) => setException({ ...exception, expires_at: event.target.value })} /></label><label className="agent-full-row">申请理由<input value={exception.reason} onChange={(event) => setException({ ...exception, reason: event.target.value })} /></label><div className="agent-form-actions"><FeedbackButton className="primary-action" onClick={() => createException()}>提交例外申请</FeedbackButton></div></div><label className="agent-approval-note">审批说明<input value={decisionNote} onChange={(event) => setDecisionNote(event.target.value)} placeholder="批准、拒绝或撤销前必填" /></label>{profile.exceptions.length ? <PagedTable className="compact-table"><thead><tr><th>对象 / 选择器</th><th>理由 / 有效期</th><th>状态 / 审批</th><th>操作</th></tr></thead><tbody>{profile.exceptions.map((item) => <tr key={item.id}><td>{item.kind === "finding" ? item.rule_id : item.capability}<span className="cell-subtext">{item.path_pattern}</span></td><td>{item.reason}<span className="cell-subtext">{item.expires_at ? formatDateTime(item.expires_at) : "永久"}</span></td><td>{item.status}<span className="cell-subtext">申请：{item.requester ?? "-"} · 审批：{item.approver ?? "-"}</span></td><td>{item.status === "pending" ? <><FeedbackButton className="secondary-action" onClick={() => decideException(item.id, "approved")}>批准</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => decideException(item.id, "rejected")}>拒绝</FeedbackButton></> : item.status === "approved" ? <FeedbackButton className="secondary-action" onClick={() => decideException(item.id, "revoked")}>撤销</FeedbackButton> : "-"}</td></tr>)}</tbody></PagedTable> : <div className="empty-project">暂无例外申请。</div>}</div>
    </> : null}
    {workspace === "delivery" ? <div className="panel full"><div className="panel-header"><h2>报告、离线 CI 与策略审计</h2><span>JSON / SARIF / HTML 均来自同一扫描快照</span></div><div className="probe-actions"><FeedbackButton className="secondary-action" disabled={!snapshot} onClick={() => downloadAgentArtifact("json")}>导出 JSON</FeedbackButton><FeedbackButton className="secondary-action" disabled={!snapshot} onClick={() => downloadAgentArtifact("sarif")}>导出 SARIF</FeedbackButton><FeedbackButton className="secondary-action" disabled={!snapshot} onClick={() => downloadAgentArtifact("html")}>导出 HTML</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => downloadAgentArtifact("ci")}>导出离线 CI 配置</FeedbackButton></div><p>CI 命令只做本地静态解析；不下载资源、不联网，也不运行目标 Agent/MCP/插件。使用基线报告时可只阻断新增 Finding 和权限扩大。</p>{profile.audit_log.length ? <details><summary>最近 {profile.audit_log.length} 条策略审计</summary><PagedTable className="compact-table"><thead><tr><th>时间</th><th>事件</th><th>操作人</th><th>详情</th></tr></thead><tbody>{profile.audit_log.slice().reverse().map((item) => <tr key={item.id}><td>{formatDateTime(item.at)}</td><td>{item.action}</td><td>{item.actor}</td><td>{<PagedItems>{Object.entries(item.detail ?? {}).map(([key, value]) => <span className="cell-subtext" key={key}>{key}: {textValue(value)}</span>)}</PagedItems>}</td></tr>)}</tbody></PagedTable></details> : <div className="empty-project">暂无策略变更记录。</div>}</div> : null}
  </section></section>;
}
...
function AgentDataflowPanel({ snapshot }: { snapshot: AgentScanSnapshot | null }) {
  ...
  return <section className="retest-panel">
    ...
    <table className="concise-table mobile-card-table"><thead><tr><th>等级 / 置信度</th><th>路径</th><th>能力与资源</th><th>控制状态</th></tr></thead><tbody>{pagination.items.length === 0 ? <tr><td colSpan={4} className="empty-cell">当前静态声明没有形成可展示的 Prompt→工具→资源路径；这不代表运行时一定不存在数据流。</td></tr> : pagination.items.map((item) => {
      const sequence = item.node_ids.map((id) => nodes.get(id)?.label).filter(Boolean).join(" → ");
      return <tr key={item.id}><td data-label="等级 / 置信度"><span className={`severity ${item.severity}`}>{severityLabel(item.severity)}</span><span className="cell-subtext">{agentDataflowConfidenceLabel(item.confidence)}</span><span className="cell-subtext">{item.source_trust === "adversarial-signal" ? "存在可疑指令信号" : "输入信任状态未知"}</span></td><td data-label="路径"><strong>{agentUiText(item.title)}</strong><span className="cell-subtext">{sequence || item.id}</span><span className="cell-subtext">Prompt 资产：{item.asset_path}</span>{item.tool_asset_path ? <span className="cell-subtext">工具资产：{item.tool_asset_path}</span> : null}<details className="record-evidence"><summary>查看路径证据</summary><ul>{<PagedItems>{item.evidence.map((value) => <li key={value}>{agentUiText(value)}</li>)}</PagedItems>}</ul></details></td><td data-label="能力与资源"><strong>{agentCapabilityLabel(item.capability)}</strong><span className="cell-subtext">{item.resource_type}: {item.resource_scope}</span><span className="cell-subtext">审批声明：{agentApprovalLabel(item.approval)}</span></td><td data-label="控制状态">{item.controls.length ? <PagedItems>{item.controls.map((control, index) => <span className="cell-subtext" key={`${control.type}-${index}`}>已有：{agentDataflowControlLabel(control.type)}{control.runtime_verified === false ? "（未验证执行）" : ""}</span>)}</PagedItems> : <span className="cell-subtext">未识别到已声明控制</span>}{<PagedItems>{item.missing_controls.map((control) => <span className="cell-subtext" key={control}>缺少：{agentDataflowControlLabel(control)}</span>)}</PagedItems>}</td></tr>;
    })}</tbody></table>
    <Pagination page={pagination.page} pageCount={pagination.pageCount} total={paths.length} onPageChange={setPage} />
    {(dataflow.limitations ?? []).length ? <details className="advanced-details"><summary>查看模型边界</summary><ul>{<PagedItems>{dataflow.limitations.map((item) => <li key={item}>{agentUiText(item)}</li>)}</PagedItems>}</ul></details> : null}
  </section>;
}

function AgentOfflineAuditPanel({ snapshot, findings }: { snapshot: AgentScanSnapshot | null; findings: Finding[] }) {
  const audit = agentSnapshotSection<AgentOfflineAudit>(snapshot?.audit, "ai-security-platform.agent-offline-audit/v1");
  if (!audit) return <section className="retest-panel"><div className="panel-header"><h3>扫描提示与复核边界</h3><span>等待新扫描</span></div><p>完成一次新版本 AGENT 扫描后，这里会显示不属于正式问题的覆盖缺口与专项复核提示。</p></section>;
  ...
  const visibleItems = visibleAgentAuditItems(audit.items, findings);
  const advisories = visibleItems.filter((item) => item.kind !== "finding");
  const activeFindingCount = findings.filter((item) => !["accepted_risk", "false_positive", "fixed", "closed"].includes(item.status)).length;
  return <section className="retest-panel">
    <div className="panel-header"><h3>扫描提示与复核边界</h3><span className="severity info">{activeFindingCount} 个正式问题 · {advisories.length} 项额外提示</span></div>
    <p className="agent-time-note">审计草案生成：{formatDateTime(audit.generated_at)}</p>
    <div className="retest-summary"><Metric label="正式问题" value={activeFindingCount} /><Metric label="额外扫描提示" value={advisories.length} /><Metric label="外部模型" value={audit.external_model_invoked ? "已调用" : "未调用"} /><Metric label="关联信任分" value={`${audit.summary.trust_score} / 100`} /></div>
    <p className="retest-note">正式问题只以上方风险表为准。这里不重复列出已经生成 Finding 的静态路径，只补充覆盖缺口、私有来源预检等非问题提示；提示不会改变问题数、治理状态、质量门禁或信任评分。</p>
    {advisories.length ? <div className="agent-advisory-list">{<PagedItems>{advisories.map((item) => <article key={item.id}><div><span className={`severity ${item.priority === "critical" || item.priority === "high" ? "high" : item.priority === "medium" ? "medium" : "info"}`}>{priorityLabel[item.priority] ?? item.priority}</span><strong>{kindLabel[item.kind] ?? item.kind}</strong></div><h4>{agentUiText(item.title)}</h4><p>{agentUiText(item.rationale)}</p><details><summary>查看证据引用</summary>{item.evidence_refs.map((value) => <code key={value}>{value}</code>)}</details></article>)}</PagedItems>}</div> : <div className="agent-empty-state">当前没有独立于正式问题之外的扫描提示。</div>}
    <details className="advanced-details"><summary>查看审计边界与哈希</summary><div className="advanced-details-body"><p>模式：本地确定性静态复核；模型状态：未调用；审计 SHA-256：<code>{audit.audit_sha256}</code></p><ul>{<PagedItems>{audit.limitations.map((item) => <li key={item}>{agentUiText(item)}</li>)}</PagedItems>}</ul></div></details>
  </section>;
}

function AgentDeepSeekReviewPanel({ snapshot, findings, loading, onRun }: { snapshot: AgentScanSnapshot | null; findings: Finding[]; loading: boolean; onRun: (confirmationPhrase: string) => Promise<void> }) {
  const audit = agentSnapshotSection<AgentOfflineAudit>(snapshot?.audit, "ai-security-platform.agent-offline-audit/v1");
  const review = agentSnapshotSection<AgentAiReview>(snapshot?.ai_review, "ai-security-platform.agent-ai-review/v1");
  const visibleCandidateCount = audit ? visibleAgentAuditItems(audit.items, findings).length : 0;
  ...
  return <section className="retest-panel">
    ...
    <div className="retest-summary"><Metric label="当前可见候选" value={visibleCandidateCount} /><Metric label="输入候选上限" value="25" /><Metric label="源码 / Prompt" value="不发送" /><Metric label="最大估算费用" value="$0.02" /></div>
    <p className="retest-note">模型只生成待人工复核建议，不能改变 Finding、质量门禁、信任评分或代码。每次调用均需在此页面二次确认；失败时离线审计草案不会被覆盖。</p>
    {!review ? <FeedbackButton className="primary-action" disabled={loading} onClick={startReview}>{loading ? "审计中" : "执行 DeepSeek 审计（需二次确认）"}</FeedbackButton> : legacyEnglishReview ? <><div className="retest-summary"><Metric label="模型" value={review.model} /><Metric label="历史建议" value={review.reviews.length} /><Metric label="耗时" value={`${review.usage.latency_ms} ms`} /><Metric label="估算费用" value={review.usage.estimated_cost_usd === null ? "未知" : `$${review.usage.estimated_cost_usd}`} /></div><div className="agent-localization-notice"><strong>当前批次保存的是旧版英文模型建议</strong><p>为避免机器翻译改变安全语义，页面不直接展示英文原文。重新执行审计后，模型将按简体中文输出；原始历史证据仍保留在扫描快照中。</p><FeedbackButton className="secondary-action" disabled={loading} onClick={startReview}>{loading ? "审计中" : "重新生成中文建议"}</FeedbackButton></div></> : <><div className="retest-summary"><Metric label="模型" value={review.model} /><Metric label="建议数量" value={review.reviews.length} /><Metric label="耗时" value={`${review.usage.latency_ms} ms`} /><Metric label="估算费用" value={review.usage.estimated_cost_usd === null ? "未知" : `$${review.usage.estimated_cost_usd}`} /></div><p className="retest-note">{review.summary || "模型未提供摘要。"}</p><PagedTable className="compact-table"><thead><tr><th>候选 ID</th><th>建议优先级</th><th>人工复核理由</th><th>建议动作</th></tr></thead><tbody>{review.reviews.length ? review.reviews.map((item) => <tr key={item.audit_item_id}><td>{item.audit_item_id}</td><td><span className={`severity ${item.review_priority === "critical" || item.review_priority === "high" ? "high" : item.review_priority === "medium" ? "medium" : "info"}`}>{severityLabel(item.review_priority as Severity)}</span></td><td>{item.rationale}<span className="cell-subtext">{item.review_questions.join("；")}</span></td><td>{<PagedItems>{item.recommended_actions.map((action) => <span className="cell-subtext" key={action}>{action}</span>)}</PagedItems>}</td></tr>) : <tr><td colSpan={4}>模型未保留可用建议；本地草案仍可人工复核。</td></tr>}</tbody></PagedTable><details className="advanced-details"><summary>查看模型审计边界</summary><p>输入 SHA-256：<code>{review.input_sha256}</code></p><ul>{<PagedItems>{review.limitations.map((item) => <li key={item}>{agentUiText(item)}</li>)}</PagedItems>}</ul></details></>}
  </section>;
}
...
function AgentTrustScorePanel({ snapshot }: { snapshot: AgentScanSnapshot | null }) {
  ...
  return <section className="retest-panel">
    ...
    <PagedTable className="compact-table mobile-card-table"><thead><tr><th>分项</th><th>得分</th><th>证据状态</th><th>扣分依据</th></tr></thead><tbody>{trust.dimensions.map((item) => <tr key={item.id}><td data-label="分项"><strong>{item.label}</strong></td><td data-label="得分">{item.score} / {item.max_score}</td><td data-label="证据状态">{statusLabels[item.status] ?? item.status}</td><td data-label="扣分依据">{item.deductions.length ? <PagedItems>{item.deductions.map((value) => <span className="cell-subtext" key={value.id}>-{value.points}：{value.detail}{value.count > 1 ? `（${value.count} 项）` : ""}</span>)}</PagedItems> : <span className="cell-subtext">本分项没有技术扣分</span>}</td></tr>)}</tbody></PagedTable>
    {trust.improvements.length ? <div><strong>优先改进建议</strong><ol>{<PagedItems>{trust.improvements.map((item) => <li key={item.id}><strong>{item.title}</strong>：{item.action}</li>)}</PagedItems>}</ol></div> : <p>当前没有生成额外改进建议。</p>}
    <details className="advanced-details"><summary>查看评分上限、限制与证据哈希</summary>{trust.score_caps.length ? <ul>{<PagedItems>{trust.score_caps.map((item) => <li key={item.id}>{agentUiText(item.detail)}</li>)}</PagedItems>}</ul> : <p>当前没有额外评分上限。</p>}<ul>{<PagedItems>{trust.limitations.map((item) => <li key={item}>{agentUiText(item)}</li>)}</PagedItems>}</ul><p>算法：{trust.algorithm_version}；证据摘要 SHA-256：{trust.trust_sha256}</p></details>
  </section>;
}
...
function AgentRuntimePreflightPanel({ project, snapshot }: { project: Project; snapshot: AgentScanSnapshot | null }) {
  ...
  const [runtimeStep, setRuntimeStep] = useState<1 | 2 | 3 | 4 | 5>(1);
  const [runtimeMethod, setRuntimeMethod] = useState<"stdio" | "remote" | "target" | null>(null);
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState("");
  useEffect(() => {
  ...
  async function runPreflight() {
    ...
    try {
      ...
      if (!next.checks.some((item) => item.status === "block")) setRuntimeStep(2);
  ...
  const verifiedBuildCount = targetStatus?.builds.length ?? 0;
  const hasRuntimeEvidence = Boolean(fixtureEvidence || mcpProbeEvidence || remoteMcpProbeEvidence || targetEvidence);
  const runtimeSteps: Array<[1 | 2 | 3 | 4 | 5, string, string]> = [[1, "运行条件", "只做预检"], [2, "安全副本", "过滤与校验"], [3, "验证方式", "选择范围"], [4, "确认执行", "精确授权"], [5, "查看证据", "结果与边界"]];
  return <section className="retest-panel">
    ...
    <nav className="agent-runtime-steps" aria-label="动态验证步骤">{runtimeSteps.map(([step, label, description]) => { const disabled = step === 2 ? !exactPlanConfirmed : step === 3 ? verifiedBuildCount === 0 && !stagingResult : step === 4 ? !runtimeMethod : step === 5 ? !hasRuntimeEvidence : false; return <FeedbackButton type="button" key={step} className={runtimeStep === step ? "active" : runtimeStep > step ? "completed" : ""} disabled={disabled} onClick={() => setRuntimeStep(step)}><i>{runtimeStep > step ? "✓" : step}</i><span><strong>{label}</strong><small>{description}</small></span></FeedbackButton>; })}</nav>
    {runtimeStep === 1 ? <>
    ...
    <div className="filter-grid"><label>拟执行命令<input value={command} onChange={(event) => { setCommand(event.target.value); setConfirmed(false); setStagingConfirmed(false); }} placeholder="必须由操作人明确选择，不自动推断" /></label><label>本地镜像（必须固定 digest）<input value={image} onChange={(event) => { setImage(event.target.value); setConfirmed(false); setStagingConfirmed(false); }} placeholder="name@sha256:...；预检不会下载" /></label><label>超时秒数<input type="number" min={1} max={30} value={timeoutSeconds} onChange={(event) => { setTimeoutSeconds(Math.max(1, Math.min(30, Number(event.target.value)))); setConfirmed(false); setStagingConfirmed(false); }} /></label><label className="inline-check"><input type="checkbox" checked={confirmed} onChange={(event) => { setConfirmed(event.target.checked); setStagingConfirmed(false); }} />我只确认预检这组命令、镜像和目标；不授权执行</label><FeedbackButton className="primary-action" disabled={loading} onClick={() => runPreflight()}>{loading ? "预检中" : "只执行安全预检"}</FeedbackButton></div>
    {message ? <p className="agent-runtime-message">{message}</p> : null}
    <PagedTable className="compact-table mobile-card-table"><thead><tr><th>状态</th><th>检查</th><th>结果</th><th>处理建议</th></tr></thead><tbody>{plan.checks.map((item) => <tr key={item.id}><td data-label="状态"><span className={`severity ${item.status === "block" ? "high" : item.status === "warn" ? "medium" : "info"}`}>{item.status === "pass" ? "通过" : item.status === "warn" ? "警告" : "阻断"}</span></td><td data-label="检查">{agentCheckLabel(item.id)}</td><td data-label="结果">{agentUiText(item.detail)}</td><td data-label="处理建议">{item.remediation ? agentUiText(item.remediation) : "-"}</td></tr>)}</tbody></PagedTable>
    {plan.candidate_dataflow_paths.length ? <details className="advanced-details"><summary>查看计划验证的 {plan.candidate_dataflow_paths.length} 条静态路径</summary><PagedTable className="compact-table mobile-card-table"><thead><tr><th>风险</th><th>路径</th><th>能力 / 资源</th></tr></thead><tbody>{plan.candidate_dataflow_paths.map((item) => <tr key={item.id}><td data-label="风险"><span className={`severity ${item.severity}`}>{severityLabel(item.severity)}</span><span className="cell-subtext">{agentDataflowConfidenceLabel(item.confidence)}</span></td><td data-label="路径">{agentUiText(item.title)}<span className="cell-subtext">{item.asset_path}{item.tool_asset_path ? ` → ${item.tool_asset_path}` : ""}</span></td><td data-label="能力 / 资源">{agentCapabilityLabel(item.capability)}<span className="cell-subtext">{item.resource_type}: {item.resource_scope}</span></td></tr>)}</tbody></PagedTable></details> : null}
    <div className="agent-step-actions"><FeedbackButton type="button" className="primary-action" disabled={!exactPlanConfirmed} onClick={() => setRuntimeStep(2)}>下一步：创建安全副本</FeedbackButton>{!exactPlanConfirmed ? <span>完成全部阻断项并重新预检后才能继续。</span> : null}</div>
    </> : null}
    {runtimeStep === 2 ? <>
    {message ? <p className="agent-runtime-message">{message}</p> : null}
    <div className="filter-grid"><label className="inline-check"><input type="checkbox" disabled={!exactPlanConfirmed} checked={stagingConfirmed} onChange={(event) => setStagingConfirmed(event.target.checked)} />我确认创建本计划对应的 D 盘过滤副本；不授权运行 Agent 或容器</label><FeedbackButton className="secondary-action" disabled={loading || !stagingConfirmed || !exactPlanConfirmed} onClick={() => buildStaging()}>{loading ? "处理中" : "创建并校验过滤副本"}</FeedbackButton>{!exactPlanConfirmed ? <span>请先勾选上方目标确认并重新执行安全预检。</span> : null}</div>
    ...
    <div className="agent-step-actions"><FeedbackButton type="button" className="secondary-action" onClick={() => setRuntimeStep(1)}>返回运行条件</FeedbackButton><FeedbackButton type="button" className="primary-action" disabled={verifiedBuildCount === 0 && !stagingResult} onClick={() => setRuntimeStep(3)}>下一步：选择验证方式</FeedbackButton></div>
    </> : null}
    {runtimeStep === 3 ? <section className="agent-runtime-methods"><div className="agent-step-intro"><strong>选择本次验证范围</strong><span>三种方式互不自动升级；能力探测不会调用工具，真实 Agent 运行仍默认关闭。</span></div><div className="agent-method-grid"><FeedbackButton type="button" onClick={() => { setRuntimeMethod("stdio"); setRuntimeStep(4); }}><span>低风险</span><strong>stdio MCP 能力探测</strong><small>只盘点工具、资源和 Prompt 名称</small></FeedbackButton><FeedbackButton type="button" onClick={() => { setRuntimeMethod("remote"); setRuntimeStep(4); }}><span>受限联网</span><strong>远程 MCP 能力探测</strong><small>仅连接通过公网边界检查的 HTTPS 端点</small></FeedbackButton><FeedbackButton type="button" className="danger" onClick={() => { setRuntimeMethod("target"); setRuntimeStep(4); }}><span>高风险</span><strong>真实 Agent 隔离运行</strong><small>需要项目开关、精确绑定和再次确认</small></FeedbackButton></div><div className="agent-step-actions"><FeedbackButton type="button" className="secondary-action" onClick={() => setRuntimeStep(2)}>返回安全副本</FeedbackButton></div></section> : null}
    {runtimeStep === 4 && runtimeMethod === "stdio" ? <>
    ...
    <div className="agent-step-actions"><FeedbackButton type="button" className="secondary-action" onClick={() => setRuntimeStep(3)}>更换验证方式</FeedbackButton>{mcpProbeEvidence ? <FeedbackButton type="button" className="primary-action" onClick={() => setRuntimeStep(5)}>查看证据</FeedbackButton> : null}</div>
    </> : null}
    {runtimeStep === 4 && runtimeMethod === "remote" ? <>
    <details className="advanced-details" open><summary>从已验证配置安全探测远程 MCP（Streamable HTTP / SSE）</summary>
    ...
    <div className="agent-step-actions"><FeedbackButton type="button" className="secondary-action" onClick={() => setRuntimeStep(3)}>更换验证方式</FeedbackButton>{remoteMcpProbeEvidence ? <FeedbackButton type="button" className="primary-action" onClick={() => setRuntimeStep(5)}>查看证据</FeedbackButton> : null}</div>
    </> : null}
    {runtimeStep === 4 && runtimeMethod === "target" ? <>
    <details className="advanced-details" open><summary>指定项目 Agent 受控运行（高风险，默认关闭）</summary>
      ...
      {targetEvidence ? <><div className="kv-list"><div><span>最近目标证据</span><strong>{targetEvidence.decision === "pass" ? "策略通过" : "需要关注"}</strong><span>{formatDateTime(targetEvidence.started_at)} 至 {formatDateTime(targetEvidence.finished_at)} · 耗时 {targetEvidence.elapsed_ms} ms</span></div><div><span>退出 / 超时 / 清理</span><strong>{targetEvidence.container.exit_code ?? "无"} / {targetEvidence.container.timed_out ? "是" : "否"} / {targetEvidence.container.removed_after_run ? "已删除" : "失败"}</strong><span>工作区保持不变：{targetEvidence.staging.unchanged_after_run ? "是" : "否"}</span></div><div><span>证据 SHA-256</span><strong>{truncateText(targetEvidence.evidence_sha256, 20)}</strong><span>{targetEvidence.evidence_path}</span></div></div><details><summary>查看运行观测覆盖</summary><div className="kv-list">{<PagedItems>{Object.entries(targetEvidence.telemetry_coverage).map(([key, value]) => <div key={key}><span>{agentUiText(key)}</span><strong>{agentUiText(value)}</strong></div>)}</PagedItems>}</div><p>“未观察”不代表行为不可能发生；当前没有展示 Agent 标准输出，以降低敏感信息二次暴露风险。</p></details>{targetEvidence.mcp_ledger ? <details><summary>MCP stdio 调用账本（{Number(mcpSummary?.request_count ?? 0)} 次请求）</summary><div className="retest-summary"><Metric label="成功响应" value={Number(mcpSummary?.successful_response_count ?? 0)} /><Metric label="工具调用" value={Number(mcpSummary?.tool_call_count ?? 0)} /><Metric label="资源读取" value={Number(mcpSummary?.resource_read_count ?? 0)} /><Metric label="Prompt 获取" value={Number(mcpSummary?.prompt_get_count ?? 0)} /></div>{mcpResponses.length ? <PagedTable className="compact-table"><thead><tr><th>方法</th><th>目标</th><th>结果 / 耗时</th><th>事件哈希</th></tr></thead><tbody>{mcpResponses.map((item) => <tr key={item.event_id}><td>{item.method ?? "-"}</td><td>{item.subject_kind ?? "-"}: {item.subject ?? "-"}</td><td>{item.outcome ?? "-"} / {item.duration_ms ?? 0} ms</td><td>{truncateText(item.event_sha256, 16)}</td></tr>)}</tbody></PagedTable> : <p>本次没有收到可验证的 MCP 响应事件。</p>}<p>账本不保存 params、result 或标准输出内容。事件格式与哈希已校验，但日志通道不是密码学认证通道，恶意目标仍可能伪造同类记录。</p></details> : null}</> : null}
    </details>
    <div className="agent-step-actions"><FeedbackButton type="button" className="secondary-action" onClick={() => setRuntimeStep(3)}>更换验证方式</FeedbackButton>{targetEvidence ? <FeedbackButton type="button" className="primary-action" onClick={() => setRuntimeStep(5)}>查看证据</FeedbackButton> : null}</div>
    </> : null}
    {runtimeStep === 5 ? <section className="agent-runtime-evidence"><div className="agent-step-intro"><strong>最近验证证据</strong><span>证据按验证方式分开保存；没有证据的方式不会显示为已验证。</span></div><div className="agent-evidence-grid">{fixtureEvidence ? <article><span>无害夹具</span><strong>{fixtureEvidence.decision === "pass" ? "策略通过" : "存在阻断"}</strong><small>{formatDateTime(fixtureEvidence.started_at)} · {fixtureEvidence.elapsed_ms} ms</small><code>{truncateText(fixtureEvidence.evidence_sha256, 24)}</code></article> : null}{mcpProbeEvidence ? <article><span>stdio MCP 探测</span><strong>{agentUiText(mcpProbeEvidence.capability_probe.status)}</strong><small>工具 {mcpProbeEvidence.capability_probe.tool_names.length} · 资源 {mcpProbeEvidence.capability_probe.resource_schemes.length} · Prompt {mcpProbeEvidence.capability_probe.prompt_names.length}</small><code>{truncateText(mcpProbeEvidence.evidence_sha256, 24)}</code></article> : null}{remoteMcpProbeEvidence ? <article><span>远程 MCP 探测</span><strong>{agentUiText(remoteMcpProbeEvidence.capability_probe.status)}</strong><small>公网目标 {remoteMcpProbeEvidence.network_policy.approved_ips.length} · 未发送配置凭据</small><code>{truncateText(remoteMcpProbeEvidence.evidence_sha256, 24)}</code></article> : null}{targetEvidence ? <article><span>真实 Agent 隔离运行</span><strong>{targetEvidence.decision === "pass" ? "策略通过" : "需要关注"}</strong><small>{formatDateTime(targetEvidence.started_at)} 至 {formatDateTime(targetEvidence.finished_at)}</small><code>{truncateText(targetEvidence.evidence_sha256, 24)}</code></article> : null}</div>{!hasRuntimeEvidence ? <div className="agent-empty-state">尚未生成任何运行证据。</div> : null}<p>{agentUiText(plan.next_action)}</p><details className="advanced-details"><summary>查看运行证据边界</summary><p>当前可记录主进程、工作区前后完整性、适配 stdio 观察器的 MCP 方法调用与 MCP Server 子进程，以及单个远程 MCP endpoint 的受控能力清单和实际公网目标。逐文件访问、系统级子进程、整个 Agent 的透明网络目的地和认证后远程能力仍未插桩；每条静态路径继续区分“已观察”与“未插桩”，不会把“未观察”写成“不可利用”。</p><p>证据脱敏：{plan.evidence_template.redaction.applied ? "启用" : "未启用"}；保存密钥值：{plan.evidence_template.redaction.secret_values_stored ? "是" : "否"}。</p></details><div className="agent-step-actions"><FeedbackButton type="button" className="secondary-action" onClick={() => setRuntimeStep(3)}>继续选择验证方式</FeedbackButton></div></section> : null}
  </section>;
}
...
function AgentPermissionMatrixPanel({ snapshot }: { snapshot: AgentScanSnapshot | null }) {
  ...
  return <section className="retest-panel">
    ...
    <table className="concise-table mobile-card-table"><thead><tr><th>资产 / 主体</th><th>能力</th><th>资源范围</th><th>审批 / 风险</th></tr></thead><tbody>{pagination.items.length === 0 ? <tr><td className="empty-cell" colSpan={4}>当前资产没有可展示的权限声明。</td></tr> : pagination.items.map((permission) => <tr key={`${permission.asset_path}-${permission.subject}-${permission.source}-${permission.capability}-${permission.scope}`}><td data-label="资产 / 主体"><strong>{permission.subject}</strong><span className="cell-subtext">{permission.asset_path}</span><span className="cell-subtext">配置：{permission.source}</span></td><td data-label="能力"><strong>{agentCapabilityLabel(permission.capability)}</strong><span className="cell-subtext">{agentAccessLabel(permission.access)}</span></td><td data-label="资源范围"><strong>{permission.scope}</strong><span className="cell-subtext">{permission.resource_type}</span></td><td data-label="审批 / 风险"><span className={`severity ${permission.risk_level}`}>{severityLabel(permission.risk_level as Severity)}</span><span className="cell-subtext">{agentApprovalLabel(permission.approval)}</span></td></tr>)}</tbody></table>
    <Pagination page={pagination.page} pageCount={pagination.pageCount} total={filtered.length} onPageChange={setPage} />
  </section>;
}
...
function AgentOfflineAuditHistoryPanel({ diff, findings }: { diff: AgentOfflineAuditDiff | null; findings: Finding[] }) {
  ...
  if (!diff.has_comparison) return <section className="retest-panel"><div className="panel-header"><h3>离线审计草案批次对比</h3><span>暂不可比较</span></div><p>{diff.comparison_status === "base-audit-not-available" ? "上一扫描批次没有兼容的离线审计草案，因此当前候选不会被标记为新增。" : "当前扫描批次没有兼容的离线审计草案，因此未生成候选项对比。"}</p><details className="advanced-details"><summary>查看能力边界</summary><ul>{<PagedItems>{diff.limitations.map((item) => <li key={item}>{agentUiText(item)}</li>)}</PagedItems>}</ul></details></section>;
  const resultLabel: Record<string, string> = { new: "新增候选", "still-pending": "持续待人工复核", "not-current-candidate": "当前未再生成候选" };
  const kindLabel: Record<string, string> = { finding: "静态发现", "coverage-gap": "覆盖缺口", "private-source-preflight": "私有来源预检", "static-dataflow": "静态数据流" };
  const visibleItems = visibleAgentAuditItems(diff.items, findings);
  const visibleSummary = {
    newCount: visibleItems.filter((item) => item.result === "new").length,
    pendingCount: visibleItems.filter((item) => item.result === "still-pending").length,
    absentCount: visibleItems.filter((item) => item.result === "not-current-candidate").length,
  };
  return <section className="retest-panel">
    <div className="panel-header"><h3>审计任务批次变化</h3><span>{diff.base_scan_id?.slice(0, 8)} → {diff.target_scan_id.slice(0, 8)}</span></div>
    <div className="retest-summary"><Metric label="新增任务" value={visibleSummary.newCount} /><Metric label="持续待复核" value={visibleSummary.pendingCount} /><Metric label="当前未再出现" value={visibleSummary.absentCount} /><Metric label="已合并重复路径" value={diff.items.length - visibleItems.length} /></div>
    <p className="retest-note">“当前未再生成候选”仅表示最新静态证据未产生同一候选项，不表示风险已修复、系统安全、运行时行为不存在或不可利用；候选内容或证据引用变化会显示为新增与旧候选不再出现，必须人工判读。</p>
    <details className="advanced-details"><summary>查看 {visibleItems.length} 条审计任务变化</summary><PagedTable className="compact-table mobile-card-table"><thead><tr><th>对比状态</th><th>优先级 / 类型</th><th>审计任务</th><th>证据引用</th></tr></thead><tbody>{visibleItems.map((item) => <tr key={`${item.result}-${item.id}`}><td data-label="对比状态"><span className={`severity ${item.result === "new" ? "high" : item.result === "still-pending" ? "medium" : "info"}`}>{resultLabel[item.result] ?? item.result}</span></td><td data-label="优先级 / 类型"><span className={`severity ${item.priority === "critical" || item.priority === "high" ? "high" : item.priority === "medium" ? "medium" : "info"}`}>{severityLabel(item.priority as Severity)}</span><span className="cell-subtext">{kindLabel[item.kind] ?? item.kind}</span></td><td data-label="审计任务">{agentUiText(item.title)}</td><td data-label="证据引用">{<PagedItems>{item.evidence_refs.map((value) => <span className="cell-subtext" key={value}>{value}</span>)}</PagedItems>}</td></tr>)}</tbody></PagedTable></details>
    <details className="advanced-details"><summary>查看能力边界</summary><ul>{<PagedItems>{diff.limitations.map((item) => <li key={item}>{agentUiText(item)}</li>)}</PagedItems>}</ul></details>
  </section>;
}
...
function AgentWorkspaceNavigation({ active, onChange }: { active: AgentWorkspaceTab; onChange: (value: AgentWorkspaceTab) => void }) {
  ...
  return <nav className="agent-workspace-tabs" aria-label="AGENT 工作区">{items.map(([key, label, description]) => <FeedbackButton type="button" className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined} onClick={() => onChange(key)} key={key}><strong>{label}</strong><span>{description}</span></FeedbackButton>)}</nav>;
}
...
function AgentAssetWorkspaceNavigation({ active, onChange }: { active: AgentAssetWorkspaceTab; onChange: (value: AgentAssetWorkspaceTab) => void }) {
  return <div className="agent-segmented-tabs" role="tablist" aria-label="AGENT 资产视图">
    <FeedbackButton type="button" role="tab" aria-selected={active === "coverage"} className={active === "coverage" ? "active" : ""} onClick={() => onChange("coverage")}>扫描覆盖</FeedbackButton>
    <FeedbackButton type="button" role="tab" aria-selected={active === "inventory"} className={active === "inventory" ? "active" : ""} onClick={() => onChange("inventory")}>资产与权限</FeedbackButton>
    <FeedbackButton type="button" role="tab" aria-selected={active === "paths"} className={active === "paths" ? "active" : ""} onClick={() => onChange("paths")}>风险路径</FeedbackButton>
  </div>;
}
...
function AgentOverviewWorkspace({ findings, history, snapshot, onOpenTab }: { findings: Finding[]; history: AgentScanHistoryItem[]; snapshot: AgentScanSnapshot | null; onOpenTab: (value: AgentWorkspaceTab) => void }) {
  ...
  const priorityFindings = [...findings].sort((left, right) => severityRank(right.severity) - severityRank(left.severity));
  ...
  return <section className="agent-overview-workspace">
    <div className="agent-overview-grid">
      <article className="agent-trust-hero"><div className="agent-trust-score"><strong>{trust?.score ?? "-"}</strong><span>/ {trust?.score_cap ?? 100}</span></div><div><span>可解释信任评分</span><h3>{trust ? agentTrustGradeLabel(trust.grade) : "等待新扫描"}</h3><p>{trust ? `证据完整度 ${trust.evidence_completeness}% · ${agentConfidenceLabel(trust.confidence)}置信度` : "完成扫描后，根据来源、权限、路径和运行证据形成评分。"}</p></div><FeedbackButton type="button" className="secondary-action" onClick={() => onOpenTab("assets")}>查看评分依据</FeedbackButton></article>
      <article className={`agent-gate-card ${gate?.decision === "block" ? "blocked" : "ready"}`}><span>质量门禁</span><strong>{gate?.decision === "block" ? "当前阻断" : gate?.decision === "pass" ? "当前通过" : "等待裁决"}</strong><p>{agentUiText(gate?.reasons?.[0] ?? "门禁结论来自最近一次扫描快照。")}</p><FeedbackButton type="button" onClick={() => onOpenTab(isAdmin ? "governance" : "assets")}>{isAdmin ? "查看策略" : "查看依据"}</FeedbackButton></article>
    </div>
    <section className="agent-section-card"><div className="agent-section-heading"><div><span>扫描覆盖</span><h3>PPT 三类检查面，一眼确认覆盖情况</h3></div><FeedbackButton type="button" className="text-action" onClick={() => onOpenTab("assets")}>查看全部资产</FeedbackButton></div><div className="agent-coverage-cards"><article><i>01</i><span>指令与 Prompt</span><strong>{instructionAssets}</strong><small>指令文件、Prompt、Skill</small></article><article><i>02</i><span>MCP 与工具</span><strong>{protocolAssets}</strong><small>工具协议、能力与资源声明</small></article><article><i>03</i><span>插件与来源</span><strong>{pluginAssets}</strong><small>插件扩展、安装来源与完整性</small></article><article className={runtimeObserved ? "observed" : "conditional"}><i>04</i><span>运行证据</span><strong>{runtimeObserved ? "已观察" : "有条件"}</strong><small>{runtimeObserved ? "存在有限目标运行证据" : "需要独立预检与明确确认"}</small></article></div></section>
    <div className="agent-overview-columns">
      <section className="agent-section-card"><div className="agent-section-heading"><div><span>优先风险</span><h3>先处理最影响当前项目的事项</h3></div><FeedbackButton type="button" className="text-action" onClick={() => onOpenTab("risks")}>全部 {findings.length} 条</FeedbackButton></div>{priorityFindings.length ? <div className="agent-priority-list">{<PagedItems>{priorityFindings.map((finding) => <FeedbackButton type="button" onClick={() => onOpenTab("risks")} key={finding.id}><span className={`severity ${finding.severity}`}>{severityLabel(finding.severity)}</span><div><strong>{agentFindingDisplayTitle(finding)}</strong><small>{finding.file_path ?? "项目级问题"} · {agentCategoryLabel(finding.ai_review?.category ?? "unknown")}</small></div><ArrowRight size={16} /></FeedbackButton>)}</PagedItems>}</div> : <div className="agent-empty-state">当前批次没有风险 Finding。请继续确认资产覆盖和未验证边界。</div>}</section>
      <aside className="agent-next-action"><span>建议下一步</span><h3>{highPaths ? `先处理 ${highPaths} 条高风险静态路径` : priorityFindings.length ? "完成风险人工复核" : "确认覆盖边界"}</h3><p>{nextAction}</p><div><FeedbackButton type="button" className="primary-action" onClick={() => onOpenTab(highPaths ? "assets" : priorityFindings.length ? "risks" : "assets")}>开始处理</FeedbackButton><FeedbackButton type="button" className="secondary-action" onClick={() => onOpenTab("validation")}>动态验证</FeedbackButton></div><small>静态路径不是运行时事实；证据不足时保持未验证。</small></aside>
    </div>
  </section>;
}

type SastWorkspaceTab = "overview" | "risks" | "ai" | "strategy" | "delivery";
type SastStrategyTab = "semgrep" | "git" | "local" | "ci";

function SastWorkspaceNavigation({ active, onChange }: { active: SastWorkspaceTab; onChange: (value: SastWorkspaceTab) => void }) {
  const items: Array<[SastWorkspaceTab, string, string]> = [
    ["overview", "概览", "引擎与下一步"],
    ["risks", "代码风险", "发现与复测"],
    ["ai", "AI 辅助复核", "可选增强"],
    ["strategy", "扫描策略", "Semgrep、Git 与规则"],
    ["delivery", "例外与报告", "豁免、历史与交付"],
  ];
  return <nav className="module-workspace-tabs" aria-label="SAST 工作区">{items.map(([key, label, description]) => <FeedbackButton type="button" className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined} onClick={() => onChange(key)} key={key}><strong>{label}</strong><span>{description}</span></FeedbackButton>)}</nav>;
}

function SastOverviewWorkspace({ project, findings, onOpenTab }: { project: Project; findings: Finding[]; onOpenTab: (value: SastWorkspaceTab) => void }) {
  const high = findings.filter((item) => item.severity === "critical" || item.severity === "high").length;
  const reviewed = findings.filter((item) => Boolean(item.ai_review)).length;
  const open = findings.filter((item) => ["open", "pending", "confirmed"].includes(item.status)).length;
  return <section className="module-overview-workspace">
    <div className="module-overview-cards">
      <FeedbackButton type="button" onClick={() => onOpenTab("risks")}><span>待处理代码风险</span><strong>{open}</strong><small>{high} 条严重或高危</small><ArrowRight size={17} /></FeedbackButton>
      <FeedbackButton type="button" onClick={() => onOpenTab("ai")}><span>AI 辅助复核</span><strong>{reviewed}</strong><small>可选，不替代人工结论</small><ArrowRight size={17} /></FeedbackButton>
      <FeedbackButton type="button" onClick={() => onOpenTab("strategy")}><span>Semgrep 增强</span><strong>前端可配置</strong><small>补充本地规则覆盖</small><ArrowRight size={17} /></FeedbackButton>
      <FeedbackButton type="button" onClick={() => onOpenTab("delivery")}><span>例外与报告</span><strong>集中管理</strong><small>保留历史原始证据</small><ArrowRight size={17} /></FeedbackButton>
    </div>
...
function SastStrategyWorkspace({ project }: { project: Project }) {
  ...
  return <section className="module-tab-panel">
    ...
    <div className="module-segmented-tabs" role="tablist" aria-label="SAST 扫描策略分类">
      <FeedbackButton type="button" role="tab" aria-selected={tab === "semgrep"} className={tab === "semgrep" ? "active" : ""} onClick={() => setTab("semgrep")}>Semgrep 增强</FeedbackButton>
      <FeedbackButton type="button" role="tab" aria-selected={tab === "git"} className={tab === "git" ? "active" : ""} onClick={() => setTab("git")}>Git 增量</FeedbackButton>
      <FeedbackButton type="button" role="tab" aria-selected={tab === "local"} className={tab === "local" ? "active" : ""} onClick={() => setTab("local")}>本地规则</FeedbackButton>
      <FeedbackButton type="button" role="tab" aria-selected={tab === "ci"} className={tab === "ci" ? "active" : ""} onClick={() => setTab("ci")}>CI / Worker</FeedbackButton>
    </div>
...
function FindingModuleGovernance({ project, moduleKey, findings, validations, evidence, graph, comparison, scanHistory = [], agentSnapshot = null, agentScanDiff = null, agentAuditDiff = null, loading, onRunReview, onRunAgentAiReview, onRun, onUpdateFinding, afterMetrics }: { project?: Project; moduleKey: "sast" | "agent"; findings: Finding[]; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; comparison: FindingRetestComparison | null; scanHistory?: AgentScanHistoryItem[]; agentSnapshot?: AgentScanSnapshot | null; agentScanDiff?: AgentScanDiff | null; agentAuditDiff?: AgentOfflineAuditDiff | null; loading: boolean; onRunReview?: () => Promise<void>; onRunAgentAiReview?: (confirmationPhrase: string) => Promise<void>; onRun: () => Promise<void>; onUpdateFinding: (findingId: string, patch: Partial<Pick<Finding, "status">>) => Promise<void>; afterMetrics?: React.ReactNode }) {
  ...
  useEffect(() => { if (moduleKey === "agent") { setAgentTab("overview"); setAgentAssetTab("coverage"); } else { setSastTab("overview"); } }, [moduleKey, project?.id]);
  const commonAdvanced = <details className="advanced-details"><summary>查看高级分析与复核信息</summary><div className="advanced-details-body"><div className="advanced-summary-grid"><div><span>风险分类</span><KeyValue data={countBy(findings.map((item) => ({ category: item.ai_review?.category ?? "unknown" })), "category")} formatKey={moduleKey === "agent" ? agentCategoryLabel : (value) => value} /></div><div><span>严重等级</span><KeyValue data={countBy(findings, "severity")} formatKey={severityLabel} /></div></div>{moduleKey === "sast" ? <section className="advanced-inline-action"><div><strong>SAST Agent 复核</strong><span>启用 DeepSeek 后执行真实七角色审计；未启用时使用本地规则化复核。修复内容始终只保存为人工评审草案，不会直接修改源码。</span></div><FeedbackButton className="secondary-action" disabled={loading || findings.length === 0} onClick={() => onRunReview?.()}>{loading ? "复核中" : "执行 Agent 复核"}</FeedbackButton></section> : <section className="advanced-inline-action"><div><strong>AGENT DeepSeek 审计边界</strong><span>模型只接收受限脱敏静态证据，不能修改风险结论或替代人工审查。指定目标运行仍须满足项目开关、精确绑定和二次确认。</span></div></section>}</div></details>;
  if (moduleKey === "sast") return <ModuleGovernanceShell moduleKey={moduleKey} lastStatus={findings.length ? "completed" : null} metrics={[["问题总数", findings.length], ["严重 / 高危", high], ["待处理", open], ["已复核", reviewed]]} action={high ? `优先处理 ${high} 个严重或高危问题，确认影响后分配整改负责人。` : findings.length ? "逐项确认中低风险问题，记录误报或修复结论。" : "当前没有检测结果，请先在安全检测中执行该模块。"} loading={loading} onRun={onRun} afterMetrics={afterMetrics}><div className="module-workspace sast-workspace"><SastWorkspaceNavigation active={sastTab} onChange={setSastTab} />
    {sastTab === "overview" ? project ? <SastOverviewWorkspace project={project} findings={findings} onOpenTab={setSastTab} /> : <div className="empty-project">请先选择项目。</div> : null}
    {sastTab === "risks" ? <section className="module-tab-panel"><div className="module-tab-heading"><div><span>代码风险</span><h3>从规则命中走到人工处置和复测结论</h3></div><small>{findings.length} 条正式 Finding</small></div><ModuleFilterBar><input value={filters.keyword} onChange={(event) => setFilters({ ...filters, keyword: event.target.value })} placeholder="搜索风险、文件或规则" /><SimpleFilter value={filters.severity} label="全部等级" options={["critical", "high", "medium", "low", "info"]} format={severityLabel} onChange={(value) => setFilters({ ...filters, severity: value })} /><SimpleFilter value={filters.status} label="全部处理状态" options={FINDING_WORKFLOW_STATUSES} format={(value) => statusLabel(value as FindingStatus)} onChange={(value) => setFilters({ ...filters, status: value })} /><SimpleFilter value={filters.category} label="全部风险分类" options={uniqueValues(findings.map((item) => item.ai_review?.category ?? "unknown"))} onChange={(value) => setFilters({ ...filters, category: value })} /></ModuleFilterBar><ConciseFindingTable findings={pagination.items} validations={validations} evidence={evidence} graph={graph} onUpdateFinding={onUpdateFinding} /><Pagination page={pagination.page} pageCount={pagination.pageCount} total={filtered.length} onPageChange={setPage} />{comparison?.has_comparison ? <RetestComparisonPanel comparison={comparison} /> : <div className="module-context-note"><strong>等待第二次扫描</strong><span>完成下一次 SAST 扫描后，这里才展示新增、仍存在、消失和变化的问题，避免把首次扫描误称为复测。</span></div>}{commonAdvanced}</section> : null}
    {sastTab === "ai" ? <section className="module-tab-panel"><div className="module-tab-heading"><div><span>AI 辅助复核</span><h3>可选 DeepSeek 复核，不替代本地检测和人工结论</h3></div><small>可能产生外部 API 费用</small></div>{project && onRunReview ? <SastAiAgentsPanel project={project} onRunReview={onRunReview} /> : <div className="empty-project">请先选择项目。</div>}</section> : null}
    {sastTab === "strategy" ? project ? <SastStrategyWorkspace project={project} /> : <div className="empty-project">请先选择项目。</div> : null}
    {sastTab === "delivery" ? <section className="module-tab-panel"><div className="module-tab-heading"><div><span>例外与报告</span><h3>豁免、扫描历史和交付证据集中管理</h3></div><small>历史原始批次不会被改写</small></div>{project ? <SastEvidenceGovernance project={project} /> : <div className="empty-project">请先选择项目。</div>}</section> : null}
  </div></ModuleGovernanceShell>;
  ...
  return <ModuleGovernanceShell moduleKey="dast" lastStatus={runs.length ? "completed" : candidates.length ? "waiting" : null} metrics={[["待验证场景", candidates.length], ["可直接生成", readyCount], ["待运行上下文", blockedCount], ["已执行任务", report?.summary.business_run_count ?? 0]]} action={blockedCount ? `${blockedCount} 个场景尚待运行上下文，其中 ${identityContextCount} 个需要统一测试身份配置；这不是要求逐条补录漏洞信息。` : candidates.length ? "上游风险已自动归一化，可直接生成策略并预执行。" : "请先运行 SAST 或 AGENT，新的 Finding 会自动进入此工作台。"} loading={parentLoading || busy} hideRunButton onRun={() => execute("dry_run")}>
    <section className="dast-verdict-panel dast-verdict-overview">
      ...
      <div className="dast-verdict-metrics">
        {(["exploitable", "uncertain", "not_exploitable"] as const).map((verdict) => <FeedbackButton type="button" key={verdict} className={verdict} aria-pressed={verdictPanelFilter === verdict} onClick={() => setVerdictPanelFilter(verdictPanelFilter === verdict ? "all" : verdict)}><b>{triColor[verdict]}</b>{dastVerdictLabel(verdict)}<small>{verdict === "exploitable" ? "明确触发且造成实际影响" : verdict === "uncertain" ? "存在异常但证据尚不充分" : "多组验证未触发且确认防护有效"}</small></FeedbackButton>)}
      </div>
      <div className="dast-verdict-results-heading"><strong>{verdictPanelFilter === "all" ? "全部已验证结果" : dastVerdictLabel(verdictPanelFilter)}</strong><span>{visibleVerdictResults.length} 条</span>{verdictPanelFilter !== "all" ? <FeedbackButton type="button" onClick={() => setVerdictPanelFilter("all")}>查看全部</FeedbackButton> : null}</div>
      <div className="dast-verdict-results">{visibleVerdictResults.length === 0 ? <div className="workbench-empty">当前筛选下暂无已形成三色裁决的漏洞。</div> : <PagedItems>{visibleVerdictResults.map((item) => <FeedbackButton type="button" key={item.id} className={selectedCandidate?.id === item.id ? "active" : ""} onClick={() => openVerdictResult(item)}><span className={`dast-result-verdict ${item.latest_verdict}`}>{dastVerdictLabel(item.latest_verdict ?? "")}</span><strong>{item.title}</strong><small>{item.recommended_strategy_name} · {formatDateTime(item.latest_verified_at)}</small><ArrowRight size={16} /></FeedbackButton>)}</PagedItems>}</div>
      {selectedCandidate?.latest_verdict ? <div className={`dast-current-verdict ${selectedCandidate.latest_verdict}`}><div><strong>{dastVerdictLabel(selectedCandidate.latest_verdict)} · {selectedCandidate.title}</strong><p>{selectedCandidate.latest_verdict_reason || "已形成证据门控裁决。"}</p><small>Task {selectedCandidate.latest_run_id} · 已持久化 {formatDateTime(selectedCandidate.latest_verified_at)}</small></div><FeedbackButton className="secondary-action" type="button" onClick={() => { void openVerdictResult(selectedCandidate).then(() => setEvidenceDrawerOpen(true)); }}>查看任务与证据</FeedbackButton></div> : null}
    </section>
    ...
    <section className="dast-overview-grid">
      <article className="dast-overview-card"><span>目标与资产</span><strong>{target || "未配置运行目标"}</strong><p>{sandboxRuntime ? `当前使用 SANDBOX 临时运行目标。${discovery ? `已识别 ${discovery.urls.length} URL、${discovery.forms.length} 表单、${discovery.api_urls.length} API、${discovery.parameters.length} 参数。` : "可直接同步运行资产。"}` : discovery ? `${discovery.urls.length} URL · ${discovery.forms.length} 表单 · ${discovery.api_urls.length} API · ${discovery.parameters.length} 参数` : "同源、GET 限定、自动去重，最多抓取 12 个页面。"}</p><FeedbackButton className="secondary-action" disabled={busy} onClick={() => discover()}>{activeAction === "discover" ? <><LoaderCircle className="sandbox-spin" size={16} />正在同步…</> : discovery ? "重新同步资产" : "同步运行资产"}</FeedbackButton></article>
    ...
    <section className="dast-auto-layout">
      <div className="dast-candidate-panel">
        ...
        <div className="dast-candidate-list">{candidates.length === 0 ? <div className="workbench-empty">暂无可验证线索。执行 SAST 或 AGENT 后会自动出现在这里。</div> : filteredCandidates.length === 0 ? <div className="workbench-empty">没有符合当前筛选条件的漏洞。</div> : <PagedItems>{[...filteredCandidates].sort((left, right) => Number(right.readiness === "ready") - Number(left.readiness === "ready")).map((item) => <FeedbackButton key={item.id} className={selectedCandidate?.id === item.id ? "active" : ""} onClick={() => { setCandidateId(item.id); const linkedFlowId = item.latest_flow_id ?? flows.find((flow) => flow.finding_id === item.id)?.id ?? ""; void selectFlow(linkedFlowId, item.latest_run_id ?? ""); }}><span><i className={`dast-ready-dot ${item.readiness}`} />{item.source} · {severityLabel(item.severity)}<b className={`dast-validation-badge ${item.validation_status}`}>{validationStatusLabel(item.validation_status)}</b></span><strong>{item.title}</strong><small>{item.latest_verdict ? `${dastVerdictLabel(item.latest_verdict)} · ${item.recommended_strategy_name}` : item.recommended_strategy_name}</small><em>{item.validation_status === "verified" ? `已验证 ${item.validation_count} 次 · ${formatDateTime(item.latest_verified_at)}` : item.validation_status === "verifying" ? `任务 ${item.latest_run_status ?? "运行中"}` : item.validation_status === "failed" ? `最近任务 ${item.latest_run_status ?? "失败"}` : item.readiness === "ready" ? "尚未验证 · 已自动补全" : item.readiness === "blocked" ? "尚未验证 · 等待目标配置" : item.missing.some((value) => value.includes("参数")) ? "尚未验证 · 等待系统自动定位输入点" : item.missing.some((value) => value.includes("身份") || value.includes("凭据")) ? "尚未验证 · 等待项目统一测试身份" : `尚未验证 · 还缺 ${item.missing.length} 项运行上下文`}</em></FeedbackButton>)}</PagedItems>}</div>
      </div>
      <div className="dast-candidate-detail">
        {selectedCandidate ? <>
          ...
          {selectedBlockingItems.length ? <div className="dast-blockers"><strong>执行所需运行上下文（不是漏洞信息补录）</strong>{<PagedItems>{selectedBlockingItems.map((item) => <span key={item}><b>{item}</b><small>{item.includes("运行地址") || item.includes("API 地址") ? "不是在此处手工填写。已上线项目在“项目资产”配置；只有源码时到 SANDBOX 启动临时目标。" : item.includes("身份") || item.includes("凭据") ? "由管理员为项目统一配置一次测试身份，队列内同类场景自动复用；页面不会要求逐条填写或暴露明文 Cookie、Token、密码。" : "系统会尽量从项目配置、上游证据和资产发现结果中补全。"}</small></span>)}</PagedItems>}</div> : null}
          ...
          <div className="dast-generate-actions"><FeedbackButton className="primary-action" disabled={busy || (!linkedSelectedFlow && (selectedMappingItems.length > 0 || selectedCandidate.strategy_match === "ai_required"))} onClick={() => materialize()}>{activeAction === "materialize" ? <><LoaderCircle className="sandbox-spin" size={16} />正在生成策略…</> : activeAction === "open-strategy" ? <><LoaderCircle className="sandbox-spin" size={16} />正在打开…</> : linkedSelectedFlow ? "打开已生成策略" : selectedMappingItems.length ? "等待自动定位输入点" : selectedCandidate.strategy_match === "builtin" ? "自动生成 DAST 策略" : "等待新策略模板"}</FeedbackButton>{aiHealth?.configured && selectedCandidate.strategy_match === "ai_required" ? <FeedbackButton className="secondary-action" disabled={busy} onClick={() => generateAiStrategy()}>{activeAction === "ai-strategy" ? <><LoaderCircle className="sandbox-spin" size={16} />DeepSeek 生成中…</> : "DeepSeek 生成待审批模板"}</FeedbackButton> : null}</div>
    ...
    <section className="dast-execution-console" ref={executionConsoleRef}>
      ...
      {selectedFlow ? <div className="dast-console-grid">
        <div className="dast-strategy-editor"><div className="dast-id-line"><span>STRATEGY ID</span><code>{selectedFlow.id}</code></div><div className="dast-flow-meta"><span>{selectedFlow.flow_mode.toUpperCase()}</span><span>{selectedFlow.strategy_source}</span><span>{selectedFlow.allowed_paths.length} 条授权路径</span><span>{selectedFlow.roles.length} 个隔离会话</span></div><details><summary>人工修改策略步骤（可选）</summary><textarea rows={14} value={strategyJson} onChange={(event) => setStrategyJson(event.target.value)} /><FeedbackButton className="secondary-action" disabled={busy} onClick={() => saveStrategy()}>{activeAction === "save-strategy" ? <><LoaderCircle className="sandbox-spin" size={16} />正在保存…</> : "保存策略修改"}</FeedbackButton></details></div>
        <div className="dast-run-controls"><h4>执行前控制</h4><label>审批依据<input value={approval.reference} onChange={(event) => setApproval({ ...approval, reference: event.target.value })} placeholder="真实授权工单 / 书面审批编号" /></label><label>审批人<input value={approval.approved_by} onChange={(event) => setApproval({ ...approval, approved_by: event.target.value })} /></label><FeedbackButton className="secondary-action" disabled={busy || selectedFlow.status === "approved"} onClick={() => approve()}>{activeAction === "approve" ? <><LoaderCircle className="sandbox-spin" size={16} />正在记录审批…</> : selectedFlow.status === "approved" ? "范围已审批" : "审批并锁定范围"}</FeedbackButton><label>操作人<input value={operator} onChange={(event) => setOperator(event.target.value)} /></label><div className="dast-run-buttons"><FeedbackButton className="secondary-action" disabled={busy} onClick={() => execute("dry_run")}>{activeAction === "dry_run" ? <><LoaderCircle className="sandbox-spin" size={17} />正在预执行…</> : "预执行（不联网）"}</FeedbackButton><FeedbackButton className="primary-action" disabled={busy || !preflight?.can_execute_local} onClick={() => execute("api_execution")}>{activeAction === "api_execution" ? <><LoaderCircle className="sandbox-spin" size={17} />DAST HTTP 执行中…</> : "DAST HTTP 执行"}</FeedbackButton>{preflight?.can_handoff_sandbox ? <FeedbackButton className="primary-action" disabled={busy} onClick={() => handoffSandbox()}>{activeAction === "sandbox-handoff" ? <><LoaderCircle className="sandbox-spin" size={17} />正在移交…</> : "交给 SANDBOX 隔离执行"}</FeedbackButton> : null}</div>{["dry_run", "api_execution", "sandbox-handoff"].includes(activeAction) ? <div className="dast-execution-progress"><LoaderCircle className="sandbox-spin" size={19} /><div><strong>{activeAction === "api_execution" ? "DAST 正在执行已审批 HTTP 策略" : activeAction === "dry_run" ? "正在进行不联网预执行" : "正在创建 SANDBOX 隔离任务"}</strong><span>{activeAction === "api_execution" ? "请求、响应、耗时和状态变化完成后会自动归档到下方任务记录。" : "操作完成后会自动刷新执行前检查、任务和证据。"}</span></div></div> : executionFeedback ? <div className={`dast-execution-result ${executionFeedback.tone}`} role="status">{executionFeedback.text}</div> : null}</div>
      </div> : <div className="workbench-empty">从左侧漏洞队列生成策略后，执行器会在这里就绪。</div>}
    </section>
    {selectedFlow ? <section className="dast-preflight-panel"><div className="panel-header"><div><h3>执行前检查</h3><p>运行目标、审批范围、会话、路径和执行后端必须分别满足。</p></div><span className={`dast-preflight-status ${preflight?.status ?? "blocked"}`}>{preflight?.status === "ready" ? "可由 DAST 执行" : preflight?.status === "waiting_sandbox" ? "等待 SANDBOX" : "存在阻塞"}</span></div><div className="dast-preflight-grid">{<PagedItems>{preflight?.checks.map((item) => <div key={item.code} className={item.status}><b>{item.label}</b><span>{item.detail}</span>{item.remediation ? <small>{item.remediation}</small> : null}</div>)}</PagedItems>}</div>{preflight?.required_capabilities.length ? <p className="dast-capability-contract">SANDBOX 合同能力：{preflight.required_capabilities.join(" / ")}。DAST 只下发已审批策略并接收事实证据，三色裁决仍由 DAST 完成。</p> : null}</section> : null}
    <section className="dast-state-panel"><div className="panel-header"><div><h3>过程状态机</h3><p>成功路径与 BLOCKED / FAILED / CANCELED 等终止分支分别记录</p></div>{runs.length ? <select value={selectedRun?.id ?? ""} onChange={(event) => void selectRun(event.target.value)}>{runs.map((run) => <option key={run.id} value={run.id}>{run.status} · {formatDateTime(run.created_at)}</option>)}</select> : null}</div>{selectedRun ? <><div className="dast-id-line"><span>TASK ID</span><code>{selectedRun.id}</code></div><div className="dast-state-track">{<PagedItems>{stateSnapshots.map((log) => { const state = String(log.detail.state ?? log.step_id); return <div className="complete" key={log.id}><i><Check size={13} /></i><b>{state}</b><small>{String(log.response_summary ?? "已记录")}</small></div>; })}</PagedItems>}</div></> : <div className="workbench-empty">运行策略后显示完整状态轨迹。</div>}</section>
    <section className="dast-evidence-panel dast-evidence-summary">
      <div className="panel-header"><div><h3>证据归档</h3><p>主页面仅显示当前任务的证据完整性；原始事实和诊断日志按需查看。</p></div><div className="dast-evidence-actions"><FeedbackButton className="primary-action" disabled={!selectedRun && !discovery} onClick={() => setEvidenceDrawerOpen(true)}>查看证据详情</FeedbackButton><FeedbackButton className="secondary-action" disabled={busy} onClick={() => onExportReport()}>导出专项报告</FeedbackButton></div></div>
    ...
    {evidenceDrawerOpen ? <div className="dast-evidence-drawer-backdrop" role="presentation" onMouseDown={() => setEvidenceDrawerOpen(false)}>
      <aside className="dast-evidence-drawer" role="dialog" aria-modal="true" aria-labelledby="dast-evidence-drawer-title" onMouseDown={(event) => event.stopPropagation()}>
        <div className="dast-evidence-drawer-header"><div><span>DAST EVIDENCE</span><h3 id="dast-evidence-drawer-title">证据详情与执行日志</h3><p>{selectedCandidate?.title ?? "项目运行证据"} · Task {selectedRun?.id ?? "未选择"}</p></div><FeedbackButton type="button" aria-label="关闭证据详情" onClick={() => setEvidenceDrawerOpen(false)}>×</FeedbackButton></div>
        <div className="dast-evidence-drawer-body">
          ...
          <section className="dast-drawer-evidence-section"><div><h4>当前任务 HTTP 证据</h4><span>{requestSnapshots.length} 条</span></div>{requestSnapshots.length ? <div className="dast-request-log">{<PagedItems>{requestSnapshots.map((item) => <details key={item.id}><summary><code>{String(item.detail.request_id ?? item.id)}</code><span>{item.step_id} · {item.status}</span><b>{item.evidence_hash}…</b></summary><dl><div><dt>Task ID</dt><dd>{String(item.detail.task_id ?? item.run_id)}</dd></div><div><dt>Strategy ID</dt><dd>{String(item.detail.strategy_id ?? item.flow_id)}</dd></div><div><dt>请求</dt><dd>{item.request_summary ?? "未发起"}</dd></div><div><dt>响应</dt><dd>{item.response_summary ?? "未记录"}</dd></div><div><dt>脱敏原始报文与时延</dt><dd><pre className="code-preview">{JSON.stringify(item.detail.exchange ?? {}, null, 2)}</pre></dd></div></dl></details>)}</PagedItems>}</div> : <div className="workbench-empty">暂无 HTTP 请求证据。Dry Run 和等待 SANDBOX 的任务不会伪造网络证据。</div>}</section>
          <section className="dast-drawer-evidence-section"><div><h4>SANDBOX 事实证据</h4><span>{sandboxEvidenceSnapshots.length} 条</span></div>{sandboxEvidenceSnapshots.length ? <div className="dast-discovery-log dast-sandbox-fact-list">{<PagedItems>{sandboxEvidenceSnapshots.map((item) => <p key={item.id}><code>{String(item.detail.evidence_id ?? item.id)}</code><span>{String(item.detail.evidence_type ?? "runtime_trace")}</span><b>{item.status} · {item.evidence_hash}…</b></p>)}</PagedItems>}</div> : <div className="workbench-empty">当前任务没有 SANDBOX 事实证据。</div>}</section>
          {discovery?.request_logs.length ? <details className="dast-discovery-log dast-advanced-log"><summary>高级诊断：资产发现请求日志 · Task {discovery.task_id}</summary><p className="dast-log-explanation">这些同源只读 GET 仅用于发现 URL、表单和 API，不属于漏洞裁决证据。</p>{<PagedItems>{discovery.request_logs.map((item) => <p key={item.request_id}><code>{item.request_id}</code><span>{item.method} {item.url}</span><b>HTTP {item.status_code ?? item.status} · {item.duration_ms} ms</b></p>)}</PagedItems>}</details> : null}
        </div>
      </aside>
    </div> : null}
  </ModuleGovernanceShell>;
  ...
  return <ModuleGovernanceShell moduleKey="dast" lastStatus={validations.length ? "completed" : null} metrics={[["观察记录", validations.length], ["需复核信号", attention], ["未完成观察", uncertain], ["已关联风险", linked]]} action={attention ? `有 ${attention} 条基础观察带有需复核信号；它们不是漏洞利用证明，应按策略补充业务验证。` : uncertain ? `有 ${uncertain} 项基础观察未完成，需要补充网络、登录态、业务参数或专用验证策略。` : "基础观察未发现异常；这不代表上游漏洞已经排除，也不等于不可利用。"} loading={loading} hideRunButton onRun={onRun}>
    <section className="validation-workbench">
      <div className="workbench-heading"><span>动态证明</span><h3>选择一条已发现风险，在运行系统中验证它是否能被触发</h3><p>只有从具体风险发起的验证才会进入证据链；当前自动能力属于 Web 基础验证，业务漏洞应补充对应测试策略。</p></div>
        <div className="module-filter-bar"><FeedbackButton className={entryMode === "baseline" ? "primary-action" : "secondary-action"} onClick={() => setEntryMode("baseline")}>自动基础观察</FeedbackButton><FeedbackButton className={entryMode === "manual" ? "primary-action" : "secondary-action"} onClick={() => setEntryMode("manual")}>人工验证记录</FeedbackButton></div>
        <div className="validation-form">
          ...
          {entryMode === "baseline" ? <><label><span>③ 已配置项目同源目标</span><input value={targetUrl} onChange={(event) => onTargetUrlChange(event.target.value)} placeholder="https://项目运行地址/具体接口" /></label><label><span>④ 精确连接确认</span><input value={targetConfirmation} onChange={(event) => onTargetConfirmationChange(event.target.value)} placeholder={`DAST_WEB_BASELINE:${targetUrl}`} /></label><ArrowRight size={18} /><FeedbackButton className="primary-action" disabled={loading || !selectedFindingId || !targetUrl.trim() || targetConfirmation !== `DAST_WEB_BASELINE:${targetUrl}` || !selectedStrategy} onClick={() => onRun()}>{loading ? "观察中" : "⑤ 执行基础观察"}</FeedbackButton></> : <><label><span>③ 验证目标（仅记录）</span><input value={manualDraft.target_url} onChange={(event) => setManualDraft({ ...manualDraft, target_url: event.target.value })} placeholder="https://已验证的目标或接口" /></label><label><span>④ 人工裁决</span><select value={manualDraft.verdict} onChange={(event) => setManualDraft({ ...manualDraft, verdict: event.target.value as ManualDastValidationDraft["verdict"] })}><option value="exploitable">可利用（需复现证据）</option><option value="uncertain">不确定</option><option value="not_exploitable">限定范围内未复现</option></select></label><FeedbackButton className="primary-action" disabled={loading || !manualReady} onClick={() => onCreateManual(manualDraft)}>{loading ? "保存中" : "⑤ 保存人工验证"}</FeedbackButton></>}
        </div>
        ...
        {selectedStrategy ? <section className="verification-strategy-card"><div><span>本次会检查</span><strong>{selectedStrategy.name}</strong><p>{selectedStrategy.description}</p><ul>{<PagedItems>{selectedStrategy.check_items.map((item) => <li key={item}>{item}</li>)}</PagedItems>}</ul></div><div><span>明确不检查</span><p>{selectedStrategy.scope_summary}</p><ul>{<PagedItems>{selectedStrategy.limitations.map((item) => <li key={item}>{item}</li>)}</PagedItems>}</ul></div></section> : <div className="workbench-empty">先选择风险，系统才会给出适合该风险的安全验证策略。</div>}
    ...
    <section className="verification-strategy-card">
      ...
      <div className="filter-grid"><FeedbackButton className="secondary-action" disabled={loading} onClick={() => onExportReport()}>导出 DAST JSON 报告</FeedbackButton><label>选择人工记录复核<select value={reviewValidationId} onChange={(event) => setReviewValidationId(event.target.value)}><option value="">请选择人工验证记录</option>{manualRecords.map((item) => <option key={item.id} value={item.id}>{dastVerdictLabel(item.verdict)} · {item.target_url} · {formatDateTime(item.created_at)}</option>)}</select></label></div>
...
function DastVerificationLedger({ project, findings, validations, strategies }: { project: Project; findings: Finding[]; validations: DastValidation[]; strategies: DastStrategy[] }) {
  ...
  return <details className="advanced-details governance-advanced-details" open><summary>PPT 验收：DAST 验证计划、证据台账与回溯</summary><div className="advanced-details-body">
    ...
    <section className="verification-strategy-card"><div><span>① 验证计划</span><strong>授权范围与策略</strong><p>计划默认是草稿。审批记录仅保存你填写的依据和人员，不替代真实身份认证或外部授权。</p></div><div className="filter-grid"><label>计划标题<input value={planDraft.title} onChange={(event) => setPlanDraft({ ...planDraft, title: event.target.value })} /></label><label>目标（仅记录）<input value={planDraft.target_url} onChange={(event) => setPlanDraft({ ...planDraft, target_url: event.target.value })} placeholder="https://已获授权的测试环境" /></label><label>关联 Finding<select value={planDraft.finding_id} onChange={(event) => setPlanDraft({ ...planDraft, finding_id: event.target.value })}><option value="">不关联 Finding</option>{findings.map((item) => <option key={item.id} value={item.id}>{item.source} · {item.title}</option>)}</select></label><label>验证策略<select value={planDraft.strategy_id} onChange={(event) => setPlanDraft({ ...planDraft, strategy_id: event.target.value })}>{strategies.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>授权范围<textarea value={planDraft.authorized_scope} onChange={(event) => setPlanDraft({ ...planDraft, authorized_scope: event.target.value })} placeholder="环境、允许测试窗口、数据边界和禁止项" /></label><label>允许路径（每行一条）<textarea value={planDraft.allowed_paths} onChange={(event) => setPlanDraft({ ...planDraft, allowed_paths: event.target.value })} /></label><label>允许 HTTP 方法（逗号分隔）<input value={planDraft.allowed_methods} onChange={(event) => setPlanDraft({ ...planDraft, allowed_methods: event.target.value })} /></label><label>限制<textarea value={planDraft.limitations} onChange={(event) => setPlanDraft({ ...planDraft, limitations: event.target.value })} /></label><label>申请人<input value={planDraft.requester} onChange={(event) => setPlanDraft({ ...planDraft, requester: event.target.value })} /></label><FeedbackButton className="primary-action" disabled={loading} onClick={() => createPlan()}>保存草稿计划</FeedbackButton></div></section>
    <section className="verification-strategy-card"><div><span>② 审批与台账</span><strong>{plans.length} 个计划 · {runs.length} 条运行记录 · {evidence.length} 项证据</strong><p>只有标为已审批的计划才能创建台账；创建台账不代表已执行 Web 测试。</p></div><div className="filter-grid"><label>选择计划<select value={selectedPlanId} onChange={(event) => { setSelectedPlanId(event.target.value); setSelectedRunId(""); }}><option value="">请选择计划</option>{plans.map((item) => <option key={item.id} value={item.id}>{item.approval_status === "approved" ? "已审批" : item.approval_status} · {item.title}</option>)}</select></label>{selectedPlan ? <><label>审批依据<input value={approval.reference} onChange={(event) => setApproval({ ...approval, reference: event.target.value })} placeholder="工单号、书面授权编号等" /></label><label>审批人<input value={approval.approved_by} onChange={(event) => setApproval({ ...approval, approved_by: event.target.value })} /></label><FeedbackButton className="secondary-action" disabled={loading || selectedPlan.approval_status === "approved"} onClick={() => approvePlan()}>记录审批</FeedbackButton><label>台账记录人<input value={runDraft.operator} onChange={(event) => setRunDraft({ ...runDraft, operator: event.target.value })} /></label><label>台账目的<input value={runDraft.purpose} onChange={(event) => setRunDraft({ ...runDraft, purpose: event.target.value })} /></label><FeedbackButton className="secondary-action" disabled={loading || selectedPlan.approval_status !== "approved"} onClick={() => createRun()}>创建仅文档台账</FeedbackButton></> : null}</div></section>
    <section className="verification-strategy-card"><div><span>③ 证据与人工结论</span><strong>脱敏摘要、哈希、时间线</strong><p>系统对常见 Authorization、Token、Password、Secret 字段脱敏后计算哈希；该能力不是原始响应、截图或完整执行日志归档。</p></div><div className="filter-grid"><label>选择运行台账<select value={selectedRunId} onChange={(event) => setSelectedRunId(event.target.value)}><option value="">请选择运行台账</option>{runs.filter((item) => !selectedPlanId || item.plan_id === selectedPlanId).map((item) => <option key={item.id} value={item.id}>{item.status} · {item.operator} · {formatDateTime(item.created_at)}</option>)}</select></label>{selectedRun ? <><label>证据类型<input value={evidenceDraft.evidence_type} onChange={(event) => setEvidenceDraft({ ...evidenceDraft, evidence_type: event.target.value })} /></label><label>实际观察摘要<textarea value={evidenceDraft.content_summary} onChange={(event) => setEvidenceDraft({ ...evidenceDraft, content_summary: event.target.value })} placeholder="仅写入允许保留且实际观察到的摘要" /></label><label>来源引用（可选）<input value={evidenceDraft.source_reference} onChange={(event) => setEvidenceDraft({ ...evidenceDraft, source_reference: event.target.value })} /></label><label>采集人（可选）<input value={evidenceDraft.collected_by} onChange={(event) => setEvidenceDraft({ ...evidenceDraft, collected_by: event.target.value })} /></label><FeedbackButton className="secondary-action" disabled={loading} onClick={() => addEvidence()}>保存脱敏证据</FeedbackButton><label>关联人工三态裁决<select value={validationId} onChange={(event) => setValidationId(event.target.value)}><option value="">请选择人工验证记录</option>{validations.filter((item) => item.validation_mode === "manual_validation").map((item) => <option key={item.id} value={item.id}>{dastVerdictLabel(item.verdict)} · {item.target_url}</option>)}</select></label><FeedbackButton className="primary-action" disabled={loading || selectedRun.status === "reviewed"} onClick={() => markReviewed()}>关联结论并复核</FeedbackButton></> : null}</div>{selectedRun ? <div className="empty-project">当前台账证据：{<PagedItems>{evidence.filter((item) => item.run_id === selectedRun.id).map((item) => <p key={item.id}>{item.evidence_type} · 哈希 {item.content_hash}… · {formatDateTime(item.created_at)}</p>)}</PagedItems>}</div> : null}</section>
...
function SandboxGovernanceView({ project }: { project: Project }) {
  ...
  return <section className="sandbox-workbench">
    <section className="module-governance-heading sandbox-workbench-heading"><div className="module-icon">{moduleIcons.sandbox}</div><div><h2>{MODULE_DISPLAY.sandbox.name}</h2><p>{MODULE_DISPLAY.sandbox.purpose}</p></div><FeedbackButton className="secondary-action" disabled={Boolean(busy)} onClick={() => refreshWorkspace()}>{busy === "refresh" ? "刷新中…" : "刷新工作台"}</FeedbackButton></section>
    ...
    <section className="panel full"><div className="panel-header"><div><h2>1. 执行能力预检</h2><span>{health?.docker.detail ?? "正在检查执行环境"}</span></div><span className={`sandbox-state ${health?.status ?? "checking"}`}>{health?.status === "ready" ? "基础执行器就绪" : "存在阻塞项"}</span></div>
      <div className="sandbox-capability-grid">{<PagedItems>{capabilityEntries.map(([key, item]) => <article key={key} className={`sandbox-capability ${item.status}`}><div><strong>{sandboxCapabilityLabel(key)}</strong><span>{sandboxStatusLabel(item.status)}</span></div><p>{item.detail}</p></article>)}</PagedItems>}</div>
    </section>
    ...
    <section className="panel full"><div className="panel-header"><div><h2>2. 项目测试目标</h2><span>已上线地址可直接注册；只有源码时由 SANDBOX 生成临时 URL，不需要手工编写地址</span></div></div>
      ...
      <div className="sandbox-target-actions"><FeedbackButton className="primary-action" disabled={!authorized || !Boolean(project.runtime_url || project.api_base_url) || Boolean(busy)} onClick={() => createTarget("external")}>{busy === "target-external" ? <><LoaderCircle className="sandbox-spin" size={17} />正在检查目标…</> : "注册已上线目标"}</FeedbackButton><FeedbackButton className="secondary-action" disabled={!authorized || !dockerTargetReady || Boolean(busy)} onClick={() => createTarget("docker")}>{busy === "target-docker" ? <><LoaderCircle className="sandbox-spin" size={17} />正在启动隔离实例…</> : "启动项目隔离实例"}</FeedbackButton></div>
    ...
    <section className="sandbox-task-layout"><section className="panel"><div className="panel-header"><div><h2>3. DAST 自动验证队列</h2><span>仅接收 DAST 已批准的隔离合同</span></div><span>{tasks.length} 个任务</span></div><div className="sandbox-task-list">{tasks.length === 0 ? <div className="empty-project">暂无任务。在 DAST 中批准策略并提交隔离执行后会自动出现在这里。</div> : <PagedItems>{tasks.map((task) => <FeedbackButton key={task.id} className={`sandbox-task-item ${selectedTask?.id === task.id ? "active" : ""}`} onClick={() => setSelectedTaskId(task.id)}><span className={`sandbox-state ${task.status}`}>{sandboxStatusLabel(task.status)}</span><strong>{String((task.contract as { target?: { url?: string } }).target?.url ?? task.strategy_id)}</strong><small>{task.required_capabilities.map(sandboxCapabilityLabel).join("、") || "基础 HTTP"} · {formatDateTime(task.created_at)}</small></FeedbackButton>)}</PagedItems>}</div></section>
      <section className="panel sandbox-task-detail"><div className="panel-header"><div><h2>4. 执行与证据回传</h2><span>{selectedTask ? `任务 ${selectedTask.id}` : "请选择任务"}</span></div></div>{!selectedTask ? <div className="empty-project">当前没有可执行任务。</div> : <>
        ...
        <div className="sandbox-target-actions"><FeedbackButton className="primary-action" disabled={Boolean(busy) || !["queued", "blocked"].includes(selectedTask.status) || runningTargets.length === 0} onClick={() => executeSelected()}>{busy.startsWith("execute-") ? "隔离执行中…" : "执行固定验证策略"}</FeedbackButton><FeedbackButton className="secondary-action" disabled={Boolean(busy) || !["queued", "blocked"].includes(selectedTask.status)} onClick={() => cancelSelected()}>取消任务</FeedbackButton></div>
        {runningTargets.length === 0 ? <div className="empty-project">没有健康目标实例，暂不能执行；任务与策略不会丢失。</div> : null}
        <details className="record-evidence" open><summary>状态机日志（{events.length}）</summary><ol className="sandbox-event-list">{<PagedItems>{events.map((event) => <li key={event.id}><b>{event.state}</b><span>{sandboxStatusLabel(event.status)}</span><p>{String(event.detail.message ?? "状态已记录")}</p><small>{formatDateTime(event.created_at)}</small></li>)}</PagedItems>}</ol></details>
        <details className="record-evidence" open><summary>事实证据（{selectedTask.evidence.length}）</summary><div className="sandbox-evidence-list">{selectedTask.evidence.length === 0 ? <p>尚未产生证据。</p> : <PagedItems>{selectedTask.evidence.map((item, index) => <article key={String(item.evidence_id ?? index)}><strong>{String(item.type ?? "runtime_trace")} · {item.confirmed ? "已确认触发" : "已记录"}</strong><p>{String(item.facts ?? "")}</p><small>request_id: {String(item.request_id ?? "-")} · sha256: {String(item.artifact_sha256 ?? "-")}</small></article>)}</PagedItems>}</div></details>
      </>}</section>
    </section>
  </section>;
}
  ...
  return <ModuleGovernanceShell moduleKey="sandbox" lastStatus={evidence.length ? "completed" : null} metrics={[["运行证据", evidence.length], ["执行完成", completed], ["进入证据链", linked], ["隔离策略", "禁网 / 只读"]]} action={linked ? "结合上游风险和 DAST 裁决复核运行行为，判断证据是否足以支持最终风险结论。" : "请先选择一条风险或 DAST 验证，独立命令执行不能证明漏洞成立。"} loading={loading} hideRunButton onRun={() => onRun(executionPlan)}>
    <section className="validation-workbench sandbox-workbench">
      ...
      <div className="validation-form sandbox-command-form">
        ...
        <FeedbackButton className="primary-action" disabled={loading || (!selectedFindingId && !selectedValidationId) || !runCommand.trim()} onClick={() => onRun(executionPlan)}>{loading ? "取证中" : "③ 执行隔离取证"}</FeedbackButton>
      </div>
...
function ModuleGovernanceShell({ moduleKey, lastStatus, metrics, action, loading, runDisabled = false, runLabel, hideRunButton = false, onRun, children, afterMetrics }: { moduleKey: Exclude<ModuleKey, "aspm">; lastStatus: string | null; metrics: Array<[string, string | number]>; action: string; loading: boolean; runDisabled?: boolean; runLabel?: string; hideRunButton?: boolean; onRun: () => Promise<void>; children: React.ReactNode; afterMetrics?: React.ReactNode }) {
  return <div className="governance-view module-governance-view">
    <section className="module-governance-heading"><div className="module-icon">{moduleIcons[moduleKey]}</div><div><h2>{MODULE_DISPLAY[moduleKey].name}</h2><p>{MODULE_DISPLAY[moduleKey].purpose}</p></div><div className="module-run-actions"><span>{lastStatus ? scanStatusLabel(lastStatus) : "尚未执行"}</span>{hideRunButton ? null : <FeedbackButton className="primary-action" disabled={loading || runDisabled} onClick={() => onRun()}>{loading ? "执行中" : runLabel ?? (moduleKey === "dast" || moduleKey === "sandbox" ? "再次执行" : moduleKey === "agent" ? "重新扫描并对比" : "重新扫描并复测")}</FeedbackButton>}</div></section>
...
function ConciseFindingTable({ findings, validations, evidence, graph, onUpdateFinding }: { findings: Finding[]; validations: DastValidation[]; evidence: SandboxEvidence[]; graph?: EvidenceGraph | null; onUpdateFinding: (findingId: string, patch: Partial<Pick<Finding, "status">>) => Promise<void> }) {
  ...
  return <div className="concise-finding-table-wrap"><PagedTable className="concise-table mobile-card-table"><thead><tr><th>风险问题</th><th>等级</th><th>位置</th><th>处理状态</th><th>验证证据</th></tr></thead><tbody>{findings.length === 0 ? <tr><td colSpan={5} className="empty-cell">当前没有需要展示的风险问题。</td></tr> : findings.map((finding) => { const rawTitle = findingTitle(finding); const rawDescription = finding.ai_review?.description ?? finding.evidence ?? "暂无影响说明"; const displayTitle = finding.source === "AGENT" ? agentFindingDisplayTitle(finding) : rawTitle; const description = finding.source === "AGENT" ? agentUiText(rawDescription) : rawDescription; const evidenceNodes = findingEvidenceNodes(finding.id, graph); const validationCount = validations.filter((item) => item.finding_id === finding.id).length; const evidenceCount = evidence.filter((item) => item.finding_id === finding.id || Boolean(item.validation_id && validations.some((validation) => validation.id === item.validation_id && validation.finding_id === finding.id))).length; const occurrenceCount = finding.ai_review?.occurrence_count ?? 1; return <tr key={finding.id}><td data-label="风险问题"><strong title={displayTitle}>{truncateText(displayTitle, 100)}</strong><span className="cell-subtext" title={description}>{truncateText(description, 140)}</span></td><td data-label="等级"><span className={`severity ${finding.severity}`}>{severityLabel(finding.severity)}</span></td><td data-label="位置">{finding.file_path ?? "项目级问题"}<span className="cell-subtext">{finding.line_start ? `第 ${finding.line_start} 行` : finding.source}{occurrenceCount > 1 ? ` · 共 ${occurrenceCount} 处证据` : ""}</span></td><td data-label="处理状态"><select value={normalizeFindingStatus(finding.status)} onChange={(event) => void onUpdateFinding(finding.id, { status: event.target.value as FindingStatus })}>{FINDING_WORKFLOW_STATUSES.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></td><td data-label="验证证据"><FeedbackButton className="secondary-action" onClick={() => setSelectedFinding(finding)}>{validationCount || evidenceCount ? `查看证据（${validationCount + evidenceCount}）` : evidenceNodes.length ? `查看关系（${evidenceNodes.length}）` : occurrenceCount > 1 ? `查看代码证据（${occurrenceCount}）` : "尚未验证"}</FeedbackButton></td></tr>; })}</tbody></PagedTable>{selectedFinding ? <FindingEvidenceDetail finding={selectedFinding} validations={validations} evidence={evidence} graph={graph ?? null} onClose={() => setSelectedFinding(null)} /> : null}</div>;
}
...
function Pagination({ page, pageCount, total, onPageChange }: { page: number; pageCount: number; total: number; onPageChange: (page: number) => void }) {
  return <div className="result-pagination"><span>共 {total} 条，每页 10 条</span><div><FeedbackButton disabled={page <= 1} onClick={() => onPageChange(page - 1)}>上一页</FeedbackButton><strong>第 {page} / {pageCount} 页</strong><FeedbackButton disabled={page >= pageCount} onClick={() => onPageChange(page + 1)}>下一页</FeedbackButton></div></div>;
}
...
function FindingEvidenceDetail({ finding, validations, evidence, graph, onClose }: { finding: Finding; validations: DastValidation[]; evidence: SandboxEvidence[]; graph: EvidenceGraph | null; onClose: () => void }) {
  ...
  return <section className="finding-evidence-detail">
    <div className="panel-header"><div><h3>风险证据链</h3><span>{finding.source === "AGENT" ? agentFindingDisplayTitle(finding) : finding.title}</span></div><FeedbackButton className="secondary-action" onClick={onClose}>关闭</FeedbackButton></div>
    <div className="evidence-conclusion"><strong>{conclusion}</strong><span>{nodes.length ? "以下内容来自已保存的显式关联，不代表自动确认漏洞成立。" : "当前问题还没有关联 DAST 或 SANDBOX 证据。"}</span></div>
      {finding.ai_review?.occurrences?.length ? <details className="record-evidence" open><summary>代码证据位置（{finding.ai_review.occurrences.length}）</summary><dl>{<PagedItems>{finding.ai_review.occurrences.map((item, index) => <div key={`${item.file_path}-${item.line_start}-${index}`}><dt>{item.file_path}:{item.line_start}</dt><dd>{item.evidence}</dd></div>)}</PagedItems>}</dl></details> : null}
      {finding.source === "SAST" && ["critical", "high"].includes(finding.severity) ? <details className="record-evidence"><summary>人工评审修复草案（不会修改源码）</summary><p>草案仅供开发人员参考，不会写入文件、创建提交、创建 PR 或自动执行回归。</p><FeedbackButton className="secondary-action" disabled={fixDraftLoading} onClick={() => loadFixDraft()}>{fixDraftLoading ? "正在生成" : fixDraft ? "重新生成草案" : "生成修复草案"}</FeedbackButton>{fixDraftMessage ? <div className="empty-project">{fixDraftMessage}</div> : null}{fixDraft ? <dl><div><dt>建议修改</dt><dd>{fixDraft.recommended_change}</dd></div><div><dt>补丁草案</dt><dd><pre className="code-preview">{fixDraft.patch}</pre></dd></div><div><dt>限制</dt><dd>{fixDraft.limitations.join("；") || "无额外说明"}</dd></div><div><dt>回归入口</dt><dd>{fixDraft.regression_scan.endpoint}（必填：{fixDraft.regression_scan.required_fields.join("、")}）</dd></div></dl> : null}</details> : null}
      ...
      <li><b>{finding.source}</b><div><strong>发现风险</strong><span>{finding.file_path ?? "项目级问题"} · {severityLabel(finding.severity)}</span></div></li>
        {<PagedItems>{nodes.map((node) => <li key={node.id}><b>{node.module}</b><div><strong>{evidenceNodeStage(node)}</strong><span>{node.module === "AGENT" ? agentUiText(node.label) : node.label}</span><small>{node.module === "AGENT" ? agentUiText(node.detail ?? "未记录证据摘要") : node.detail ?? "未记录证据摘要"} · {formatDateTime(node.created_at)}</small></div></li>)}</PagedItems>}
      </ol>
  ...
  return (
    <section className="panel full evidence-link-selector">
      ...
      <div className="link-suggestion-list">
        {suggestions.length === 0 ? <div className="empty-project">暂无可靠候选。当前记录可以保持未关联，或手动选择。</div> : <PagedItems>{suggestions.map((suggestion, index) => {
          ...
          return <article className={`link-suggestion ${selected ? "selected" : ""}`} key={`${suggestion.validation_id ?? suggestion.finding_id ?? index}`}>
            ...
            <div className="suggestion-score"><b className={suggestion.confidence_level}>{suggestion.confidence}%</b><span>{confidenceLevelLabel(suggestion.confidence_level)}</span><FeedbackButton className="secondary-action" onClick={() => onSuggestionApply(suggestion)}>{selected ? "已采用" : "采用建议"}</FeedbackButton></div>
          </article>;
        })}</PagedItems>}
      </div>
...
function SastView({ project, findings, categorySummary, sourcePath, loading, onSourcePathChange, onRunScan, onAgentReview }: { project: Project | null; findings: Finding[]; categorySummary: Record<string, number>; sourcePath: string; loading: boolean; onSourcePathChange: (value: string) => void; onRunScan: () => Promise<void>; onAgentReview: () => Promise<void> }) {
  ...
  async function exportSarif() {
    if (!project) return;
    try {
      const response = await feedbackFetch(`${API_BASE}/sast/projects/${project.id}/sarif`, { credentials: "include" });
      if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
      const url = URL.createObjectURL(await response.blob());
      const link = document.createElement("a");
      link.href = url;
  ...
  return <section className="sca-layout">
    <div className="sca-toolbar panel full"><div><h2>SAST 智能静态审计</h2><p>Semgrep 与本地规则的实际执行或降级状态会随扫描快照保留；规则化 Sub-agent 用于复核、证据归档和修复建议归一化。</p></div><div className="path-control"><input value={sourcePath} onChange={(event) => onSourcePathChange(event.target.value)} /><FeedbackButton className="primary-action" onClick={() => onRunScan()} disabled={loading || !project}>{loading ? "执行中" : "执行 SAST 审计"}</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => onAgentReview()} disabled={loading || !project || findings.length === 0}>执行 Agent 复核</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => setAdvancedOpen((value) => !value)} disabled={!project}>{advancedOpen ? "收起扫描治理" : "扫描治理与导出"}</FeedbackButton></div></div>
...
function ScaView({ project, components, scanHistory, selectedScanId, scanDiff, dependencyGraph, sourcePath, toolScanEnabled, ecosystemSummary, riskSummary, loading, onSourcePathChange, onToolScanChange, onRunScan, onExportSbom, onExportReport, onSelectScan }: { project: Project | null; components: Component[]; scanHistory: ScaScanHistoryItem[]; selectedScanId: string | null; scanDiff: ScaScanDiff | null; dependencyGraph: DependencyGraph | null; sourcePath: string; toolScanEnabled: boolean; ecosystemSummary: Record<string, number>; riskSummary: Record<string, number>; loading: boolean; onSourcePathChange: (value: string) => void; onToolScanChange: (enabled: boolean) => void; onRunScan: () => Promise<void>; onExportSbom: (format: "cyclonedx" | "spdx") => Promise<void>; onExportReport: () => Promise<void>; onSelectScan: (scanTaskId: string) => Promise<void> }) {
  ...
  if (mode === "graph") {
    return <section className="sca-layout"><div className="sca-toolbar panel full"><div><h2>完整依赖图谱</h2><p>优先展示项目 Python 环境实际安装关系；其次为 NPM 原生依赖树，最后才是锁文件推断关系。</p></div><div className="path-control"><FeedbackButton className="secondary-action" onClick={() => setMode("list")}>返回 SCA 清单</FeedbackButton></div></div><section className="module-summary"><Metric label="节点" value={dependencyGraph?.summary.node_count ?? 0} /><Metric label="依赖边" value={dependencyGraph?.summary.edge_count ?? 0} /><Metric label="风险节点" value={dependencyGraph?.summary.risk_node_count ?? 0} /><Metric label="Python 实际 / NPM / 推断" value={`${dependencyGraph?.summary.python_environment_edge_count ?? 0} / ${dependencyGraph?.summary.native_tree_edge_count ?? 0} / ${dependencyGraph?.summary.lockfile_inferred_edge_count ?? 0}`} /></section><div className="content-grid"><div className="panel full"><div className="panel-header"><h2>完整图谱</h2><span>{selectedScanId ? `批次 ${selectedScanId.slice(0, 8)}` : project?.name ?? "未连接"}</span></div><DependencyGraphView graph={dependencyGraph} full /></div><div className="panel full"><div className="panel-header"><h2>升级杠杆</h2><span>{dependencyGraph?.upgrade_levers.length ?? 0} 项</span></div><UpgradeLeverTable levers={dependencyGraph?.upgrade_levers ?? []} /></div></div></section>;
  }

  if (mode === "history") {
    return <section className="sca-layout"><div className="sca-toolbar panel full"><div><h2>SCA 扫描历史</h2><p>查看项目历次 SCA 扫描快照，切换后组件清单、SBOM 和依赖图谱都会使用对应批次。</p></div><div className="path-control"><FeedbackButton className="secondary-action" onClick={() => setMode("list")}>返回 SCA 清单</FeedbackButton></div></div><section className="module-summary"><Metric label="历史批次" value={scanHistory.length} /><Metric label="当前批次" value={selectedScanId ? selectedScanId.slice(0, 8) : "-"} /><Metric label="当前组件" value={components.length} /><Metric label="最近完成" value={formatDateTime(scanHistory[0]?.finished_at)} /></section><div className="content-grid"><div className="panel full"><div className="panel-header"><h2>与上一批次对比</h2><span>{scanDiff?.base_scan_id ? `${scanDiff.base_scan_id.slice(0, 8)} → ${scanDiff.target_scan_id.slice(0, 8)}` : "需要至少两次扫描"}</span></div><ScaScanDiffView diff={scanDiff} /></div><div className="panel full"><div className="panel-header"><h2>全部历史记录</h2><span>{project?.name ?? "未连接"}</span></div><ScaScanHistoryTable history={scanHistory} selectedScanId={selectedScanId} loading={loading} onSelect={async (scanTaskId) => { await onSelectScan(scanTaskId); setMode("list"); }} /></div></div></section>;
  }

  return <section className="sca-layout"><div className="sca-toolbar panel full"><div><h2>SCA 供应链风险分析</h2><p>锁文件、实际环境或 Syft 提供精确版本后才会进行漏洞匹配；版本范围或情报不可用时明确标记“需要复核”，不会误报为无风险。</p></div><div className="path-control"><input value={sourcePath} onChange={(event) => onSourcePathChange(event.target.value)} /><label className="inline-check"><input type="checkbox" checked={toolScanEnabled} onChange={(event) => onToolScanChange(event.target.checked)} disabled={loading} />Syft/Grype/Trivy 增强</label><FeedbackButton className="primary-action" onClick={() => onRunScan()} disabled={loading || !project}>{loading ? "执行中" : "执行 SCA 风险分析"}</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => onExportSbom("cyclonedx")} disabled={loading || !project || components.length === 0}>导出 CycloneDX</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => onExportSbom("spdx")} disabled={loading || !project || components.length === 0}>导出 SPDX</FeedbackButton><FeedbackButton className="secondary-action" onClick={() => onExportReport()} disabled={loading || !project || components.length === 0}>导出 SCA 报告</FeedbackButton></div></div><section className="module-summary"><Metric label="筛选结果" value={`${filteredComponents.length} / ${components.length}`} /><Metric label="直接 / 传递" value={`${directCount} / ${dependencyTypeSummary.transitive ?? 0}`} /><Metric label="风险传递依赖" value={riskyTransitiveCount} /><Metric label="漏洞情报覆盖" value={currentAssurance ? `${currentAssurance.vulnerability_coverage_percent ?? 0}%` : "旧批次未记录"} /></section><div className="content-grid"><ScaToolHealthPanel health={toolHealth} loading={toolHealthLoading} onRefresh={refreshToolHealth} /><div className="panel full"><div className="panel-header"><h2>扫描可信度</h2><span>{scanAssuranceLabel(currentAssurance?.status)} · {confidenceLabel(currentAssurance?.confidence)}</span></div><div className="kv-list"><div><span>组件 / 精确版本</span><strong>{currentAssurance?.component_count ?? components.length} / {currentAssurance?.resolved_component_count ?? 0}</strong></div><div><span>锁文件或实际环境</span><strong>{currentAssurance?.lock_or_environment_component_count ?? 0}</strong></div><div><span>版本范围待解析</span><strong>{currentAssurance?.constraint_component_count ?? 0}</strong></div><div><span>漏洞情报已验证 / 未验证</span><strong>{currentAssurance?.verified_component_count ?? 0} / {currentAssurance?.unverified_component_count ?? components.length}</strong></div><div><span>覆盖率</span><strong>{currentAssurance?.vulnerability_coverage_percent ?? 0}%</strong></div></div><p>{currentAssurance?.statement ?? "旧扫描批次没有可信度快照，建议重新执行 SCA。"}</p>{currentAssurance?.reasons?.length ? <p>待完善：{currentAssurance.reasons.join("；")}</p> : null}</div><div className="panel full"><div className="panel-header"><h2>增强引擎状态</h2><span>{toolStatusLabel(currentToolStatus?.status)}</span></div><div className="kv-list"><div><span>临时依赖解析</span><strong>{toolExecutionStatusLabel(currentToolStatus?.dependency_resolution_status)}<span className="cell-subtext">{currentToolStatus?.dependency_resolution_detail ?? "本批次未记录"}</span></strong></div><div><span>Syft</span><strong>{toolExecutionStatusLabel(currentToolStatus?.syft_status)} · {currentToolStatus?.syft_component_count ?? 0} 组件<span className="cell-subtext">{currentToolStatus?.syft_detail ?? "-"}</span></strong></div><div><span>Grype</span><strong>{toolExecutionStatusLabel(currentToolStatus?.grype_status)} · {currentToolStatus?.grype_vulnerability_count ?? 0} 漏洞<span className="cell-subtext">{currentToolStatus?.grype_detail ?? "-"}</span></strong></div><div><span>Trivy</span><strong>{toolExecutionStatusLabel(currentToolStatus?.trivy_status)} · 漏洞 {currentToolStatus?.trivy_vulnerability_count ?? 0} · 配置 {currentToolStatus?.trivy_misconfiguration_count ?? 0} · 密钥 {currentToolStatus?.trivy_secret_count ?? 0}<span className="cell-subtext">{currentToolStatus?.trivy_detail ?? "-"}</span></strong></div><div><span>错误</span><strong>{currentToolStatus?.errors.length ?? 0}</strong></div></div>{currentToolStatus?.errors.length ? <div className="empty-project"><PagedItems>{currentToolStatus.errors.map((error, index) => <p key={index}>{error}</p>)}</PagedItems></div> : null}</div><FeedbackButton className="graph-open-card panel full" onClick={() => setMode("history")} disabled={scanHistory.length === 0}><span>扫描历史</span><strong>{scanHistory.length ? `${scanHistory.length} 次扫描` : "暂无历史"}</strong><em>{scanDiff?.has_comparison ? `新增风险 ${scanDiff.summary.risk_added} · 已消失 ${scanDiff.summary.risk_removed} · 组件变化 ${scanDiff.summary.added_components + scanDiff.summary.removed_components + scanDiff.summary.version_changes}` : selectedScanId ? `当前批次 ${selectedScanId.slice(0, 8)} · 最近完成 ${formatDateTime(scanHistory[0]?.finished_at)}` : "执行 SCA 后可查看全部历史记录"}</em></FeedbackButton><FeedbackButton className="graph-open-card panel full" onClick={() => setMode("graph")} disabled={!dependencyGraph}><span>依赖图谱</span><strong>{dependencyGraph ? `${dependencyGraph.summary.node_count ?? 0} 节点 / ${dependencyGraph.summary.edge_count ?? 0} 边` : "暂无图谱"}</strong><em>Python 实际边 {dependencyGraph?.summary.python_environment_edge_count ?? 0} · NPM 原生边 {dependencyGraph?.summary.native_tree_edge_count ?? 0}</em></FeedbackButton><div className="panel full"><div className="panel-header"><h2>升级杠杆</h2><span>{dependencyGraph?.upgrade_levers.length ?? 0} 项</span></div><UpgradeLeverTable levers={dependencyGraph?.upgrade_levers ?? []} /></div><div className="panel full"><div className="panel-header"><h2>组件筛选</h2><span>Filter</span></div><div className="filter-grid"><FilterSelect label="生态" value={filters.ecosystem} options={filterOptions.ecosystems} onChange={(value) => setFilters((current) => ({ ...current, ecosystem: value }))} /><FilterSelect label="依赖类型" value={filters.dependencyType} options={filterOptions.dependencyTypes} formatOption={dependencyTypeLabel} onChange={(value) => setFilters((current) => ({ ...current, dependencyType: value }))} /><FilterSelect label="风险状态" value={filters.riskStatus} options={filterOptions.riskStatuses} formatOption={riskStatusLabel} onChange={(value) => setFilters((current) => ({ ...current, riskStatus: value }))} /><FilterSelect label="严重等级" value={filters.severity} options={filterOptions.severities} formatOption={severityLabel} onChange={(value) => setFilters((current) => ({ ...current, severity: value }))} /><FilterSelect label="许可证策略" value={filters.licensePolicy} options={filterOptions.licensePolicies} formatOption={licensePolicyLabel} onChange={(value) => setFilters((current) => ({ ...current, licensePolicy: value }))} /><FeedbackButton className="secondary-action" onClick={() => setFilters({ ecosystem: "all", dependencyType: "all", riskStatus: "all", severity: "all", licensePolicy: "all" })}>清空筛选</FeedbackButton></div></div><div className="panel"><div className="panel-header"><h2>生态分布</h2><span>SBOM ecosystem</span></div><KeyValue data={filteredEcosystemSummary} /></div><div className="panel"><div className="panel-header"><h2>依赖类型</h2><span>Dependency</span></div><KeyValue data={dependencyTypeSummary} formatKey={dependencyTypeLabel} /></div><div className="panel"><div className="panel-header"><h2>许可证策略</h2><span>License</span></div><KeyValue data={licensePolicySummary} formatKey={licensePolicyLabel} /></div><div className="panel full"><div className="panel-header"><h2>组件风险清单</h2><span>Project: {project?.name ?? "未连接"}</span></div><table><thead><tr><th>生态</th><th>组件</th><th>版本</th><th>类型</th><th>风险</th><th>来源 / OSV</th><th>漏洞编号</th><th>许可证</th><th>修复建议</th></tr></thead><tbody>{components.length === 0 ? <tr><td colSpan={9} className="empty-cell">暂无组件，执行 SCA 扫描后显示结果。</td></tr> : filteredComponents.length === 0 ? <tr><td colSpan={9} className="empty-cell">当前筛选条件下没有组件。</td></tr> : pageComponents.map((component) => <tr key={component.id}><td><span className="ecosystem-badge">{component.ecosystem}</span></td><td><strong>{component.name}</strong><span className="cell-subtext">{component.source_file}</span></td><td>{component.version ?? "-"}</td><td>{dependencyTypeLabel(component.dependency_type)}</td><td><RiskBadge status={component.risk_status ?? "not_checked"} severity={component.severity ?? null} /></td><td><span className="risk-badge review-required">{sourceLabel(component.risk_source)}</span><span className="cell-subtext">{component.osv_checked ? "OSV 已查询" : "OSV 未查询"}</span>{component.osv_error ? <span className="cell-subtext">{component.osv_error}</span> : null}</td><td>{component.vulnerability_ids?.length ? component.vulnerability_ids.join(", ") : "-"}</td><td>{component.license ?? "-"}{component.license_risk ? <span className="cell-subtext">策略：{licensePolicyLabel(component.license_risk)}</span> : null}</td><td>{component.remediation ?? component.risk_summary ?? "-"}</td></tr>)}</tbody></table><div className="pagination"><FeedbackButton disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>上一页</FeedbackButton><span>第 {currentPage} / {pageCount} 页，每页 {pageSize} 条，共 {filteredComponents.length} 条</span><FeedbackButton disabled={currentPage >= pageCount} onClick={() => setPage((value) => Math.min(pageCount, value + 1))}>下一页</FeedbackButton></div></div></div></section>;
}
...
function ScaToolHealthPanel({ health, loading, onRefresh }: { health: ScaToolHealth | null; loading: boolean; onRefresh: () => Promise<void> }) {
  ...
  return <div className="panel full">
    ...
    <div className="kv-list">
      ...
      <div><span>操作</span><FeedbackButton className="secondary-action" disabled={loading} onClick={() => onRefresh()}>{loading ? "检查中" : "重新预检"}</FeedbackButton></div>
    </div>
...
function UpgradeLeverTable({ levers }: { levers: UpgradeLever[] }) {
  ...
  return <><table><thead><tr><th>直接依赖</th><th>风险传递依赖</th><th>最高等级</th><th>影响组件</th><th>建议动作</th></tr></thead><tbody>{pagination.items.map((lever) => <tr key={lever.component_id}><td><strong>{lever.component}</strong><span className="cell-subtext">{lever.ecosystem} · {lever.version ?? "-"}</span></td><td>{lever.risk_transitive_count}</td><td>{severityLabel(lever.highest_severity ?? "none")}</td><td><PagedItems>{lever.affected_components.map(item => <span className="cell-subtext" key={item}>{item}</span>)}</PagedItems></td><td>{lever.recommendation}</td></tr>)}</tbody></table><Pagination page={pagination.page} pageCount={pagination.pageCount} total={levers.length} onPageChange={setPage} /></>;
}
...
function ScaGovernancePanel({ summary }: { summary?: ScaGovernanceSummary }) {
  ...
  return <div className="panel full sca-governance-panel">
    ...
    <div className="sca-tool-status">
      ...
      <div><span>Trivy</span><strong>{toolExecutionStatusLabel(toolStatus?.trivy_status)} · 漏洞 {toolStatus?.trivy_vulnerability_count ?? 0} · 配置 {toolStatus?.trivy_misconfiguration_count ?? 0} · 密钥 {toolStatus?.trivy_secret_count ?? 0}</strong></div>
    ...
    {toolStatus?.errors?.length ? <div className="sca-tool-errors">{<PagedItems>{toolStatus.errors.map((error, index) => <p key={`${index}-${error}`}>{error}</p>)}</PagedItems>}</div> : null}
    <ScaToolSecurityFindingTable findings={toolStatus?.security_findings ?? []} />
...
function ScaToolSecurityFindingTable({ findings }: { findings: ScaToolSecurityFinding[] }) {
  if (!findings.length) return null;
  return <><div className="panel-header"><h3>Trivy 配置与密钥结果</h3><span>{findings.length} 条（不计入组件漏洞数）</span></div><PagedTable className="compact-table"><thead><tr><th>类型</th><th>规则</th><th>等级</th><th>位置</th><th>说明</th></tr></thead><tbody>{findings.map((finding, index) => <tr key={`${finding.kind}-${finding.rule_id}-${finding.target}-${finding.line ?? index}`}><td>{finding.kind === "secret" ? "密钥" : "配置"}</td><td>{finding.rule_id}<span className="cell-subtext">{finding.title}</span></td><td>{severityLabel(finding.severity)}</td><td>{finding.target}{finding.line ? `:${finding.line}` : ""}</td><td>{finding.description ?? "-"}<span className="cell-subtext">{finding.remediation ?? ""}</span></td></tr>)}</tbody></PagedTable>{findings.length > 20 ? <p className="cell-subtext">当前展示前 20 条，完整结果已随扫描批次保存。</p> : null}</>;
}
function KeyValue({ data, formatKey = (key) => key }: { data: Record<string, number>; formatKey?: (key: string) => string }) { const entries = Object.entries(data); return <div className="kv-list">{entries.length === 0 ? <span className="empty-inline">暂无数据</span> : <PagedItems>{entries.map(([key, value]) => <div key={key}><span>{formatKey(key)}</span><strong>{value}</strong></div>)}</PagedItems>}</div>; }
...
function toolExecutionStatusLabel(value?: string | null) { return value === "success" ? "成功" : value === "fallback" ? "已使用基础清单" : value === "failed" ? "失败" : value === "not_needed" ? "无需执行" : value === "not_run" ? "未运行" : value ?? "旧批次未记录"; }
...
function agentUiText(value?: unknown) {
  ...
  const detailedPath = text.match(/^Prompt context can reach (.+): (.+)$/);
  if (detailedPath) return `Prompt 可能触达${agentCapabilityLabel(detailedPath[1])}：${agentResourceScopeLabel(detailedPath[2])}`;
  const suspiciousPath = text.match(/^Suspicious instructions can reach (.+): (.+)$/);
  if (suspiciousPath) return `可疑指令可能触达${agentCapabilityLabel(suspiciousPath[1])}：${agentResourceScopeLabel(suspiciousPath[2])}`;
  const secretPath = text.match(/^Agent declarations form a potential secret-to-network path: (.+)$/);
  if (secretPath) return `Agent 声明形成潜在的密钥到网络路径：${agentResourceScopeLabel(secretPath[1])}`;
  return AGENT_UI_TEXT_ZH[text] ?? text;
}
function agentResourceScopeLabel(value: string) {
  const [kind, ...scopeParts] = value.trim().split(/\s+/);
  const kindLabel = ({ command: "命令", "command-arguments": "命令参数", network: "网络目标", file: "文件", directory: "目录", secret: "密钥", resource: "资源" } as Record<string, string>)[kind] ?? kind;
  return `${kindLabel}${scopeParts.length ? ` ${scopeParts.join(" ").replaceAll(", ", "、")}` : ""}`;
}
function agentFindingPathId(finding: Finding) {
  return finding.evidence?.match(/(?:^|;\s*)path_id=([^;]+)/)?.[1]?.trim() ?? "";
}
function agentFindingDisplayTitle(finding: Finding) {
  const rawTitle = agentUiText(findingTitle(finding));
  if (!finding.rule_id.startsWith("AGENT.FLOW.")) return rawTitle;
  const capability = finding.evidence?.match(/(?:^|;\s*)capability=([^;]+)/)?.[1]?.trim() ?? "";
  const resource = finding.evidence?.match(/(?:^|;\s*)resource=([^;]+)/)?.[1]?.trim() ?? "";
  if (!resource) return rawTitle;
  const [resourceType, ...scopeParts] = resource.split(":");
  const scope = scopeParts.join(":").replaceAll(", ", "、");
  return `${finding.rule_id === "AGENT.FLOW.UNTRUSTED_TO_HIGH_RISK_TOOL" ? "可疑指令" : "Prompt"}可能触达${agentCapabilityLabel(capability || "敏感能力")}：${agentResourceScopeLabel(`${resourceType} ${scope}`)}`;
}
function agentAuditItemPathId(item: Pick<AgentOfflineAuditItem, "evidence_refs"> | Pick<AgentOfflineAuditDiffItem, "evidence_refs">) {
  return item.evidence_refs.find((value) => value.startsWith("path:"))?.slice(5) ?? "";
}
function visibleAgentAuditItems<T extends Pick<AgentOfflineAuditItem, "id" | "kind" | "evidence_refs">>(items: T[], findings: Finding[]) {
  const representedPaths = new Set(findings.map(agentFindingPathId).filter(Boolean));
  return items.filter((item) => {
    const pathId = agentAuditItemPathId(item);
    if (item.kind === "static-dataflow" && pathId && representedPaths.has(pathId)) return false;
    return true;
  });
}
function agentProfileChangeSummary(saved: AgentProfile | null, current: AgentProfile | null) {
  if (!saved || !current) return [];
  const changes: string[] = [];
  if (JSON.stringify(saved.disabled_rule_ids) !== JSON.stringify(current.disabled_rule_ids)) changes.push("停用规则");
  if (JSON.stringify(saved.excluded_paths) !== JSON.stringify(current.excluded_paths)) changes.push("排除路径");
  if (JSON.stringify(saved.required_approval_capabilities) !== JSON.stringify(current.required_approval_capabilities)) changes.push("强制审批能力");
  if (saved.target_runtime_execution_enabled !== current.target_runtime_execution_enabled) changes.push("真实目标执行入口");
  if (JSON.stringify(saved.permission_allowlist) !== JSON.stringify(current.permission_allowlist)) changes.push("权限 Allowlist");
  if (JSON.stringify(saved.quality_gate) !== JSON.stringify(current.quality_gate)) changes.push("质量门禁");
  return changes;
}
...
async function uploadZipProject(file: File, draft: ProjectDraft): Promise<ProjectImportResult> {
  ...
  const response = await feedbackFetch(`${API_BASE}/projects/import/zip?${query.toString()}`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/zip" }, body: file });
