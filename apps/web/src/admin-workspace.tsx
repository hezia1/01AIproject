...
export function AdminWorkspace({ projectName, users, groups, onOpenKnowledge }: {
  ...
  return <section className="admin-center admin-workspace">
    <nav className="admin-primary-tabs" aria-label="管理中心工作区">
      ...
      <button className={section === "modules" ? "active" : ""} onClick={() => setSection("modules")}>模块配置</button>
      <button onClick={onOpenKnowledge}>知识审核</button>
    </nav>
    {section === "users" ? users : <>
      ...
      <nav className="admin-module-tabs" aria-label="管理员模块选择">
        {groups.map((group) => <button key={group.id} className={group.id === moduleId ? "active" : ""} onClick={() => { setModuleId(group.id); setItemId(""); }}>{group.label}</button>)}
      </nav>
      <nav className="admin-config-tabs" aria-label="管理员配置分类">
        {group?.items.map((candidate) => <button key={candidate.id} className={candidate.id === item?.id ? "active" : ""} onClick={() => setItemId(candidate.id)}>{candidate.label}</button>)}
      </nav>
