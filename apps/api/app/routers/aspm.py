...
def project_report_capability_boundaries() -> dict[str, list[str]]:
    ...
    return {
        ...
        "SAST": [
            "当前结果来自本地规则、Semgrep（可用时）以及已配置时的 DeepSeek 多角色审计。",
            "AI 复核结果是辅助判断和修复草案，不表示已经完成业务数据流或人工最终裁决。",
        ],
        "AGENT": [
            "静态结果来自仓库内指令、MCP、工具与插件配置；经操作员确认后，可在一次性 Docker 目标中执行受控 Agent/MCP 合同探针。",
            "不会连接生产 MCP 或调用真实副作用工具；未接入结构化运行时证据合同的目标不能形成红/绿动态裁决。",
        ],
        "DAST": [
            "当前动态验证使用审批范围内的同源目标、类型专属固定探针、请求预算和证据门槛。",
            "只有满足对应策略证据条件的运行才能形成红/绿裁决；异常信号或不完整运行保持黄色或未验证。",
        ],
        "SANDBOX": [
            "当前记录 Docker 隔离策略、目标生命周期、固定探针事件和 HAR/截图/控制台/时延等结构化证据。",
            "不是 eBPF、Sysmon 或恶意样本级行为探针，不提供完整文件、网络、进程、环境变量或系统调用观测。",
        ],
        "ASPM / 证据链": [
